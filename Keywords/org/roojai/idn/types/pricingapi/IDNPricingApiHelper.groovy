			...
			if(!IGNUemaHelper.checkObjectNullOfObject(lCell)){
				if((lCellType==CellType.NUMERIC)){
					...
					if(DateUtil.isCellDateFormatted(lCell)){
						lCellValue=IGNUemaHelper.getStrDataFormatterOfExcelCell(lCell)
						LocalDate lLocalDate=IGNUemaHelper.convertDateStringtoLocalDate('MM/dd/yy',lCellValue)
						lCellValue=IGNUemaHelper.convertLocalDateToString('yyyy-MM-dd',lLocalDate)
					}else{
