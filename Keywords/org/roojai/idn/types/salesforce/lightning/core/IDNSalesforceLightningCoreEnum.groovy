...
public class IDNSalesforceLightningCoreEnum{
	...
	public enum IDNEnumSalesforceProductType{
		MotorCar('MotorCar'),
		MotorBike('MotorBike'),
		PersonalAccident('PersonalAccident'),
		CriticalIllness('CriticalIllness'),
		Cancer('Cancer'),
		ElectricMotorCar('ElectricMotorCar')
		private final String lStrName
		private IDNEnumSalesforceProductType(String strName){
			try{
				this.lStrName=strName
			}catch(Exception e){
			}
		...
		public static IDNEnumSalesforceProductType fromRefNameToEnum(String strRefName){
			IDNEnumSalesforceMode lreturn=null
			if(strRefName==null) return lreturn
			if(strRefName.length()<=0) return lreturn
			try{
				try{
					IDNEnumSalesforceProductType[] lArrayValueAll=IDNEnumSalesforceProductType.values()
					for(IDNEnumSalesforceMode lArrayValueItem in lArrayValueAll){
						String lStrRefString=lArrayValueItem.toRefString()
						if(lStrRefString.equals(strRefName)){
							lreturn=lArrayValueItem
		...
		public static String fromRefNameToString(String strRefName){
			...
			if(strRefName.length()<=0) return lreturn
			try{
				IDNEnumSalesforceProductType lEnumValue=this.fromRefNameToEnum(strRefName)
				if(lEnumValue!=null){
					lreturn=lEnumValue.toString()
				}
			}catch(Exception e){
