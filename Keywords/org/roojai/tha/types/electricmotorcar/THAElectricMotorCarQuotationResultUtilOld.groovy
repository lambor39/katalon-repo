...
public class THAElectricMotorCarQuotationResultUtilOld{
	...
	public THAElectricMotorCarQuotationResultUtilOld(WebDriver newDriver,WebDriver newSelenium){
		super()
		...
		try{
			...
			if(lIsVisibleMrKumkaCompareButton){
				...
				if(lIsKumkaCompareToDo){
					...
					if(THAElectricMotorCarCoreType.CURRENT_QUOTATION_KUMKA_LOW_CONVERSION_COMPARE_PLAN){
			...
			if(lIsActualResult){
				...
				if(lIsSalesforceProceedTodo){
					...
					THAElectricMotorCarCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_GENERAL=caseOutput.SalesforceVerifyDataOpportunityObjQuoteAccessUrlC
					THAElectricMotorCarCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_PAYMENT=caseOutput.SalesforceVerifyDataOpportunityObjPayOnlineUrlC
					THAElectricMotorCarCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_SOCIAL=caseOutput.SalesforceVerifyDataOpportunityObjCustomerRatingUrlC
					...
					caseOutput.ActualResult=lActualResult
					if(lIsActualResult){
						lIsChangedFromAnyToDealerWorkshop=caseOutput.SalesforceVerifyDataTransactionSyncedObjPolicyProductVersionC>=THAElectricMotorCarCoreType.CURRENT_QUOTATION_POLICY_PRODUCT_VERSION
						...
						if(lIsChangedFromAnyToDealerWorkshop&&lIsDealerWorkshop){
							...
							THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_WORKSHOP_VALUE=lAutomationWorkshopValue
							caseOutput.PlanGarage=lPlanGarage
							caseOutput.AutomationWorkshopValue=lAutomationWorkshopValue
						}
						Map lMapSalesforceProceedVerifyBefore=this.inputSalesforceProceedVerifyBefore(caseOutput)
						lMapSalesforceProceedVerifyBefore.Result=true
					...
					if(lIsActualResult){
						...
						if(lIsSalesforceProceedRetrieveTodo){
							Map lMapSalesforceProceedRetrieveWebFromUrl=this.inputSalesforceProceedRetrieveWebFromUrl(THAElectricMotorCarCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_GENERAL)
	...
	public Map inputPositiveCase(String strPositiveCase){
		...
		if(lPositiveCase.length()<=0){
			lPositiveCase=THAElectricMotorCarQuotationResultType.DEFAULT_POSITIVE_CASE
		}
		try{
	...
	public Map inputActualResultA(Boolean isPositiveCase,String strQuoteBy){
		...
		try{
			...
			lreturn.put('Result',lResult)
			if(lQuoteBy.length()<=0){
				lQuoteBy=THAElectricMotorCarQuotationResultType.DEFAULT_QUOTE_BY.toUpperCase()
			}
			...
			if(!lElementInfo){
				...
				THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ROOJAI_REWARD_POINT=lInfoText
				lVersion=lVersion+'|Roojai Rewards='+lInfoText
				lIsOK=lIsOK&&(lInfoText.length()>0)
			}
			...
			if(lQuoteBy.equalsIgnoreCase('NONE')){
				if(IGNUemaHelper.selectButtonByLocator(this.driver,IGNUemaHelper.getTagDataSeleniumKey('buyNow'))){
					...
					if(!lRedirected){
						lRedirected=IGNUemaHelper.checkElementPresentByElementId(this.driver,'pd1ev-dorothy',10)
					}
					if(isPositiveCase){
	...
	public Map inputLanguage(String strLanguage){
		...
		if(lLanguage.length()<=0){
			lLanguage=THAElectricMotorCarQuotationResultType.DEFAULT_LANGUAGE
		}
		...
		try{
			...
			if(!lLanguageItem){
				lLanguageItem=lLanguageList.get(THAElectricMotorCarQuotationResultType.DEFAULT_LANGUAGE)
			}
	...
	public Map inputPrePackage(String strPrePackage){
		...
		if(lPrePackage.length()<=0){
			lPrePackage=THAElectricMotorCarQuotationResultType.DEFAULT_PRE_PACKAGE
		}
	...
	public Map inputPostPrePackageA(){
		...
		try{
			...
			lRedirected=IGNUemaHelper.checkElementPresentByElementId(this.driver,'evqrp2dorothy',10)
			if(!lRedirected){
				return lreturn
			}
			String lTagPaymentOptionMotorSwitchButton='paymentOptions'
	...
	public Map inputPlanTypeA(String strPlanType){
		...
		if(!lPlanTypeItem){
			lPlanTypeItem=lPlanTypeList.get(THAElectricMotorCarQuotationResultType.DEFAULT_PLAN_TYPE.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PlanType','')
	...
	public Map inputPlanGarageA(String strPlanGarage){
		...
		if(!lPlanGarageItem){
			lPlanGarageItem=lPlanGarageList.get(THAElectricMotorCarQuotationResultType.DEFAULT_PLAN_GARAGE.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PlanGarage','')
	...
	public Map inputPlanExcessA(String strPlanExcess){
		...
		if(!lPlanExcessItem){
			lPlanExcessItem=lPlanExcessList.get(THAElectricMotorCarQuotationResultType.DEFAULT_PLAN_EXCESS.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PlanExcess','')
			...
			String lLocatorButton=IGNUemaHelper.getTagDataSeleniumKey(lTagButton)
			String lTagDropdownList='excess-item'
			String lLocatorDropdownList=IGNUemaHelper.getTagDataSeleniumKey(lTagDropdownList)
	...
	public Map inputPlanDriverA(String strPlanDriver){
		...
		if(!lPlanDriverItem){
			lPlanDriverItem=lPlanDriverList.get(THAElectricMotorCarQuotationResultType.DEFAULT_PLAN_DRIVER.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PlanDriver','')
	...
	public Map inputCompulsoryA(String strCompulsory){
		...
		if(lCompulsory.length()<=0){
			lCompulsory=THAElectricMotorCarQuotationResultType.DEFAULT_COMPULSORY
		}
		try{
			Boolean lResult=false
			lreturn.put('Compulsory','')
	...
	public Map inputWithRsaA(String strWithRsa){
		...
		if(lWithRsa.length()<=0){
			lWithRsa=THAElectricMotorCarQuotationResultType.DEFAULT_WITH_RSA
		}
		try{
			Boolean lResult=false
			lreturn.put('WithRsa','')
	...
	public Map inputVoluntaryTpbiA(String strVoluntaryTpbi){
		...
		if(!lVoluntaryTpbiItem){
			lVoluntaryTpbiItem=lVoluntaryTpbiList.get(THAElectricMotorCarQuotationResultType.DEFAULT_VOLUNTARY_TPBI.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('VoluntaryTpbi','')
	...
	public Map inputPersonalAccidentPaAndMeA(String strPersonalAccidentPaAndMe){
		...
		if(!lPersonalAccidentPaAndMeItem){
			lPersonalAccidentPaAndMeItem=lPersonalAccidentPaAndMeList.get(THAElectricMotorCarQuotationResultType.DEFAULT_PERSONAL_ACCIDENT_PA_AND_ME.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PersonalAccidentPaAndMe','')
			...
			if(!lElementValue){
				...
				IGNUemaHelper.delayThreadSecond(1)
				String lValueStr=IGNUemaHelper.getDataSeleniumValueWebElement(this.driver,lElementValue)
				String lTagButtonTarget='mepa-item'
	...
	public Map inputPersonalAccidentPaOnlyA(String strPersonalAccidentPaOnly){
		...
		if(!lPersonalAccidentPaOnlyItem){
			lPersonalAccidentPaOnlyItem=lPersonalAccidentPaOnlyList.get(THAElectricMotorCarQuotationResultType.DEFAULT_PERSONAL_ACCIDENT_PA_ONLY.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PersonalAccidentPaOnly','')
	...
	public Map inputPersonalAccidentMeOnlyA(String strPersonalAccidentMeOnly){
		...
		if(!lPersonalAccidentMeOnlyItem){
			lPersonalAccidentMeOnlyItem=lPersonalAccidentMeOnlyList.get(THAElectricMotorCarQuotationResultType.DEFAULT_PERSONAL_ACCIDENT_ME_ONLY.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PersonalAccidentMeOnly','')
	...
	public Map inputCarRentalServiceA(String strCarRentalService){
		...
		if(lCarRentalService.length()<=0){
			lCarRentalService=THAElectricMotorCarQuotationResultType.DEFAULT_CAR_RENTAL_SERVICE
		}
		try{
			Boolean lResult=false
			lreturn.put('CarRentalService','')
	...
	public Map inputWallChargerCoverage(String strWallChargerCoverage){
		...
		try{
			...
			if(IGNUemaHelper.CURRENT_AB_TESTING_ENABLE){
				switch(IGNUemaHelper.CURRENT_AB_TESTING_VERSION){
					case 'A':
						lreturn=this.inputWallChargerCoverageA(strWallChargerCoverage)
						break
					case 'B':
						lreturn=this.inputWallChargerCoverageB(strWallChargerCoverage)
						break
					default:
						return lreturn
						break
	...
	public Map inputWallChargerCoverageA(String strWallChargerCoverage){
		Map lreturn=[:]
		String lWallChargerCoverage=strWallChargerCoverage.trim()
		if(lWallChargerCoverage.length()<=0){
			lWallChargerCoverage=THAElectricMotorCarQuotationResultType.DEFAULT_WALL_CHARGER_COVERAGE
		}
		Map lWallChargerCoverageList=['1':'0','2':'20000','3':'30000','4':'40000','5':'50000']
		try{
			Boolean lResult=false
			lreturn.put('WallChargerCoverage','')
			lreturn.put('Result',lResult)
			String lWallChargerCoverageItem=lWallChargerCoverageList.get(lWallChargerCoverage)
			if(!lWallChargerCoverageItem){
				lWallChargerCoverageItem=lWallChargerCoverageList.get(THAElectricMotorCarQuotationResultType.DEFAULT_WALL_CHARGER_COVERAGE)
			}
			String lTagButton='wallcharger'
			String lLocatorButton=IGNUemaHelper.getTagDataSeleniumKey(lTagButton)
			lWallChargerCoverage=''
			WebElement lElementValue=IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,lTagButton)
			if(!lElementValue){
				lWallChargerCoverage='No Option'
			}else{
				IGNUemaHelper.webJsScrollToElement(this.driver,lElementValue)
				IGNUemaHelper.delayThreadSecond(1)
				String lValueStr=IGNUemaHelper.getDataSeleniumValueWebElement(this.driver,lElementValue)
				String lTagButtonTarget='wallcharger-option'
				String lLocatorButtonTarget=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagButtonTarget,lWallChargerCoverageItem)
				WebElement lElementButtonTarget=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagButtonTarget,lWallChargerCoverageItem)
				if(lElementButtonTarget){
					IGNUemaHelper.webJsScrollToElement(this.driver,lElementButtonTarget)
					IGNUemaHelper.delayThreadSecond(1)
					IGNUemaHelper.webJsClickWebElementFinalWithScroll(this.driver,lElementButtonTarget)
				...
				lWallChargerCoverage=lValueStr
			}
	...
	public Map inputWallChargerCoverageB(String strWallChargerCoverage){
		Map lreturn=[:]
		String lWallChargerCoverage=strWallChargerCoverage.trim()
		if(lWallChargerCoverage.length()<=0){
			lWallChargerCoverage=THAElectricMotorCarQuotationResultType.DEFAULT_WALL_CHARGER_COVERAGE
	...
	public Map inputAccessoryAddA(String strAccessoryAdd,String strAccessoryCarryBoyAdd,String strAccessoryCarryBoyOption,String strAccessoryGasFuelingSystemAdd,String strAccessoryGasFuelingSystemOption,String strAccessoryBodyKitAdd,String strAccessoryBodyKitOption,String strAccessoryRimsTiresAdd,String strAccessoryRimsTiresOption,String strAccessorySolarFilmAdd,String strAccessorySolarFilmOption,String strAccessorySuspensionStabilizerSystemAdd,String strAccessorySuspensionStabilizerSystemOption,String strAccessoryInVehicleEntertainmentSystemAdd,String strAccessoryInVehicleEntertainmentSystemOption,String strAccessoryAirIntakeExhaustSystemAdd,String strAccessoryAirIntakeExhaustSystemOption,String strAccessoryCarCameraAdd,String strAccessoryCarCameraOption){
		...
		if(lAccessoryAdd.length()<=0){
			lAccessoryAdd=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_ADD
		}
		String lAccessoryCarryBoyAdd=strAccessoryCarryBoyAdd.trim()
		if(lAccessoryCarryBoyAdd.length()<=0){
			lAccessoryCarryBoyAdd=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_CARRY_BOY_ADD
		}
		String lAccessoryCarryBoyOption=strAccessoryCarryBoyOption.trim()
		if(lAccessoryCarryBoyOption.length()<=0){
			lAccessoryCarryBoyOption=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_CARRY_BOY_OPTION
		}
		String lAccessoryGasFuelingSystemAdd=strAccessoryGasFuelingSystemAdd.trim()
		if(lAccessoryGasFuelingSystemAdd.length()<=0){
			lAccessoryGasFuelingSystemAdd=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_GAS_FUELING_SYSTEM_ADD
		}
		...
		if(lAccessoryGasFuelingSystemOption.length()<=0){
			lAccessoryGasFuelingSystemOption=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_GAS_FUELING_SYSTEM_OPTION
		}
		String lAccessoryBodyKitAdd=strAccessoryBodyKitAdd.trim()
		if(lAccessoryBodyKitAdd.length()<=0){
			lAccessoryBodyKitAdd=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_BODY_KIT_ADD
		}
		String lAccessoryBodyKitOption=strAccessoryBodyKitOption.trim()
		if(lAccessoryBodyKitOption.length()<=0){
			lAccessoryBodyKitOption=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_BODY_KIT_OPTION
		}
		String lAccessoryRimsTiresAdd=strAccessoryRimsTiresAdd.trim()
		if(lAccessoryRimsTiresAdd.length()<=0){
			lAccessoryRimsTiresAdd=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_RIMS_TIRES_ADD
		}
		String lAccessoryRimsTiresOption=strAccessoryRimsTiresOption.trim()
		if(lAccessoryRimsTiresOption.length()<=0){
			lAccessoryRimsTiresOption=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_RIMS_TIRES_OPTION
		}
		String lAccessorySolarFilmAdd=strAccessorySolarFilmAdd.trim()
		if(lAccessorySolarFilmAdd.length()<=0){
			lAccessorySolarFilmAdd=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_SOLAR_FILM_ADD
		}
		String lAccessorySolarFilmOption=strAccessorySolarFilmOption.trim()
		if(lAccessorySolarFilmOption.length()<=0){
			lAccessorySolarFilmOption=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_SOLAR_FILM_OPTION
		}
		...
		if(lAccessorySuspensionStabilizerSystemAdd.length()<=0){
			lAccessorySuspensionStabilizerSystemAdd=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_SUSPENSION_STABILIZER_SYSTEM_ADD
		}
		...
		if(lAccessorySuspensionStabilizerSystemOption.length()<=0){
			lAccessorySuspensionStabilizerSystemOption=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_SUSPENSION_STABILIZER_SYSTEM_OPTION
		}
		...
		if(lAccessoryInVehicleEntertainmentSystemAdd.length()<=0){
			lAccessoryInVehicleEntertainmentSystemAdd=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_IN_VEHICLE_ENTERTAINMENT_SYSTEM_ADD
		}
		...
		if(lAccessoryInVehicleEntertainmentSystemOption.length()<=0){
			lAccessoryInVehicleEntertainmentSystemOption=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_IN_VEHICLE_ENTERTAINMENT_SYSTEM_OPTION
		}
		...
		if(lAccessoryAirIntakeExhaustSystemAdd.length()<=0){
			lAccessoryAirIntakeExhaustSystemAdd=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_AIR_INTAKE_EXHAUST_SYSTEM_ADD
		}
		...
		if(lAccessoryAirIntakeExhaustSystemOption.length()<=0){
			lAccessoryAirIntakeExhaustSystemOption=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_AIR_INTAKE_EXHAUST_SYSTEM_OPTION
		}
		String lAccessoryCarCameraAdd=strAccessoryCarCameraAdd.trim()
		if(lAccessoryCarCameraAdd.length()<=0){
			lAccessoryCarCameraAdd=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_CAR_CAMERA_ADD
		}
		String lAccessoryCarCameraOption=strAccessoryCarCameraOption.trim()
		if(lAccessoryCarCameraOption.length()<=0){
			lAccessoryCarCameraOption=THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_CAR_CAMERA_OPTION
		}
		String lAccessoryAmount=''
		String lAccessoryDetail=''
		...
		try{
			...
			if(!lElementAccessoryAddButtonOpen){
				...
				if(!lAccessoryCarryBoyOptionItem){
					lAccessoryCarryBoyOptionItem=lAccessoryItemOptionList.get(THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_CARRY_BOY_OPTION)
				}
				...
				if(!lAccessoryGasFuelingSystemOptionItem){
					lAccessoryGasFuelingSystemOptionItem=lAccessoryItemOptionList.get(THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_GAS_FUELING_SYSTEM_OPTION)
				}
				String lAccessoryBodyKitOptionItem=lAccessoryItemOptionList.get(lAccessoryBodyKitOption)
				if(!lAccessoryBodyKitOptionItem){
					lAccessoryBodyKitOptionItem=lAccessoryItemOptionList.get(THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_BODY_KIT_OPTION)
				}
				...
				if(!lAccessoryRimsTiresOptionItem){
					lAccessoryRimsTiresOptionItem=lAccessoryItemOptionList.get(THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_RIMS_TIRES_OPTION)
				}
				...
				if(!lAccessorySolarFilmOptionItem){
					lAccessorySolarFilmOptionItem=lAccessoryItemOptionList.get(THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_SOLAR_FILM_OPTION)
				}
				...
				if(!lAccessorySuspensionStabilizerSystemOptionItem){
					lAccessorySuspensionStabilizerSystemOptionItem=lAccessoryItemOptionList.get(THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_SUSPENSION_STABILIZER_SYSTEM_OPTION)
				}
				...
				if(!lAccessoryInVehicleEntertainmentSystemOptionItem){
					lAccessoryInVehicleEntertainmentSystemOptionItem=lAccessoryItemOptionList.get(THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_IN_VEHICLE_ENTERTAINMENT_SYSTEM_OPTION)
				}
				...
				if(!lAccessoryAirIntakeExhaustSystemOptionItem){
					lAccessoryAirIntakeExhaustSystemOptionItem=lAccessoryItemOptionList.get(THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_AIR_INTAKE_EXHAUST_SYSTEM_OPTION)
				}
				...
				if(!lAccessoryCarCameraOptionItem){
					lAccessoryCarCameraOptionItem=lAccessoryItemOptionList.get(THAElectricMotorCarQuotationResultType.DEFAULT_ACCESSORY_CAR_CAMERA_OPTION)
				}
			...
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_AMOUNT=lAccessoryAmount
	...
	public Map inputQuoteMeA(String strQuoteBy,String strQuotePhoneNo,String strQuoteEmail,String strQuoteNewsletter,String strQuoteCallMeNow,String strQuotePDFDownloadToDo){
		...
		if(lQuotePhoneNo.length()<=0){
			lQuotePhoneNo=THAElectricMotorCarQuotationResultType.DEFAULT_QUOTE_PHONE_NO
		}
		String lQuoteEmail=strQuoteEmail.trim()
		if(lQuoteEmail.length()<=0){
			lQuoteEmail=THAElectricMotorCarQuotationResultType.DEFAULT_QUOTE_EMAIL
		}
		String lQuoteNewsletter=strQuoteNewsletter.trim()
		if(lQuoteNewsletter.length()<=0){
			lQuoteNewsletter=THAElectricMotorCarQuotationResultType.DEFAULT_QUOTE_NEWSLETTER
		}
		String lQuoteCallMeNow=strQuoteCallMeNow.trim()
		if(lQuoteCallMeNow.length()<=0){
			lQuoteCallMeNow=THAElectricMotorCarQuotationResultType.DEFAULT_QUOTE_CALL_ME_NOW
		}
		String lQuotePDFDownloadToDo=strQuotePDFDownloadToDo.trim()
		if(lQuotePDFDownloadToDo.length()<=0){
			lQuotePDFDownloadToDo=THAElectricMotorCarQuotationResultType.DEFAULT_QUOTE_PDF_DOWNLOAD_TO_DO
		}
		String lQuotePDFDownloadDetail=''
		String lQuotePDFDownloadResult=''
		try{
			...
			switch(lQuoteBy){
				...
				case 'EMAIL':
					lQuoteBy='Email'
					String lLocatorEmailModal='#save4form1'
					WebElement lElementEmailModal=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorEmailModal)
					if(!lElementEmailModal){
						return lreturn
					}
					...
					if(IGNUemaHelper.selectButtonByLocator(this.driver,lLocatorButtonEmailMe)){
						...
						WebElement lElementTextEmailMeEmail=IGNUemaHelper.getChildWebElementFromDataSeleniumKey(this.driver,lElementEmailModal,lTagTextEmailMeEmail)
						...
						lQuoteEmail=IGNUemaHelper.typeTextByWebElement(this.driver,lElementTextEmailMeEmail,lQuoteEmail)
						...
						WebElement lElementCheckboxEmailMeNewsletter=IGNUemaHelper.getChildWebElementFromDataSeleniumKey(this.driver,lElementEmailModal,lTagCheckboxEmailMeNewsletter)
						...
						WebElement lElementQuotePDFDownload=IGNUemaHelper.getChildWebElementFromDataSeleniumKey(this.driver,lElementEmailModal,lTagQuotePDFDownload)
						if(lElementQuotePDFDownload){
							...
							if(lIsQuotePDFDownloadToDo){
								lElementQuotePDFDownload=IGNUemaHelper.getChildWebElementFromDataSeleniumKey(this.driver,lElementEmailModal,lTagQuotePDFDownload)
								if(!lElementQuotePDFDownload){
									return lreturn
								}
						...
						WebElement lElementButtonEmailMeEmailNext=IGNUemaHelper.getChildWebElementFromDataSeleniumKey(this.driver,lElementEmailModal,lTagButtonEmailMeEmailNext)
						if(IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementButtonEmailMeEmailNext)){
							IGNUemaHelper.delayThreadSecond(1)
							String lLocatorPhoneModal='#ask4phone'
							WebElement lElementPhoneModal=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPhoneModal)
							String lTagTextEmailMePhoneNo='savequote-v4d-mobile'
							String lLocatorTextEmailMePhoneNo=IGNUemaHelper.getTagDataSeleniumKey(lTagTextEmailMePhoneNo)
							WebElement lElementTextEmailMePhoneNo=IGNUemaHelper.getChildWebElementFromDataSeleniumKey(this.driver,lElementPhoneModal,lTagTextEmailMePhoneNo)
							String lTagButtonEmailMeGoBack=''
							String lLocatorButtonEmailMeGoBack=''
							WebElement lElementButtonEmailMeGoBack=null
							if(lElementTextEmailMePhoneNo){
								lQuotePhoneNo=IGNUemaHelper.typeTextByWebElement(this.driver,lElementTextEmailMePhoneNo,lQuotePhoneNo)
								...
								WebElement lElementButtonEmailMePhoneNext=IGNUemaHelper.getChildWebElementFromDataSeleniumKey(this.driver,lElementPhoneModal,lTagButtonEmailMePhoneNext)
								IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementButtonEmailMePhoneNext)
								IGNUemaHelper.delayThreadSecond(1)
								...
								lElementButtonEmailMeGoBack=IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,lTagButtonEmailMeGoBack)
							}
							IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementButtonEmailMeGoBack)
							lResult=true
							IGNUemaHelper.delayThreadSecond(1)
							IGNUemaHelper.webJsScrollToTop(this.driver)
						}
					...
					break
				default:
					lQuoteBy=THAElectricMotorCarQuotationResultType.DEFAULT_QUOTE_BY
					lQuotePhoneNo=''
					lQuoteEmail=''
					lQuoteNewsletter=''
					lQuoteCallMeNow=''
			...
			if(THAElectricMotorCarCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER.toUpperCase().contains('MC')||THAElectricMotorCarCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER.toUpperCase().contains('TBA')){
	...
	public Map inputSalesforceProceedTodo(String strSalesforceProceedTodo){
		...
		if(lSalesforceProceedTodo.length()<=0){
			lSalesforceProceedTodo=THAElectricMotorCarQuotationResultType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
			...
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER=this.checkOpportunityNumber(THAElectricMotorCarCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER)
	...
	public Map inputAutomationDataBefore(){
		...
		try{
			...
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PAGE_VALUE=lAutomationPageValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PAGE_TEXT=lAutomationPageText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_VERSION_VALUE=lAutomationVersionValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_VERSION_TEXT=lAutomationVersionText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_OPPORTUNITY_NUMBER_VALUE=lAutomationOpportunityNumberValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_OPPORTUNITY_NUMBER_TEXT=lAutomationOpportunityNumberText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_VEHICLE_DETAIL_VALUE=lAutomationVehicleDetailValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_VEHICLE_DETAIL_TEXT=lAutomationVehicleDetailText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_YEARLY_VALUE=lAutomationPremiumYearlyValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_YEARLY_TEXT=lAutomationPremiumYearlyText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_DEPOSIT_VALUE=lAutomationPremiumDepositValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_DEPOSIT_TEXT=lAutomationPremiumDepositText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_MONTHLY_VALUE=lAutomationPremiumMonthlyValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_MONTHLY_TEXT=lAutomationPremiumMonthlyText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_INSTALMENT_VALUE=lAutomationPremiumInstalmentValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_INSTALMENT_TEXT=lAutomationPremiumInstalmentText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_NO_DEPOSIT_AMOUNT_VALUE=lAutomationPremiumNoDepositAmountValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_NO_DEPOSIT_AMOUNT_TEXT=lAutomationPremiumNoDepositAmountText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_NO_DEPOSIT_TOTAL_VALUE=lAutomationPremiumNoDepositTotalValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_NO_DEPOSIT_TOTAL_TEXT=lAutomationPremiumNoDepositTotalText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_QUARTERLY_AMOUNT_VALUE=lAutomationPremiumQuarterlyAmountValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_QUARTERLY_AMOUNT_TEXT=lAutomationPremiumQuarterlyAmountText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_QUARTERLY_TOTAL_VALUE=lAutomationPremiumQuarterlyTotalValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_QUARTERLY_TOTAL_TEXT=lAutomationPremiumQuarterlyTotalText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PLAN_TYPE_VALUE=lAutomationPlanTypeValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PLAN_TYPE_TEXT=lAutomationPlanTypeText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_SUM_INSURED_VALUE=lAutomationSumInsuredValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_SUM_INSURED_TEXT=lAutomationSumInsuredText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_WORKSHOP_VALUE=lAutomationWorkshopValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_WORKSHOP_TEXT=lAutomationWorkshopText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_EXCESS_PRICE_VALUE=lAutomationExcessPriceValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_EXCESS_PRICE_TEXT=lAutomationExcessPriceText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_DRIVER_PLAN_VALUE=lAutomationDriverPlanValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_DRIVER_PLAN_TEXT=lAutomationDriverPlanText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_TPBI_VALUE=lAutomationTpbiValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_TPBI_TEXT=lAutomationTpbiText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_TPPD_VALUE=lAutomationTppdValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_TPPD_TEXT=lAutomationTppdText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PA_AND_ME_VALUE=lAutomationPaAndMeValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PA_AND_ME_TEXT=lAutomationPaAndMeText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PA_ONLY_VALUE=lAutomationPaOnlyValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PA_ONLY_TEXT=lAutomationPaOnlyText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ME_ONLY_VALUE=lAutomationMeOnlyValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ME_ONLY_TEXT=lAutomationMeOnlyText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_COMPULSORY_VALUE=lAutomationCompulsoryValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_COMPULSORY_TEXT=lAutomationCompulsoryText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_RSA_VALUE=lAutomationRsaValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_RSA_TEXT=lAutomationRsaText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_CAR_REPLACEMENT_SERVICE_VALUE=lAutomationCarReplacementServiceValue
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_CAR_REPLACEMENT_SERVICE_TEXT=lAutomationCarReplacementServiceText
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_01_VALUE=lAutomationAccessoryItem01Value
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_01_TEXT=lAutomationAccessoryItem01Text
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_02_VALUE=lAutomationAccessoryItem02Value
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_02_TEXT=lAutomationAccessoryItem02Text
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_03_VALUE=lAutomationAccessoryItem03Value
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_03_TEXT=lAutomationAccessoryItem03Text
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_04_VALUE=lAutomationAccessoryItem04Value
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_04_TEXT=lAutomationAccessoryItem04Text
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_05_VALUE=lAutomationAccessoryItem05Value
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_05_TEXT=lAutomationAccessoryItem05Text
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_06_VALUE=lAutomationAccessoryItem06Value
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_06_TEXT=lAutomationAccessoryItem06Text
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_07_VALUE=lAutomationAccessoryItem07Value
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_07_TEXT=lAutomationAccessoryItem07Text
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_08_VALUE=lAutomationAccessoryItem08Value
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_08_TEXT=lAutomationAccessoryItem08Text
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_09_VALUE=lAutomationAccessoryItem09Value
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_09_TEXT=lAutomationAccessoryItem09Text
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_10_VALUE=lAutomationAccessoryItem10Value
			THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ACCESSORY_ITEM_10_TEXT=lAutomationAccessoryItem10Text
			lResult=true
			if(lResult){
		...
		try{
			...
			if(lElementAutomationParent){
				...
				if(!lElementAutomationWorkshop){
					...
					if(lIsChangedFromAnyToDealerWorkshop&&lIsDealerWorkshop){
						lAutomationWorkshopValue=THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_WORKSHOP_VALUE
					}
				}
				String lTagAutomationExcessPrice='ExcessPrice'
				String lLocatorAutomationExcessPrice=IGNUemaHelper.getTagDataSeleniumKey(lTagAutomationExcessPrice)
	...
	public Map inputSalesforceProceedVerifyBefore(Map caseOutput){
		...
		try{
			...
			String lAutomationReward=THAElectricMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_ROOJAI_REWARD_POINT
	...
	public Map inputSalesforceProceedRetrieveTodo(String strSalesforceProceedRetrieveTodo,String strAccountNameAnonymous){
		...
		if(lSalesforceProceedRetrieveTodo.length()<=0){
			lSalesforceProceedRetrieveTodo=THAElectricMotorCarQuotationResultType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
	...
	public Map inputSalesforceProceedRetrieveWebFromUrl(String strRetrievePageURL){
		...
		try{
			...
			try{
				...
				String lLocatorResultPage='#evqrp2dorothy'
				...
				if(lRedirected){
					...
					lRedirected=lCurrentURL.toUpperCase().contains('EV-PACKAGES')
				}
				lResult=lRedirected
			}catch(Exception ex){
				return lreturn
	...
	public Boolean inputExpectedKumkaCompareButton(Map targetMap){
		...
		if(!lExpectedKumkaCompareButtonItem){
			lExpectedKumkaCompareButtonItem=lExpectedKumkaCompareButtonList.get(THAElectricMotorCarQuotationResultType.DEFAULT_EXPECTED_KUMKA_COMPARE_BUTTON.toUpperCase())
		...
		if(!lKumkaCompareToDoItem){
			lKumkaCompareToDoItem=lKumkaCompareToDoList.get(THAElectricMotorCarQuotationResultType.DEFAULT_KUMKA_COMPARE_TO_DO.toUpperCase())
	...
	public Boolean inputKumkaCompareNowV1(String strLanguage,String strEmailToSave,String strCallMeBack,String strCallMeBackPhoneNo,Map targetMap){
		...
		if(lKumkaCompareToDo.length()<=0){
			lKumkaCompareToDo=THAElectricMotorCarQuotationResultType.DEFAULT_KUMKA_COMPARE_TO_DO
		}
		String lKumkaLoginPage=THAElectricMotorCarQuotationResultType.DEFAULT_KUMKA_LOGIN_URL
		String lLoginUser=THAElectricMotorCarQuotationResultType.DEFAULT_KUMKA_LOGIN_USER
		String lLoginPassword=THAElectricMotorCarQuotationResultType.DEFAULT_KUMKA_LOGIN_PASSWORD
		...
		try{
			...
			if(lElementButton){
				...
				if(lCurrentURLText.length()>0){
					THAElectricMotorCarCoreType.CURRENT_URL_UNDER_PAGE_RESULT_NEW_KUMKA=IGNUemaHelper.checkStringContainString(lCurrentURLText,'/CARDSNEW')
				}
				if(THAElectricMotorCarCoreType.CURRENT_URL_UNDER_PAGE_RESULT_NEW_KUMKA){
	...
	public Boolean checkAnonymousAccountToGetActualOpportunityNumberByRedirecting(){
		...
		try{
			Boolean lRedirected=false
			lRedirected=IGNUemaHelper.checkElementPresentByElementId(this.driver,'pd1ev-dorothy',30)
			if(!lRedirected){
			...
			if(lRedirected){
				IGNUemaHelper.selectButtonByLocator(this.driver,IGNUemaHelper.getTagDataSeleniumKey('goback'))
				lRedirected=IGNUemaHelper.checkElementPresentByElementId(this.driver,'evqrp2dorothy',30)
				if(!lRedirected){
					WebUI.back()
				}
				lRedirected=IGNUemaHelper.checkElementPresentByElementId(this.driver,'evqrp2dorothy',30)
