...
public class THAElectricMotorCarQuotationSocialUtilOld{
	...
	public THAElectricMotorCarQuotationSocialUtilOld(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	public Boolean inputSocial(Map caseInput,Map caseOutput){
		...
		try{
			...
			if(!lIsPaymentPromptPayPageVisible){
				...
				String lPaymentMethod=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_METHOD
				String lQrSuccessCase=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_QR_CODE_SUCCESS_CASE
				String lTrueMoneyMobileNo=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_MOBILE_NO
				String lTrueMoneyEmail=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_EMAIL
				String lTrueMoneySuccessCase=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_SUCCESS_CASE
				String lRabbitLinePayMobileNo=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_MOBILE_NO
				String lRabbitLinePayEmail=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_EMAIL
				String lRabbitLinePaySuccessCase=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_SUCCESS_CASE
				String lMobileBankingCard=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_CARD
				String lMobileBankingSuccessCase=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_SUCCESS_CASE
				String lOpportunityNumber=''
				...
				if(lIsSalesforceProceedTodo){
					...
					if(lIsActualResult){
						...
						if(lIsSalesforceProceedRetrieveTodo){
							Map lMapSalesforceProceedRetrieveWebFromUrl=this.inputSalesforceProceedRetrieveWebFromUrl(THAElectricMotorCarCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_SOCIAL)
	...
	public Map inputLanguage(String strLanguage){
		...
		if(lLanguage.length()<=0){
			lLanguage=THAElectricMotorCarQuotationSocialType.DEFAULT_LANGUAGE
		}
		...
		try{
			...
			if(!lLanguageItem){
				lLanguageItem=lLanguageList.get(THAElectricMotorCarQuotationSocialType.DEFAULT_LANGUAGE)
			}
	...
	public Map inputFeefoRating(String strPolicyInsurer,String strRatingForOverall,String strRatingForWebsite,String strRatingForCallCenter,String strRatingForProduct,String strRatingText,String strWithFacebook,String strFacebookLoginId,String strFacebookLoginPassword,String strShareFacebook){
		...
		if(lRatingForOverall.length()<=0){
			lRatingForOverall=THAElectricMotorCarQuotationSocialType.DEFAULT_RATING_FOR_OVERALL
		}
		String lRatingForWebsite=strRatingForWebsite.trim()
		if(lRatingForWebsite.length()<=0){
			lRatingForWebsite=THAElectricMotorCarQuotationSocialType.DEFAULT_RATING_FOR_WEBSITE
		}
		String lRatingForCallCenter=strRatingForCallCenter.trim()
		if(lRatingForCallCenter.length()<=0){
			lRatingForCallCenter=THAElectricMotorCarQuotationSocialType.DEFAULT_RATING_FOR_CALL_CENTER
		}
		String lRatingForProduct=strRatingForProduct.trim()
		if(lRatingForProduct.length()<=0){
			lRatingForProduct=THAElectricMotorCarQuotationSocialType.DEFAULT_RATING_FOR_PRODUCT
		}
		String lRatingText=strRatingText.trim()
		if(lRatingText.length()<=0){
			lRatingText=THAElectricMotorCarQuotationSocialType.DEFAULT_RATING_TEXT
		}
		String lWithFacebook=strWithFacebook.trim()
		if(lWithFacebook.length()<=0){
			lWithFacebook=THAElectricMotorCarQuotationSocialType.DEFAULT_WITH_FACEBOOK
		}
		String lFacebookLoginId=strFacebookLoginId.trim()
		if(lFacebookLoginId.length()<=0){
			lFacebookLoginId=THAElectricMotorCarQuotationSocialType.DEFAULT_FACEBOOK_LOGIN_ID
		}
		String lFacebookLoginPassword=strFacebookLoginPassword.trim()
		if(lFacebookLoginPassword.length()<=0){
			lFacebookLoginPassword=THAElectricMotorCarQuotationSocialType.DEFAULT_FACEBOOK_LOGIN_PASSWORD
		}
		String lShareFacebook=strShareFacebook.trim()
		if(lShareFacebook.length()<=0){
			lShareFacebook=THAElectricMotorCarQuotationSocialType.DEFAULT_SHARE_FACEBOOK
		}
					...
					if(lIsFoundFeefoWindow){
						...
						if(IGNUemaHelper.waitElementVisibleByLocator(this.driver,lLocatorRatingForOverall,30)){
							...
							if(lElementRatingForOverall){
								...
								if(THAElectricMotorCarQuotationDetail2Type.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail2Output)){
	...
	public Map inputPaymentByTrueMoney(String strTrueMoneyMobileNo,String strTrueMoneyEmail,String strTrueMoneySuccessCase){
		...
		if(lTrueMoneyMobileNo.length()<=0){
			lTrueMoneyMobileNo=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_MOBILE_NO
		}
		String lTrueMoneyEmail=strTrueMoneyEmail.trim()
		if(lTrueMoneyEmail.length()<=0){
			lTrueMoneyEmail=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_EMAIL
		}
		String lTrueMoneySuccessCase=strTrueMoneySuccessCase.trim()
		if(lTrueMoneySuccessCase.length()<=0){
			lTrueMoneySuccessCase=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public Map inputPaymentByRabbitLinePay(String strRabbitLinePayMobileNo,String strRabbitLinePayEmail,String strRabbitLinePaySuccessCase){
		...
		if(lRabbitLinePayMobileNo.length()<=0){
			lRabbitLinePayMobileNo=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_MOBILE_NO
		}
		String lRabbitLinePayEmail=strRabbitLinePayEmail.trim()
		if(lRabbitLinePayEmail.length()<=0){
			lRabbitLinePayEmail=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_EMAIL
		}
		String lRabbitLinePaySuccessCase=strRabbitLinePaySuccessCase.trim()
		if(lRabbitLinePaySuccessCase.length()<=0){
			lRabbitLinePaySuccessCase=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public Map inputPaymentByMobileBanking(String strMobileBankingCard,String strMobileBankingSuccessCase){
		...
		if(lMobileBankingCard.length()<=0){
			lMobileBankingCard=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_CARD
		}
		String lMobileBankingSuccessCase=strMobileBankingSuccessCase.trim()
		if(lMobileBankingSuccessCase.length()<=0){
			lMobileBankingSuccessCase=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public String inputPositiveCase(String strPositiveCase){
		...
		try{
			String lText=''
			if(lPositiveCase.length()<=0){
				lPositiveCase=THAElectricMotorCarQuotationSocialType.DEFAULT_POSITIVE_CASE
			}
			Boolean lIsPositiveCase=IGNUemaHelper.convertStringToBoolean(lPositiveCase)
	...
	public Map inputSalesforceProceedTodo(String strSalesforceProceedTodo){
		...
		if(lSalesforceProceedTodo.length()<=0){
			lSalesforceProceedTodo=THAElectricMotorCarQuotationSocialType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
	...
	public Map inputSalesforceProceedVerifyBefore(Map caseOutput){
		...
		try{
			...
			lreturn.put('Result','')
			String lText=''
			String lAutomationOpportunityNumber=THAElectricMotorCarCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER
