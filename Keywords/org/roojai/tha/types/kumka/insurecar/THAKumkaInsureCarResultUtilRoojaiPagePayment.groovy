...
public class THAKumkaInsureCarResultUtilRoojaiPagePayment{
	private WebDriver driver=null
	private WebDriver selenium=null
	private def lMotorCarQuotationPaymentUtil=null
	public THAKumkaInsureCarResultUtilRoojaiPagePayment(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	private THAMotorCarQuotationPaymentUtil getlMotorCarQuotationPaymentUtil(){
		return this.lMotorCarQuotationPaymentUtil
	}
	private void setlMotorCarQuotationPaymentUtil(THAMotorCarQuotationPaymentUtil newlMotorCarQuotationPaymentUtil){
		//Do Nothing
		//this.lMotorCarQuotationPaymentUtil=newlMotorCarQuotationPaymentUtil
	}
	...
	public Boolean inputPayment(Map caseInput,Map caseOutput){
		...
		try{
			...
			if(THAKumkaInsureCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION){
				this.lMotorCarQuotationPaymentUtil=new THAMotorCarQuotationPaymentUtil(this.driver,this.selenium)
			}else{
				this.lMotorCarQuotationPaymentUtil=new THAMotorCarQuotationPaymentUtilOld(this.driver,this.selenium)
			}
			...
			if(lIsSalesforceProceedTodo){
				...
				if(lIsActualResult){
					...
					if(lIsSalesforceProceedRetrieveTodo){
						Map lMapSalesforceProceedRetrieveWebFromUrl=this.inputSalesforceProceedRetrieveWebFromUrl(THAKumkaInsureCarCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_PAYMENT)
			...
			switch(lPaymentMethod){
				...
				case '4':
					...
					THAKumkaInsureCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_MOBILE_NO=lTrueMoneyMobileNo
					THAKumkaInsureCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_EMAIL=lTrueMoneyEmail
					THAKumkaInsureCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_SUCCESS_CASE=lTrueMoneySuccessCase
					if(!lMapPaymentByTrueMoney.Result){
						return lreturn
					}
					break
				case '5':
					...
					THAKumkaInsureCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_MOBILE_NO=lRabbitLinePayMobileNo
					THAKumkaInsureCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_EMAIL=lRabbitLinePayEmail
					THAKumkaInsureCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_SUCCESS_CASE=lRabbitLinePaySuccessCase
					if(!lMapPaymentByRabbitLinePay.Result){
						return lreturn
					}
					break
	...
	public Map inputPositiveCase(String strPositiveCase){
		...
		if(lPositiveCase.length()<=0){
			lPositiveCase=THAMotorCarQuotationPaymentType.DEFAULT_POSITIVE_CASE
		}
		try{
	...
	public Map inputActualResult(Boolean isPositiveCase){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapActualResult=this.lMotorCarQuotationPaymentUtil.inputActualResult(isPositiveCase)
			if(!lMapActualResult.Result){
				lActualResult=''
			}else{
				lActualResult=lMapActualResult.ActualResult
	...
	public Map inputLanguage(String strLanguage){
		...
		if(lLanguage.length()<=0){
			lLanguage=THAMotorCarQuotationPaymentType.DEFAULT_LANGUAGE.toUpperCase()
		}
		try{
			Boolean lResult=false
			lreturn.put('Language','')
			lreturn.put('Result',lResult)
			Map lMapLanguage=this.lMotorCarQuotationPaymentUtil.inputLanguage(lLanguage)
			if(!lMapLanguage.Result){
				lLanguage=''
			}else{
				lLanguage=lMapLanguage.Language
	...
	public Map inputOpportunityNumber(){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapOpportunityNumber=this.lMotorCarQuotationPaymentUtil.inputOpportunityNumber()
			if(!lMapOpportunityNumber.Result){
				lOpportunityNumber=''
			}else{
				lOpportunityNumber=lMapOpportunityNumber.OpportunityNumber
	...
	public Map inputPaymentMethod(String strPaymentMethod){
		...
		if(lPaymentMethod.length()<=0){
			lPaymentMethod=THAMotorCarQuotationPaymentType.DEFAULT_PAYMENT_METHOD
		}
		try{
			Boolean lResult=false
			lreturn.put('PaymentMethod','')
			lreturn.put('Result',lResult)
			Map lMapPaymentMethod=this.lMotorCarQuotationPaymentUtil.inputPaymentMethod(lPaymentMethod)
			if(!lMapPaymentMethod.Result){
				lPaymentMethod=''
			}else{
				lPaymentMethod=lMapPaymentMethod.PaymentMethod
	...
	public Map inputPaymentByQR(String strEPolicy){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapPaymentByQR=this.lMotorCarQuotationPaymentUtil.inputPaymentByQR(lEPolicy)
			if(!lMapPaymentByQR.Result){
				lEPolicy=''
				lQrAmtYearly=''
			}else{
	...
	public Map inputPaymentByIB(String strIbBank,String strEPolicy,String strAutoRenew,String strIbSuccessCase){
		...
		if(lIbBank.length()<=0){
			lIbBank=THAMotorCarQuotationPaymentType.DEFAULT_IB_BANK.toUpperCase()
		}
		String lEPolicy=strEPolicy.trim()
		if(lEPolicy.length()<=0){
			lEPolicy=THAMotorCarQuotationPaymentType.DEFAULT_E_POLICY
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapPaymentByIB=this.lMotorCarQuotationPaymentUtil.inputPaymentByIB(lIbBank,lEPolicy,lAutoRenew,lIbSuccessCase)
			if(!lMapPaymentByIB.Result){
				lIbBank=''
				lEPolicy=''
				lAutoRenew=''
	...
	public Map inputPaymentByCC(String strCcIsYearly,String strCcInstalment,String strCcNumber,String strCcName,String strCcExpiryMonth,String strCcExpiryYear,String strCcCvv,String strCcBank,String strEPolicy,String strAutoRenew,String strCcSuccessCase){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapPaymentByCC=this.lMotorCarQuotationPaymentUtil.inputPaymentByCC(lCcIsYearly,lCcInstalment,lCcNumber,lCcName,lCcExpiryMonth,lCcExpiryYear,lCcCvv,lCcBank,lEPolicy,lAutoRenew,lCcSuccessCase)
			if(!lMapPaymentByCC.Result){
				lCcIsYearly=''
				lCcInstalment=''
				lCcNumber=''
	...
	public Map inputPolicyInsurer(String strPolicyInsurer){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapPolicyInsurer=this.lMotorCarQuotationPaymentUtil.inputPolicyInsurer(lPolicyInsurer)
			if(!lMapPolicyInsurer.Result){
				lPolicyInsurer=''
			}else{
				lPolicyInsurer=lMapPolicyInsurer.PolicyInsurer
	...
	public Map inputSalesforceProceedTodo(String strSalesforceProceedTodo){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedTodo=this.lMotorCarQuotationPaymentUtil.inputSalesforceProceedTodo(lSalesforceProceedTodo)
			if(!lMapSalesforceProceedTodo.Result){
				lSalesforceProceedTodo=''
			}else{
				lSalesforceProceedTodo=lMapSalesforceProceedTodo.SalesforceProceedTodo
	...
	public Map inputAutomationDataBefore(){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result',lResult)
			Map lMapAutomationDataBefore=this.lMotorCarQuotationPaymentUtil.inputAutomationDataBefore()
	...
	public Map inputAutomationDataAfter(){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapAutomationDataAfter=this.lMotorCarQuotationPaymentUtil.inputAutomationDataAfter()
			if(!lMapAutomationDataAfter.Result){
				lSalesforceRetrieveDataOpportunityNumber=''
				lSalesforceRetrieveDataPremiumYearly=''
				lSalesforceRetrieveDataPremiumDeposit=''
	...
	public Map inputSalesforceProceedVerifyBefore(Map caseOutput){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedVerifyBefore=this.lMotorCarQuotationPaymentUtil.inputSalesforceProceedVerifyBefore(caseOutput)
	...
	public Map inputSalesforceProceedVerifyAfter(Map caseOutput){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedVerifyAfter=this.lMotorCarQuotationPaymentUtil.inputSalesforceProceedVerifyAfter(caseOutput)
	...
	public Map inputSalesforceProceedRetrieveTodo(String strSalesforceProceedRetrieveTodo,String strAccountNameAnonymous){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedRetrieveTodo=this.lMotorCarQuotationPaymentUtil.inputSalesforceProceedRetrieveTodo(lSalesforceProceedRetrieveTodo,strAccountNameAnonymous)
			if(!lMapSalesforceProceedRetrieveTodo.Result){
				lSalesforceProceedRetrieveTodo=''
			}else{
				lSalesforceProceedRetrieveTodo=lMapSalesforceProceedRetrieveTodo.SalesforceProceedRetrieveTodo
	...
	public Map inputSalesforceProceedRetrieveWebFromUrl(String strRetrievePageURL){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedRetrieveWebFromUrl=this.lMotorCarQuotationPaymentUtil.inputSalesforceProceedRetrieveWebFromUrl(lRetrievePageURL)
