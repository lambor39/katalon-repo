...
public class THAMotorBikeQuotationResultUtil{
	private WebDriver driver=null
	private WebDriver selenium=null
	private def lTHAKumkaInsureCarResultUtil=null
	public THAMotorBikeQuotationResultUtil(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	public String checkAmountTotal(String strAmountTotal){
		String lreturn=''
		String lAmountTotal=strAmountTotal.trim()
		try{
			String lNumber=lAmountTotal
			lNumber=lNumber.replaceAll('[^0-9.]','').trim()
			if(lNumber.length()>0){
				lreturn=lNumber
			}
		}catch(Exception e){
		...
		try{
			...
			if(lIsActualResult){
				...
				if(lIsSalesforceProceedTodo){
					...
					THAMotorBikeCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_GENERAL=caseOutput.SalesforceVerifyDataOpportunityObjQuoteAccessUrlC
					THAMotorBikeCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_PAYMENT=caseOutput.SalesforceVerifyDataOpportunityObjPayOnlineUrlC
					THAMotorBikeCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_SOCIAL=caseOutput.SalesforceVerifyDataOpportunityObjCustomerRatingUrlC
					...
					if(lIsActualResult){
						...
						if(lIsSalesforceProceedRetrieveTodo){
							Map lMapSalesforceProceedRetrieveWebFromUrl=this.inputSalesforceProceedRetrieveWebFromUrl(THAMotorBikeCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_GENERAL)
	...
	public Map inputPositiveCase(String strPositiveCase){
		...
		if(lPositiveCase.length()<=0){
			lPositiveCase=THAMotorBikeQuotationResultType.DEFAULT_POSITIVE_CASE
		}
		try{
	...
	public Map inputActualResultA(Boolean isPositiveCase,String strQuoteBy){
		...
		try{
			...
			lreturn.put('Result',lResult)
			if(lQuoteBy.length()<=0){
				lQuoteBy=THAMotorBikeQuotationResultType.DEFAULT_QUOTE_BY.toUpperCase()
			}
			...
			String lSumInsured=''
			String lTagSumInsured=''
			String lLocatorSumInsured='#section-1 .sub2text'
			lSumInsured=IGNUemaHelper.getChildTextByLocator(driver,lLocatorSumInsured)
			THAMotorBikeCoreType.CURRENT_QUOTATION_PREMIUM_SUM_INSURED_VALUE=lSumInsured
			...
			if(!lElementInfo){
				...
				THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_ROOJAI_REWARD_POINT=lInfoText
				lVersion=lVersion+'|Roojai Rewards='+lInfoText
				lIsOK=lIsOK&&(lInfoText.length()>0)
			}
	...
	public Map inputLanguage(String strLanguage){
		...
		if(lLanguage.length()<=0){
			lLanguage=THAMotorBikeQuotationResultType.DEFAULT_LANGUAGE
		}
		...
		try{
			...
			if(!lLanguageItem){
				lLanguageItem=lLanguageList.get(THAMotorBikeQuotationResultType.DEFAULT_LANGUAGE)
			}
	...
	public Map inputPrePackage(String strPrePackage){
		...
		if(lPrePackage.length()<=0){
			lPrePackage=THAMotorBikeQuotationResultType.DEFAULT_PRE_PACKAGE
		}
	...
	public Map inputPlanTypeA(String strPlanType){
		...
		if(!lPlanTypeItem){
			lPlanTypeItem=lPlanTypeList.get(THAMotorBikeQuotationResultType.DEFAULT_PLAN_TYPE.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PlanType','')
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_PLAN_TYPE_VALUE=lPlanType
	...
	public Map inputPlanGarageA(String strPlanGarage){
		...
		if(!lPlanGarageItem){
			lPlanGarageItem=lPlanGarageList.get(THAMotorBikeQuotationResultType.DEFAULT_PLAN_GARAGE.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PlanGarage','')
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_WORKSHOP_VALUE=lPlanGarage
	...
	public Map inputPlanExcessA(String strPlanExcess){
		...
		if(!lPlanExcessItem){
			lPlanExcessItem=lPlanExcessList.get(THAMotorBikeQuotationResultType.DEFAULT_PLAN_EXCESS.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PlanExcess','')
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_EXCESS_PRICE_VALUE=lPlanExcess
	...
	public Map inputPlanDriverA(String strPlanDriver){
		...
		if(!lPlanDriverItem){
			lPlanDriverItem=lPlanDriverList.get(THAMotorBikeQuotationResultType.DEFAULT_PLAN_DRIVER.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PlanDriver','')
			...
			if(lValid){
				...
				if(lButtonOpen){
					...
					Boolean lIsButtonOpen=IGNUemaHelper.convertStringToBoolean(lButtonStage)
					if(lIsButtonOpen){
						if(IGNUemaHelper.clickByWebElementWithScroll(this.driver,lButtonOpen)){
							IGNUemaHelper.delayThreadSecond(1)
						}
					}
				}
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_DRIVER_PLAN_VALUE=lPlanDriver
	...
	public Map inputCompulsoryA(String strCompulsory){
		...
		if(lCompulsory.length()<=0){
			lCompulsory=THAMotorBikeQuotationResultType.DEFAULT_COMPULSORY
		}
		try{
			Boolean lResult=false
			lreturn.put('Compulsory','')
			...
			if(!lElementCheckbox){
				...
				if(lCheckboxStageStr.equalsIgnoreCase('NOCOMP')){
					if(lIsCompulsory){
						...
						String lCompulsoryPrice=''
						String lTagCompulsoryPrice=''
						String lLocatorCompulsoryPrice='#section-7 strong'
						lCompulsoryPrice=IGNUemaHelper.getChildTextByLocator(driver,lLocatorCompulsoryPrice)
						THAMotorBikeCoreType.CURRENT_QUOTATION_PREMIUM_COMPULSORY_PRICE_VALUE=lCompulsoryPrice
					}
				}else{
					if(!lIsCompulsory){
						IGNUemaHelper.webJsClickWebElementFinalWithScroll(this.driver,lElementCheckbox)
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_COMPULSORY_VALUE=lCompulsory
	...
	public Map inputWithRsaA(String strWithRsa){
		...
		if(lWithRsa.length()<=0){
			lWithRsa=THAMotorBikeQuotationResultType.DEFAULT_WITH_RSA
		}
		try{
			Boolean lResult=false
			lreturn.put('WithRsa','')
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_RSA_VALUE=lWithRsa
	...
	public Map inputVoluntaryTPBIA(String strVoluntaryTPBI){
		...
		if(!lVoluntaryTPBIItem){
			lVoluntaryTPBIItem=lVoluntaryTPBIList.get(THAMotorBikeQuotationResultType.DEFAULT_VOLUNTARY_TPBI.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('VoluntaryTPBI','')
			...
			String lLocatorValue='#section-5'
			WebElement lElementValue=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorValue)
			if(!lElementValue){
				...
				if(lElementButtonTarget){
					...
					String lTagVoluntary=''
					String lLocatorVoluntary='#section-5 strong'
					List<WebElement> lElementVoluntaryList=IGNUemaHelper.getWebElementListFromCssLocator(this.driver,lLocatorVoluntary)
					String lVoluntaryTPPD=''
					Integer lIndexVoluntaryItem=0
					for(WebElement lElementVoluntaryItem in lElementVoluntaryList){
						String lStrItemText=lElementVoluntaryItem.getText()
						if(lIndexVoluntaryItem==0){
							lVoluntaryTPBI=this.checkAmountTotal(lStrItemText)
							THAMotorBikeCoreType.CURRENT_QUOTATION_PREMIUM_VOLUNTARY_TPBI_VALUE=lVoluntaryTPBI
						}
						if(lIndexVoluntaryItem==1){
							lVoluntaryTPPD=this.checkAmountTotal(lStrItemText)
							THAMotorBikeCoreType.CURRENT_QUOTATION_PREMIUM_VOLUNTARY_TPPD_VALUE=lVoluntaryTPPD
						}
						lIndexVoluntaryItem=lIndexVoluntaryItem+1
					}
				}
			}
	...
	public Map inputPersonalAccidentPaAndMeA(String strPersonalAccidentPaAndMe){
		...
		if(!lPersonalAccidentPaAndMeItem){
			lPersonalAccidentPaAndMeItem=lPersonalAccidentPaAndMeList.get(THAMotorBikeQuotationResultType.DEFAULT_PERSONAL_ACCIDENT_PA_AND_ME.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PersonalAccidentPaAndMe','')
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_PA_AND_ME_VALUE=lPersonalAccidentPaAndMe
	...
	public Map inputPersonalAccidentPaOnlyA(String strPersonalAccidentPaOnly){
		...
		if(!lPersonalAccidentPaOnlyItem){
			lPersonalAccidentPaOnlyItem=lPersonalAccidentPaOnlyList.get(THAMotorBikeQuotationResultType.DEFAULT_PERSONAL_ACCIDENT_PA_ONLY.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PersonalAccidentPaOnly','')
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_PA_ONLY_VALUE=lPersonalAccidentPaOnly
	...
	public Map inputPersonalAccidentMeOnlyA(String strPersonalAccidentMeOnly){
		...
		if(!lPersonalAccidentMeOnlyItem){
			lPersonalAccidentMeOnlyItem=lPersonalAccidentMeOnlyList.get(THAMotorBikeQuotationResultType.DEFAULT_PERSONAL_ACCIDENT_ME_ONLY.toUpperCase())
		}
		try{
			Boolean lResult=false
			lreturn.put('PersonalAccidentMeOnly','')
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_ME_ONLY_VALUE=lPersonalAccidentMeOnly
	...
	public Map inputCarRentalServiceA(String strCarRentalService){
		...
		if(lCarRentalService.length()<=0){
			lCarRentalService=THAMotorBikeQuotationResultType.DEFAULT_CAR_RENTAL_SERVICE
		}
		try{
			Boolean lResult=false
			lreturn.put('CarRentalService','')
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_CAR_REPLACEMENT_SERVICE_VALUE=lCarRentalService
	...
	public Map inputAccessoryAddA(String strAccessoryAdd,String strAccessoryCarryBoyAdd,String strAccessoryCarryBoyOption,String strAccessoryGasFuelingSystemAdd,String strAccessoryGasFuelingSystemOption,String strAccessoryBodyKitAdd,String strAccessoryBodyKitOption,String strAccessoryRimsTiresAdd,String strAccessoryRimsTiresOption,String strAccessorySolarFilmAdd,String strAccessorySolarFilmOption,String strAccessorySuspensionStabilizerSystemAdd,String strAccessorySuspensionStabilizerSystemOption,String strAccessoryInVehicleEntertainmentSystemAdd,String strAccessoryInVehicleEntertainmentSystemOption,String strAccessoryAirIntakeExhaustSystemAdd,String strAccessoryAirIntakeExhaustSystemOption,String strAccessoryCarCameraAdd,String strAccessoryCarCameraOption){
		...
		if(lAccessoryAdd.length()<=0){
			lAccessoryAdd=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_ADD
		}
		String lAccessoryCarryBoyAdd=strAccessoryCarryBoyAdd.trim()
		if(lAccessoryCarryBoyAdd.length()<=0){
			lAccessoryCarryBoyAdd=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_CARRY_BOY_ADD
		}
		String lAccessoryCarryBoyOption=strAccessoryCarryBoyOption.trim()
		if(lAccessoryCarryBoyOption.length()<=0){
			lAccessoryCarryBoyOption=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_CARRY_BOY_OPTION
		}
		String lAccessoryGasFuelingSystemAdd=strAccessoryGasFuelingSystemAdd.trim()
		if(lAccessoryGasFuelingSystemAdd.length()<=0){
			lAccessoryGasFuelingSystemAdd=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_GAS_FUELING_SYSTEM_ADD
		}
		...
		if(lAccessoryGasFuelingSystemOption.length()<=0){
			lAccessoryGasFuelingSystemOption=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_GAS_FUELING_SYSTEM_OPTION
		}
		String lAccessoryBodyKitAdd=strAccessoryBodyKitAdd.trim()
		if(lAccessoryBodyKitAdd.length()<=0){
			lAccessoryBodyKitAdd=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_BODY_KIT_ADD
		}
		String lAccessoryBodyKitOption=strAccessoryBodyKitOption.trim()
		if(lAccessoryBodyKitOption.length()<=0){
			lAccessoryBodyKitOption=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_BODY_KIT_OPTION
		}
		String lAccessoryRimsTiresAdd=strAccessoryRimsTiresAdd.trim()
		if(lAccessoryRimsTiresAdd.length()<=0){
			lAccessoryRimsTiresAdd=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_RIMS_TIRES_ADD
		}
		String lAccessoryRimsTiresOption=strAccessoryRimsTiresOption.trim()
		if(lAccessoryRimsTiresOption.length()<=0){
			lAccessoryRimsTiresOption=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_RIMS_TIRES_OPTION
		}
		String lAccessorySolarFilmAdd=strAccessorySolarFilmAdd.trim()
		if(lAccessorySolarFilmAdd.length()<=0){
			lAccessorySolarFilmAdd=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_SOLAR_FILM_ADD
		}
		String lAccessorySolarFilmOption=strAccessorySolarFilmOption.trim()
		if(lAccessorySolarFilmOption.length()<=0){
			lAccessorySolarFilmOption=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_SOLAR_FILM_OPTION
		}
		...
		if(lAccessorySuspensionStabilizerSystemAdd.length()<=0){
			lAccessorySuspensionStabilizerSystemAdd=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_SUSPENSION_STABILIZER_SYSTEM_ADD
		}
		...
		if(lAccessorySuspensionStabilizerSystemOption.length()<=0){
			lAccessorySuspensionStabilizerSystemOption=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_SUSPENSION_STABILIZER_SYSTEM_OPTION
		}
		...
		if(lAccessoryInVehicleEntertainmentSystemAdd.length()<=0){
			lAccessoryInVehicleEntertainmentSystemAdd=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_IN_VEHICLE_ENTERTAINMENT_SYSTEM_ADD
		}
		...
		if(lAccessoryInVehicleEntertainmentSystemOption.length()<=0){
			lAccessoryInVehicleEntertainmentSystemOption=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_IN_VEHICLE_ENTERTAINMENT_SYSTEM_OPTION
		}
		...
		if(lAccessoryAirIntakeExhaustSystemAdd.length()<=0){
			lAccessoryAirIntakeExhaustSystemAdd=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_AIR_INTAKE_EXHAUST_SYSTEM_ADD
		}
		...
		if(lAccessoryAirIntakeExhaustSystemOption.length()<=0){
			lAccessoryAirIntakeExhaustSystemOption=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_AIR_INTAKE_EXHAUST_SYSTEM_OPTION
		}
		String lAccessoryCarCameraAdd=strAccessoryCarCameraAdd.trim()
		if(lAccessoryCarCameraAdd.length()<=0){
			lAccessoryCarCameraAdd=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_CAR_CAMERA_ADD
		}
		String lAccessoryCarCameraOption=strAccessoryCarCameraOption.trim()
		if(lAccessoryCarCameraOption.length()<=0){
			lAccessoryCarCameraOption=THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_CAR_CAMERA_OPTION
		}
		String lAccessoryAmount=''
		String lAccessoryDetail=''
		...
		try{
			...
			if(!lElementAccessoryAddButtonOpen){
				...
				if(!lAccessoryCarryBoyOptionItem){
					lAccessoryCarryBoyOptionItem=lAccessoryItemOptionList.get(THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_CARRY_BOY_OPTION)
				}
				...
				if(!lAccessoryGasFuelingSystemOptionItem){
					lAccessoryGasFuelingSystemOptionItem=lAccessoryItemOptionList.get(THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_GAS_FUELING_SYSTEM_OPTION)
				}
				String lAccessoryBodyKitOptionItem=lAccessoryItemOptionList.get(lAccessoryBodyKitOption)
				if(!lAccessoryBodyKitOptionItem){
					lAccessoryBodyKitOptionItem=lAccessoryItemOptionList.get(THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_BODY_KIT_OPTION)
				}
				...
				if(!lAccessoryRimsTiresOptionItem){
					lAccessoryRimsTiresOptionItem=lAccessoryItemOptionList.get(THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_RIMS_TIRES_OPTION)
				}
				...
				if(!lAccessorySolarFilmOptionItem){
					lAccessorySolarFilmOptionItem=lAccessoryItemOptionList.get(THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_SOLAR_FILM_OPTION)
				}
				...
				if(!lAccessorySuspensionStabilizerSystemOptionItem){
					lAccessorySuspensionStabilizerSystemOptionItem=lAccessoryItemOptionList.get(THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_SUSPENSION_STABILIZER_SYSTEM_OPTION)
				}
				...
				if(!lAccessoryInVehicleEntertainmentSystemOptionItem){
					lAccessoryInVehicleEntertainmentSystemOptionItem=lAccessoryItemOptionList.get(THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_IN_VEHICLE_ENTERTAINMENT_SYSTEM_OPTION)
				}
				...
				if(!lAccessoryAirIntakeExhaustSystemOptionItem){
					lAccessoryAirIntakeExhaustSystemOptionItem=lAccessoryItemOptionList.get(THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_AIR_INTAKE_EXHAUST_SYSTEM_OPTION)
				}
				...
				if(!lAccessoryCarCameraOptionItem){
					lAccessoryCarCameraOptionItem=lAccessoryItemOptionList.get(THAMotorBikeQuotationResultType.DEFAULT_ACCESSORY_CAR_CAMERA_OPTION)
				}
	...
	public Map inputQuoteMeA(String strQuoteBy,String strQuotePhoneNo,String strQuoteEmail,String strQuoteNewsletter,String strQuoteCallMeNow,String strQuotePDFDownloadToDo){
		...
		if(lQuotePhoneNo.length()<=0){
			lQuotePhoneNo=THAMotorBikeQuotationResultType.DEFAULT_QUOTE_PHONE_NO
		}
		String lQuoteEmail=strQuoteEmail.trim()
		if(lQuoteEmail.length()<=0){
			lQuoteEmail=THAMotorBikeQuotationResultType.DEFAULT_QUOTE_EMAIL
		}
		String lQuoteNewsletter=strQuoteNewsletter.trim()
		if(lQuoteNewsletter.length()<=0){
			lQuoteNewsletter=THAMotorBikeQuotationResultType.DEFAULT_QUOTE_NEWSLETTER
		}
		String lQuoteCallMeNow=strQuoteCallMeNow.trim()
		if(lQuoteCallMeNow.length()<=0){
			lQuoteCallMeNow=THAMotorBikeQuotationResultType.DEFAULT_QUOTE_CALL_ME_NOW
		}
		String lQuotePDFDownloadToDo=strQuotePDFDownloadToDo.trim()
		if(lQuotePDFDownloadToDo.length()<=0){
			lQuotePDFDownloadToDo=THAMotorBikeQuotationResultType.DEFAULT_QUOTE_PDF_DOWNLOAD_TO_DO
		}
		String lQuotePDFDownloadDetail=''
		String lQuotePDFDownloadResult=''
		try{
			...
			switch(lQuoteBy){
				...
				case 'EMAIL':
					...
					if(IGNUemaHelper.selectButtonByLocator(this.driver,lLocatorButtonEmailMe)){
						IGNUemaHelper.delayThreadSecond(1)
						String lTagTextEmailMe='savequote-v4d-email'
						String lLocatorTextEmailMe=IGNUemaHelper.getTagDataSeleniumKey(lTagTextEmailMe)
						Boolean lIsTextEmailMeDisplayed=IGNUemaHelper.checkElementVisibleWithBlockByLocatorJs(this.driver,lLocatorTextEmailMe)
						if(!lIsTextEmailMeDisplayed){
							...
							if(!lIsQuoteCallMeNow){
								lTagButtonEmailMePhoneNext='savequote-mobile-no-submit'
							}
							if(lIsQuoteCallMeNow){
								lTagButtonEmailMePhoneNext='savequote-mobile-yes-submit'
							}
							lLocatorButtonEmailMePhoneNext=IGNUemaHelper.getTagDataSeleniumKey(lTagButtonEmailMePhoneNext)
						...
						String lTextQuoteEmail=IGNUemaHelper.typeTextByLocator(this.driver,lLocatorTextEmailMe,lQuoteEmail)
						if(lTextQuoteEmail.length()<=0){
							lTextQuoteEmail=IGNUemaHelper.typeTextByLocator(this.driver,'#savequote-v4d-email',lQuoteEmail)
						}
						lQuoteEmail=lTextQuoteEmail
						String lTagCheckboxEmailMeNewsletter='mb-savequote-v4d-userAcceptConsent'
						...
						if(lElementQuotePDFDownload){
							...
							if(lIsQuotePDFDownloadToDo){
								...
								IGNUemaHelper.delayThreadSecond(3)
								String lTagFormLoading=''
								String lLocatorFormLoading='#isPDFgenerating'
						...
						String lTagButtonEmailMeNext='savequote-v4d-email-submit'
						String lLocatorButtonEmailMeNext=IGNUemaHelper.getTagDataSeleniumKey(lTagButtonEmailMeNext)
						if(IGNUemaHelper.selectButtonByLocator(this.driver,lLocatorButtonEmailMeNext)){
							IGNUemaHelper.delayThreadSecond(1)
						...
						if(IGNUemaHelper.selectButtonByLocator(this.driver,'#step2email2 #pk2save')){
							lResult=true
							IGNUemaHelper.delayThreadSecond(3)
							IGNUemaHelper.webJsScrollToTop(this.driver)
						}
				...
				case 'PDF':
					...
					if(IGNUemaHelper.selectButtonByLocator(this.driver,lLocatorButtonEmailMe)){
						...
						String lLocatorTextEmailMeEmail='#savequote-v4d-email'
						...
						String lLocatorEmailModal='#savepdf-modal-v4'
						...
						WebElement lElementButtonEmailMeGoBack=IGNUemaHelper.getChildWebElementFromDataSeleniumKey(this.driver,lElementEmailModal,lTagButtonEmailMeGoBack)
					...
					break
				default:
					lQuoteBy=THAMotorBikeQuotationResultType.DEFAULT_QUOTE_BY
					lQuotePhoneNo=''
					lQuoteEmail=''
					lQuoteNewsletter=''
					lQuoteCallMeNow=''
			...
			if(THAMotorBikeCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER.toUpperCase().contains('MB')||THAMotorBikeCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER.toUpperCase().contains('TBA')){
	...
	public Map inputSalesforceProceedTodo(String strSalesforceProceedTodo){
		...
		if(lSalesforceProceedTodo.length()<=0){
			lSalesforceProceedTodo=THAMotorBikeQuotationResultType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
			...
			THAMotorBikeCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER=this.checkOpportunityNumber(THAMotorBikeCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER)
	...
	public Map inputSalesforceProceedVerifyBefore(Map caseOutput){
		...
		try{
			...
			String lAutomationPlanType=caseOutput.PlanType
			...
			if(!lPlanTypeItem){
				...
				lPlanTypeList=['Type2':'Type2','Type3':'Type3']
				lPlanTypeItem=lPlanTypeList.get(lPlanTypeItem)
				if(!lPlanTypeItem){
					String lAutomationWorkshop=caseOutput.PlanGarage
					...
					String lAutomationSumInsured=THAMotorBikeCoreType.CURRENT_QUOTATION_PREMIUM_SUM_INSURED_VALUE
					String lSalesforceSumInsured=caseOutput.SalesforceVerifyDataTransactionSyncedObjVehicleSumInsuredC
					if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationSumInsured,lSalesforceSumInsured)){
						String lSumInsured=this.checkAmountTotal(lAutomationSumInsured)
						Float lAutomationSumInsuredNumber=IGNUemaHelper.convertStringToFloat(lSumInsured,0)
			...
			String lAutomationDriverPlan=caseOutput.PlanDriver
			...
			String lAutomationCompulsoryPlan=caseOutput.Compulsory
			...
			if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationCompulsoryPlan,lSalesforceCompulsoryPlan)){
				String lCompulsoryPlan=lSalesforceCompulsoryPlan.toUpperCase().trim()
				Map lCompulsoryPlanList=['YES':'Comp','NO':'NoComp']
				String lCompulsoryPlanItem=lCompulsoryPlanList.get(lCompulsoryPlan)
				if(!lCompulsoryPlanItem){
					lIsOK=false
					lText=IGNUemaHelper.concatMessageLeftRightByPipe('CompulsoryPlan='+lAutomationCompulsoryPlan,lSalesforceCompulsoryPlan)
					lSalesforceVerifyCompareDetail=lSalesforceVerifyCompareDetail+lText+'\n'
				}else{
					if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationCompulsoryPlan,lCompulsoryPlanItem)){
						lIsOK=false
						lText=IGNUemaHelper.concatMessageLeftRightByPipe('CompulsoryPlan='+lAutomationCompulsoryPlan,lSalesforceCompulsoryPlan)
						lSalesforceVerifyCompareDetail=lSalesforceVerifyCompareDetail+lText+'\n'
					}
			...
			String lAutomationRsaPlan=caseOutput.WithRsa
			...
			String lAutomationCarReplacementServicePlan=caseOutput.CarRentalService
			...
			if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationCarReplacementServicePlan,lSalesforceCarReplacementServicePlan)){
				...
				Map lCarReplacementServicePlanList=['GOLD':'Yes','':'No Option']
			...
			String lAutomationExcess=caseOutput.PlanExcess
			...
			if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationExcess,lSalesforceExcess)){
				...
				Float lSalesforceExcessPriceNumber=IGNUemaHelper.convertStringToFloat(lSalesforceExcess,0)
				Float lExcessPrice=Math.abs(lAutomationExcessNumber-lSalesforceExcessPriceNumber)
				if(lExcessPrice>1){
					lIsOK=false
			...
			String lAutomationYearly=caseOutput.PremiumYearly
			...
			if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationYearly,lSalesforceYearly)){
				String lYearly=this.checkAmountTotal(lAutomationYearly)
				Float lAutomationYearlyNumber=IGNUemaHelper.convertStringToFloat(lYearly,0)
			...
			String lAutomationPremiumNoDepositAmount=caseOutput.PremiumNoDepositAmount
			...
			String lAutomationPremiumNoDepositTotal=caseOutput.PremiumNoDepositTotal
			...
			if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationPremiumNoDepositTotal,lSalesforcePremiumNoDepositTotal)){
				String lPremiumNoDepositTotal=this.checkAmountTotal(lAutomationPremiumNoDepositTotal)
				Float lAutomationPremiumNoDepositTotalNumber=IGNUemaHelper.convertStringToFloat(lPremiumNoDepositTotal,0)
				...
				if(lPremiumNoDepositTotalPrice>5){
					lIsOK=false
			...
			for(Integer lIndex=0;lIndex<=5;lIndex++){
				...
				if(IGNUemaHelper.checkStringContainString(lQuoteLineItem,'COMP')){
					String lAutomationCompulsoryPrice=THAMotorBikeCoreType.CURRENT_QUOTATION_PREMIUM_COMPULSORY_PRICE_VALUE
					...
					if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationCompulsoryPrice,lSalesforceCompulsoryPrice)){
						String lCompulsory=this.checkAmountTotal(lAutomationCompulsoryPrice)
						Float lAutomationCompulsoryPriceNumber=IGNUemaHelper.convertStringToFloat(lCompulsory,0)
				...
				if(IGNUemaHelper.checkStringContainString(lQuoteLineItem,'VOL')){
					String lAutomationTPBI=THAMotorBikeCoreType.CURRENT_QUOTATION_PREMIUM_VOLUNTARY_TPBI_VALUE
					...
					String lAutomationTPPD=THAMotorBikeCoreType.CURRENT_QUOTATION_PREMIUM_VOLUNTARY_TPPD_VALUE
					...
					String lAutomationPaAndMe=caseOutput.PersonalAccidentPaAndMe
					...
					if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationPaAndMe,lSalesforcePaAndMe)){
						if(lSalesforcePaAndMe.length()<=0){
							String lAutomationPaOnly=caseOutput.PersonalAccidentPaOnly
							...
							String lAutomationMeOnly=caseOutput.PersonalAccidentMeOnly
				...
				if(IGNUemaHelper.checkStringContainString(lQuoteLineItem,'RSA')){
					String lAutomationRsaPrice=caseOutput.AutomationRsaText
					String lSalesforceRsaPrice=caseOutput.('SalesforceVerifyDataQuoteLineItem'+lStrRoundPad+'DetailTransGrossPremiumC')
					if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationRsaPrice,lSalesforceRsaPrice)){
						Float lAutomationRsaNumber=IGNUemaHelper.convertStringToFloat(lAutomationRsaPrice,0)
						Float lSalesforceRsaNumber=IGNUemaHelper.convertStringToFloat(lSalesforceRsaPrice,0)
						Float lRsaPrice=Math.abs(lAutomationRsaNumber-lSalesforceRsaNumber)
						if(lRsaPrice>1){
							lIsOK=false
							lText=IGNUemaHelper.concatMessageLeftRightByPipe('RSA Price='+lAutomationRsaPrice,lSalesforceRsaPrice)
							lSalesforceVerifyCompareDetail=lSalesforceVerifyCompareDetail+lText+'\n'
						}
					}
				}
				if(IGNUemaHelper.checkStringContainString(lQuoteLineItem,'CRS')){
					String lAutomationCarReplacementServicePrice=caseOutput.AutomationCarReplacementServiceText
					String lSalesforceCarReplacementServicePrice=caseOutput.('SalesforceVerifyDataQuoteLineItem'+lStrRoundPad+'DetailTransGrossPremiumC')
					if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationCarReplacementServicePrice,lSalesforceCarReplacementServicePrice)){
						Float lAutomationCarReplacementServiceNumber=IGNUemaHelper.convertStringToFloat(lAutomationCarReplacementServicePrice,0)
						Float lSalesforceCarReplacementServiceNumber=IGNUemaHelper.convertStringToFloat(lSalesforceCarReplacementServicePrice,0)
						...
						if(lCarReplacementServicePrice>1){
							lIsOK=false
							lText=IGNUemaHelper.concatMessageLeftRightByPipe('Car Replacement Service Price='+lAutomationCarReplacementServicePrice,lSalesforceCarReplacementServicePrice)
							lSalesforceVerifyCompareDetail=lSalesforceVerifyCompareDetail+lText+'\n'
						}
					}
				}
			}
			String lAutomationReward=THAMotorBikeCoreType.CURRENT_QUOTATION_AUTOMATION_ROOJAI_REWARD_POINT
			...
			if(!IGNUemaHelper.checkStringContainString(lAutomationReward,lSalesforceReward)){
				String lRewardNumber=this.checkAmountTotal(lAutomationReward)
	...
	public Map inputSalesforceProceedRetrieveTodo(String strSalesforceProceedRetrieveTodo,String strAccountNameAnonymous){
		...
		if(lSalesforceProceedRetrieveTodo.length()<=0){
			lSalesforceProceedRetrieveTodo=THAMotorBikeQuotationResultType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
		...
		try{
			...
			try{
				...
				if(lRedirected){
					...
					lRedirected=lCurrentURL.toUpperCase().contains('MOTORBIKE-PACKAGES')
				}
				lResult=lRedirected
			}catch(Exception ex){
				return lreturn
	...
	public Boolean inputExpectedKumkaCompareButton(Map targetMap){
		...
		if(!lExpectedKumkaCompareButtonItem){
			lExpectedKumkaCompareButtonItem=lExpectedKumkaCompareButtonList.get(THAMotorBikeQuotationResultType.DEFAULT_EXPECTED_KUMKA_COMPARE_BUTTON.toUpperCase())
		...
		if(!lKumkaCompareToDoItem){
			lKumkaCompareToDoItem=lKumkaCompareToDoList.get(THAMotorBikeQuotationResultType.DEFAULT_KUMKA_COMPARE_TO_DO.toUpperCase())
	...
	public Boolean inputKumkaCompareNow(String strLanguage,String strEmailToSave,String strCallMeBack,String strCallMeBackPhoneNo,Map targetMap){
		...
		if(lKumkaCompareToDo.length()<=0){
			lKumkaCompareToDo=THAMotorBikeQuotationResultType.DEFAULT_KUMKA_COMPARE_TO_DO
		}
		String lKumkaLoginPage=THAMotorBikeQuotationResultType.DEFAULT_KUMKA_LOGIN_URL
		String lLoginUser=THAMotorBikeQuotationResultType.DEFAULT_KUMKA_LOGIN_USER
		String lLoginPassword=THAMotorBikeQuotationResultType.DEFAULT_KUMKA_LOGIN_PASSWORD
		...
		try{
			...
			if(lElementButton){
				...
				if(lCurrentURLText.length()>0){
					THAMotorBikeCoreType.CURRENT_URL_UNDER_PAGE_RESULT_NEW_KUMKA=IGNUemaHelper.checkStringContainString(lCurrentURLText,'/CARDSNEW')
				}
				if(THAMotorBikeCoreType.CURRENT_URL_UNDER_PAGE_RESULT_NEW_KUMKA){
	...
	public Boolean checkAnonymousAccountToGetActualOpportunityNumberByRedirecting(){
		...
		try{
			Boolean lRedirected=false
			lRedirected=IGNUemaHelper.checkElementPresentByElementId(this.driver,'policy1-v3',30)
			if(!lRedirected){
