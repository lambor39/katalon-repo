...
public class THAMotorBikeQuotationSocialUtilOld{
	...
	public THAMotorBikeQuotationSocialUtilOld(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	public Boolean inputSocial(Map caseInput,Map caseOutput){
		...
		try{
			...
			if(!lIsPaymentPromptPayPageVisible){
				...
				String lPaymentMethod=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_METHOD
				String lQrSuccessCase=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_QR_CODE_SUCCESS_CASE
				String lTrueMoneyMobileNo=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_MOBILE_NO
				String lTrueMoneyEmail=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_EMAIL
				String lTrueMoneySuccessCase=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_SUCCESS_CASE
				String lRabbitLinePayMobileNo=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_MOBILE_NO
				String lRabbitLinePayEmail=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_EMAIL
				String lRabbitLinePaySuccessCase=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_SUCCESS_CASE
				String lMobileBankingCard=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_CARD
				String lMobileBankingSuccessCase=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_SUCCESS_CASE
				String lOpportunityNumber=''
				...
				if(lIsSalesforceProceedTodo){
					...
					if(lIsActualResult){
						...
						if(lIsSalesforceProceedRetrieveTodo){
							Map lMapSalesforceProceedRetrieveWebFromUrl=this.inputSalesforceProceedRetrieveWebFromUrl(THAMotorBikeCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_SOCIAL)
	...
	public Map inputLanguage(String strLanguage){
		...
		if(lLanguage.length()<=0){
			lLanguage=THAMotorBikeQuotationSocialType.DEFAULT_LANGUAGE
		}
		...
		try{
			...
			if(!lLanguageItem){
				lLanguageItem=lLanguageList.get(THAMotorBikeQuotationSocialType.DEFAULT_LANGUAGE)
			}
	...
	public Map inputFeefoRating(String strPolicyInsurer,String strRatingForOverall,String strRatingForWebsite,String strRatingForCallCenter,String strRatingForProduct,String strRatingText,String strWithFacebook,String strFacebookLoginId,String strFacebookLoginPassword,String strShareFacebook){
		...
		if(lRatingForOverall.length()<=0){
			lRatingForOverall=THAMotorBikeQuotationSocialType.DEFAULT_RATING_FOR_OVERALL
		}
		String lRatingForWebsite=strRatingForWebsite.trim()
		if(lRatingForWebsite.length()<=0){
			lRatingForWebsite=THAMotorBikeQuotationSocialType.DEFAULT_RATING_FOR_WEBSITE
		}
		String lRatingForCallCenter=strRatingForCallCenter.trim()
		if(lRatingForCallCenter.length()<=0){
			lRatingForCallCenter=THAMotorBikeQuotationSocialType.DEFAULT_RATING_FOR_CALL_CENTER
		}
		String lRatingForProduct=strRatingForProduct.trim()
		if(lRatingForProduct.length()<=0){
			lRatingForProduct=THAMotorBikeQuotationSocialType.DEFAULT_RATING_FOR_PRODUCT
		}
		String lRatingText=strRatingText.trim()
		if(lRatingText.length()<=0){
			lRatingText=THAMotorBikeQuotationSocialType.DEFAULT_RATING_TEXT
		}
		String lWithFacebook=strWithFacebook.trim()
		if(lWithFacebook.length()<=0){
			lWithFacebook=THAMotorBikeQuotationSocialType.DEFAULT_WITH_FACEBOOK
		}
		String lFacebookLoginId=strFacebookLoginId.trim()
		if(lFacebookLoginId.length()<=0){
			lFacebookLoginId=THAMotorBikeQuotationSocialType.DEFAULT_FACEBOOK_LOGIN_ID
		}
		String lFacebookLoginPassword=strFacebookLoginPassword.trim()
		if(lFacebookLoginPassword.length()<=0){
			lFacebookLoginPassword=THAMotorBikeQuotationSocialType.DEFAULT_FACEBOOK_LOGIN_PASSWORD
		}
		String lShareFacebook=strShareFacebook.trim()
		if(lShareFacebook.length()<=0){
			lShareFacebook=THAMotorBikeQuotationSocialType.DEFAULT_SHARE_FACEBOOK
		}
					...
					if(lIsFoundFeefoWindow){
						...
						if(IGNUemaHelper.waitElementVisibleByLocator(this.driver,lLocatorRatingForOverall,30)){
							...
							if(lElementRatingForOverall){
								...
								if(THAMotorBikeQuotationDetail2Type.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail2Output)){
	...
	public Map inputPaymentByQR(String strQrSuccessCase){
		...
		if(lQrSuccessCase.length()<=0){
			lQrSuccessCase=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_QR_CODE_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public Map inputPaymentByTrueMoney(String strTrueMoneyMobileNo,String strTrueMoneyEmail,String strTrueMoneySuccessCase){
		...
		if(lTrueMoneyMobileNo.length()<=0){
			lTrueMoneyMobileNo=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_MOBILE_NO
		}
		String lTrueMoneyEmail=strTrueMoneyEmail.trim()
		if(lTrueMoneyEmail.length()<=0){
			lTrueMoneyEmail=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_EMAIL
		}
		String lTrueMoneySuccessCase=strTrueMoneySuccessCase.trim()
		if(lTrueMoneySuccessCase.length()<=0){
			lTrueMoneySuccessCase=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public Map inputPaymentByRabbitLinePay(String strRabbitLinePayMobileNo,String strRabbitLinePayEmail,String strRabbitLinePaySuccessCase){
		...
		if(lRabbitLinePayMobileNo.length()<=0){
			lRabbitLinePayMobileNo=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_MOBILE_NO
		}
		String lRabbitLinePayEmail=strRabbitLinePayEmail.trim()
		if(lRabbitLinePayEmail.length()<=0){
			lRabbitLinePayEmail=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_EMAIL
		}
		String lRabbitLinePaySuccessCase=strRabbitLinePaySuccessCase.trim()
		if(lRabbitLinePaySuccessCase.length()<=0){
			lRabbitLinePaySuccessCase=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public Map inputPaymentByMobileBanking(String strMobileBankingCard,String strMobileBankingSuccessCase){
		...
		if(lMobileBankingCard.length()<=0){
			lMobileBankingCard=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_CARD
		}
		String lMobileBankingSuccessCase=strMobileBankingSuccessCase.trim()
		if(lMobileBankingSuccessCase.length()<=0){
			lMobileBankingSuccessCase=THAMotorBikeCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public String inputPositiveCase(String strPositiveCase){
		...
		try{
			String lText=''
			if(lPositiveCase.length()<=0){
				lPositiveCase=THAMotorBikeQuotationSocialType.DEFAULT_POSITIVE_CASE
			}
			Boolean lIsPositiveCase=IGNUemaHelper.convertStringToBoolean(lPositiveCase)
	...
	public Map inputSalesforceProceedTodo(String strSalesforceProceedTodo){
		...
		if(lSalesforceProceedTodo.length()<=0){
			lSalesforceProceedTodo=THAMotorBikeQuotationSocialType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
	...
	public Map inputSalesforceProceedRetrieveTodo(String strSalesforceProceedRetrieveTodo,String strAccountNameAnonymous){
		...
		if(lSalesforceProceedRetrieveTodo.length()<=0){
			lSalesforceProceedRetrieveTodo=THAMotorBikeQuotationSocialType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
	...
	public Map inputSalesforceProceedRetrieveWebFromUrl(String strRetrievePageURL){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result',lResult)
			String lRetrievePageURLOld=lRetrievePageURL
			lRetrievePageURL=''
			Map lMapQuery=IGNUemaHelper.extractURLStringToMapQuery(lRetrievePageURLOld,false,false)
			String lOpportunityNumberText=lMapQuery.get('opportunityNumber')
			if(!lOpportunityNumberText){
				return lreturn
			}else{
				lOpportunityNumberText=lOpportunityNumberText.trim()
				if(lOpportunityNumberText.length()<=0){
					return lreturn
				}
			}
			lRetrievePageURL='https://insure.uat-roojai.com/quote/#/?productType=MotorBike&pageName=review&opportunityNumber='+lOpportunityNumberText
			Boolean lRedirected=false
			String lMainWindow=this.driver.getWindowHandle()
			if(lMainWindow.length()<=0){
				lMainWindow=this.driver.getWindowHandle()
			...
			try{
				...
				if(lRedirected){
					String lCurrentURL=WebUI.getUrl()
					IGNUemaHelper.printLog(lCurrentURL)
					lRedirected=lCurrentURL.toUpperCase().contains('MOTORBIKE-REVIEW')
				}
				lResult=lRedirected
			}catch(Exception ex){
				return lreturn
