...
public class THAMotorCarQuotationDetail2UtilOld{
	private WebDriver driver=null
	private WebDriver selenium=null
	public THAMotorCarQuotationDetail2UtilOld(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	public Boolean inputDetail2(Map caseInput,Map caseOutput){
		...
		try{
			...
			if(THAMotorCarCoreType.CURRENT_SKIP_PAGE_DETAIL2){
				lActualResult=IGNUemaHelper.convertBooleanToString(lIsPositiveCase)
				caseOutput.ActualResult=lActualResult
				lreturn=lIsPositiveCase
				return lreturn
			}
	...
	public Map inputLanguage(String strLanguage){
		...
		try{
			...
			if(!lLanguageItem){
				lLanguageItem=lLanguageList.get(THAMotorCarQuotationDetail2Type.DEFAULT_LANGUAGE)
			}
		...
		try{
			...
			if(lElementInfo){
				...
				if(lInfoText.length()>0){
					THAMotorCarCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER=lInfoText
					lreturn=lInfoText
	...
	public Map inputSalesforceProceedVerifyBefore(Map caseOutput){
		...
		try{
			...
			switch(THAMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_METHOD){
				case '2':
					lAutomationYearly=THAMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_YEARLY_VALUE
					...
					break
				case '3':
					lAutomationPaymentFrequency=THAMotorCarCoreType.CURRENT_QUOTATION_PREMIUM_QR_CODE_IS_YEARLY
					...
					if(lIsPaymentFrequency){
						...
						lAutomationPremiumNoDepositAmount=THAMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_NO_DEPOSIT_AMOUNT_VALUE
						...
						lAutomationPremiumNoDepositTotal=THAMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_NO_DEPOSIT_TOTAL_VALUE
					...
					break
				case '4':
					lAutomationPaymentFrequency=THAMotorCarCoreType.CURRENT_QUOTATION_PREMIUM_TRUE_MONEY_IS_YEARLY
					lSalesforcePaymentFrequency=caseOutput.SalesforceVerifyDataOpportunityObjPaymentFrequencyC
					lIsPaymentFrequency=IGNUemaHelper.convertStringToBoolean(lAutomationPaymentFrequency)
					if(lIsPaymentFrequency){
						lAutomationYearly=THAMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_YEARLY_VALUE
					...
					break
				case '5':
					lAutomationPaymentFrequency=THAMotorCarCoreType.CURRENT_QUOTATION_PREMIUM_RABBIT_LINE_PAY_IS_YEARLY
					lSalesforcePaymentFrequency=caseOutput.SalesforceVerifyDataOpportunityObjPaymentFrequencyC
					lIsPaymentFrequency=IGNUemaHelper.convertStringToBoolean(lAutomationPaymentFrequency)
					if(lIsPaymentFrequency){
						lAutomationYearly=THAMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_YEARLY_VALUE
					...
					break
				default:
					lAutomationPaymentFrequency=THAMotorCarCoreType.CURRENT_QUOTATION_PREMIUM_CC_IS_YEARLY
					lSalesforcePaymentFrequency=caseOutput.SalesforceVerifyDataOpportunityObjPaymentFrequencyC
					lIsPaymentFrequency=IGNUemaHelper.convertStringToBoolean(lAutomationPaymentFrequency)
					if(lIsPaymentFrequency){
						lAutomationYearly=THAMotorCarCoreType.CURRENT_QUOTATION_AUTOMATION_PREMIUM_YEARLY_VALUE
