...
public class THAMotorCarQuotationVerifyFactory{
	public Boolean IsValid=false
	private lCaseList=[]
	public Integer CurrentCase=0
	public THAMotorCarQuotationVerifyFactory(TestData dataInput){
		super()
	...
	public Boolean validateDataInput(TestData dataInput){
		...
		try{
			...
			lreturn=(dataColNum>=THAMotorCarQuotationVerifyType.INPUT_DATA_COL_BEGIN)&&(dataRowNum>=THAMotorCarQuotationVerifyType.INPUT_DATA_ROW_END)
		}catch(Exception e){
			e.printStackTrace()
			return lreturn
		}
	...
	public void initCaseList(TestData dataInput){
		...
		try{
			...
			Integer dataColNum=dataInput.getColumnNumbers()
			Integer dataRowNum=dataInput.getRowNumbers()
			Integer dataColIndex=THAMotorCarQuotationVerifyType.INPUT_DATA_COL_BEGIN
			Integer dataRowIndex=THAMotorCarQuotationVerifyType.INPUT_DATA_ROW_BEGIN
			THAMotorCarQuotationVerifyType dataCase
			Map caseInput=[:]
			caseInput.clear()
			String cellVal=''
			for(dataColIndex=THAMotorCarQuotationVerifyType.INPUT_DATA_COL_BEGIN;dataColIndex<=dataColNum;dataColIndex++){
				dataCase=new THAMotorCarQuotationVerifyType()
				dataCase.initInput()
				dataCase.initOutput()
				caseInput.clear()
				...
				dataRowIndex=THAMotorCarQuotationVerifyType.INPUT_DATA_ROW_BEGIN
	...
	public THAMotorCarQuotationVerifyType CaseData(){
	...
	public Boolean SaveOutput(){
		Boolean lreturn=false
		THAMotorCarQuotationVerifyType caseData
		try{
			Integer dataColIndex=THAMotorCarQuotationVerifyType.OUTPUT_EXCEL_COL_BEGIN+(this.CurrentCase-1)
			Integer dataRowIndex=THAMotorCarQuotationVerifyType.OUTPUT_EXCEL_ROW_BEGIN
			if((this.CurrentCase>0)&&(this.lCaseList.size()>0)){
				caseData=this.lCaseList.get(this.CurrentCase-1)
				def xlsWorkbook=ExcelKeywords.getWorkbook(THAMotorCarQuotationVerifyType.OUTPUT_EXCEL_FILE_NAME)
				def xlsSheet=ExcelKeywords.getExcelSheet(xlsWorkbook,THAMotorCarQuotationVerifyType.OUTPUT_EXCEL_SHEET_NAME)
				...
				dataRowIndex+=1
				IGNUemaHelper.saveExcelWorkbookByFileName(THAMotorCarQuotationVerifyType.OUTPUT_EXCEL_FILE_NAME,xlsWorkbook)
				lreturn=true
			}
		}catch(Exception e){
			e.printStackTrace()
