...
public class THACriticalIllnessQuotationInputFactory{
	public Boolean IsValid=false
	private lCaseList=[]
	public Integer CurrentCase=0
	public THACriticalIllnessQuotationInputFactory(TestData dataInput){
		super()
	...
	public Boolean validateDataInput(TestData dataInput){
		...
		try{
			...
			lreturn=(dataColNum>=THACriticalIllnessQuotationInputType.INPUT_DATA_COL_BEGIN)&&(dataRowNum>=THACriticalIllnessQuotationInputType.INPUT_DATA_ROW_END)
		}catch(Exception e){
			e.printStackTrace()
			return lreturn
		}
	...
	public void initCaseList(TestData dataInput){
		...
		try{
			...
			Integer dataColNum=dataInput.getColumnNumbers()
			Integer dataRowNum=dataInput.getRowNumbers()
			Integer dataColIndex=THACriticalIllnessQuotationInputType.INPUT_DATA_COL_BEGIN
			Integer dataRowIndex=THACriticalIllnessQuotationInputType.INPUT_DATA_ROW_BEGIN
			THACriticalIllnessQuotationInputType dataCase
			Map caseInput=[:]
			caseInput.clear()
			String cellVal=''
			for(dataColIndex=THACriticalIllnessQuotationInputType.INPUT_DATA_COL_BEGIN;dataColIndex<=dataColNum;dataColIndex++){
				dataCase=new THACriticalIllnessQuotationInputType()
				dataCase.initInput()
				dataCase.initOutput()
				caseInput.clear()
				...
				dataRowIndex=THACriticalIllnessQuotationInputType.INPUT_DATA_ROW_BEGIN
	...
	public THACriticalIllnessQuotationInputType CaseData(){
	...
	public Boolean SaveOutput(){
		Boolean lreturn=false
		THACriticalIllnessQuotationInputType caseData
		try{
			Integer dataColIndex=THACriticalIllnessQuotationInputType.OUTPUT_EXCEL_COL_BEGIN+(this.CurrentCase-1)
			Integer dataRowIndex=THACriticalIllnessQuotationInputType.OUTPUT_EXCEL_ROW_BEGIN
			if((this.CurrentCase>0)&&(this.lCaseList.size()>0)){
				caseData=this.lCaseList.get(this.CurrentCase-1)
				def xlsWorkbook=ExcelKeywords.getWorkbook(THACriticalIllnessQuotationInputType.OUTPUT_EXCEL_FILE_NAME)
				def xlsSheet=ExcelKeywords.getExcelSheet(xlsWorkbook,THACriticalIllnessQuotationInputType.OUTPUT_EXCEL_SHEET_NAME)
				...
				dataRowIndex+=1
				IGNUemaHelper.saveExcelWorkbookByFileName(THACriticalIllnessQuotationInputType.OUTPUT_EXCEL_FILE_NAME,xlsWorkbook)
				lreturn=true
			}
		}catch(Exception e){
			e.printStackTrace()
