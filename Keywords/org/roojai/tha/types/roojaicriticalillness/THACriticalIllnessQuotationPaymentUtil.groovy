...
public class THACriticalIllnessQuotationPaymentUtil{
	private WebDriver driver=null
	private WebDriver selenium=null
	private lIsIBSuccessCase=false
	public THACriticalIllnessQuotationPaymentUtil(WebDriver newDriver,WebDriver newSelenium){
		super()
		...
		try{
			...
			if(lIsActualResult){
				...
				if(lIsSalesforceProceedRetrieveTodo){
					Map lMapSalesforceProceedRetrieveWebFromUrl=this.inputSalesforceProceedRetrieveWebFromUrl(THACriticalIllnessCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_PAYMENT)
			...
			switch(lPaymentMethod){
				...
				case '4':
					...
					THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_MOBILE_NO=lTrueMoneyMobileNo
					THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_EMAIL=lTrueMoneyEmail
					THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_SUCCESS_CASE=lTrueMoneySuccessCase
					if(!lMapPaymentByTrueMoney.Result){
						return lreturn
					}
					break
				case '5':
					...
					THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_MOBILE_NO=lRabbitLinePayMobileNo
					THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_EMAIL=lRabbitLinePayEmail
					THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_SUCCESS_CASE=lRabbitLinePaySuccessCase
					if(!lMapPaymentByRabbitLinePay.Result){
						return lreturn
					}
					break
				case '6':
					...
					THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_CARD=lMobileBankingCard
					THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_SUCCESS_CASE=lMobileBankingSuccessCase
					if(!lMapPaymentByMobileBanking.Result){
						return lreturn
					}
					break
	...
	public Map inputLanguage(String strLanguage){
		...
		if(lLanguage.length()<=0){
			lLanguage=THACriticalIllnessQuotationPaymentType.DEFAULT_LANGUAGE
		}
		...
		try{
			...
			if(!lLanguageItem){
				lLanguageItem=lLanguageList.get(THACriticalIllnessQuotationPaymentType.DEFAULT_LANGUAGE)
			}
	...
	public Map inputPaymentMethod(String strPaymentMethod){
		...
		try{
			...
			if(!lPaymentMethodItem){
				lPaymentMethodItem=lPaymentMethodList.get(THACriticalIllnessQuotationPaymentType.DEFAULT_PAYMENT_METHOD)
			}
			lPaymentMethod=''
			...
			THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_METHOD=lPaymentMethod
	...
	public Map inputPaymentByQR(String strPaymentTaxDeduction,String strPaymentEPolicy){
		...
		if(lPaymentTaxDeduction.length()<=0){
			lPaymentTaxDeduction=THACriticalIllnessQuotationPaymentType.DEFAULT_PAYMENT_TAX_DEDUCTION
		}
		String lPaymentEPolicy=strPaymentEPolicy.trim()
		if(lPaymentEPolicy.length()<=0){
			lPaymentEPolicy=THACriticalIllnessQuotationPaymentType.DEFAULT_PAYMENT_E_POLICY
		}
		String lQrAmtYearly=''
		try{
			Boolean lResult=false
	...
	public Map inputPaymentByIB(String strIbBank,String strPaymentAutoRenew,String strPaymentTaxDeduction,String strPaymentEPolicy,String strIbSuccessCase){
		...
		if(lPaymentAutoRenew.length()<=0){
			lPaymentAutoRenew=THACriticalIllnessQuotationPaymentType.DEFAULT_PAYMENT_AUTO_RENEW
		}
		String lPaymentTaxDeduction=strPaymentTaxDeduction.trim()
		if(lPaymentTaxDeduction.length()<=0){
			lPaymentTaxDeduction=THACriticalIllnessQuotationPaymentType.DEFAULT_PAYMENT_TAX_DEDUCTION
		...
		if(lIbSuccessCase.length()<=0){
			lIbSuccessCase=THACriticalIllnessQuotationPaymentType.DEFAULT_IB_SUCCESS_CASE
		}
		String lIbAmtYearly=''
		try{
			Boolean lResult=false
			...
			if(!lIbBankItem){
				lIbBankItem=lIbBankList.get(THACriticalIllnessQuotationPaymentType.DEFAULT_IB_BANK)
			}
			lIbBank=''
	...
	public Map inputPaymentByCC(String strCcIsYearly,String strCcIsInstalment,String strCcNumber,String strCcName,String strCcExpiryMonth,String strCcExpiryYear,String strCcCvv,String strCcBank,String strPaymentAutoRenew,String strPaymentTaxDeduction,String strPaymentEPolicy,String strCcSuccessCase){
		...
		if(lCcIsYearly.length()<=0){
			lCcIsYearly=THACriticalIllnessQuotationPaymentType.DEFAULT_CC_IS_YEARLY
		}
		String lCcIsInstalment=strCcIsInstalment.trim()
		if(lCcIsInstalment.length()<=0){
			lCcIsInstalment=THACriticalIllnessQuotationPaymentType.DEFAULT_CC_IS_INSTALMENT
		}
		String lCcNumber=strCcNumber.trim()
		if(lCcNumber.length()<=0){
			lCcNumber=THACriticalIllnessQuotationPaymentType.DEFAULT_CC_NUMBER
		}
		String lCcName=strCcName.trim()
		if(lCcName.length()<=0){
			lCcName=THACriticalIllnessQuotationPaymentType.DEFAULT_CC_NAME
		}
		String lCcExpiryMonth=strCcExpiryMonth.trim()
		if(lCcExpiryMonth.length()<=0){
			lCcExpiryMonth=THACriticalIllnessQuotationPaymentType.DEFAULT_CC_EXPIRY_MONTH
		}
		String lCcExpiryYear=strCcExpiryYear.trim()
		if(lCcExpiryYear.length()<=0){
			lCcExpiryYear=THACriticalIllnessQuotationPaymentType.DEFAULT_CC_EXPIRY_YEAR
		}
		String lCcCvv=strCcCvv.trim()
		if(lCcCvv.length()<=0){
			lCcCvv=THACriticalIllnessQuotationPaymentType.DEFAULT_CC_CVV
		}
		String lCcBank=strCcBank.trim()
		if(lCcBank.length()<=0){
			lCcBank=THACriticalIllnessQuotationPaymentType.DEFAULT_CC_BANK
		}
		String lPaymentAutoRenew=strPaymentAutoRenew.trim()
		if(lPaymentAutoRenew.length()<=0){
			lPaymentAutoRenew=THACriticalIllnessQuotationPaymentType.DEFAULT_PAYMENT_AUTO_RENEW
		...
		if(lCcSuccessCase.length()<=0){
			lCcSuccessCase=THACriticalIllnessQuotationPaymentType.DEFAULT_CC_SUCCESS_CASE
		}
		String lCcAmtYearly=''
		String lCcAmtDeposit=''
		String lCcAmtMonthly=''
	...
	public Map inputPaymentByTrueMoney(String strTrueMoneyIsYearly,String strTrueMoneyInstalment,String strTrueMoneyMobileNo,String strTrueMoneyEmail,String strPaymentAutoRenew,String strPaymentTaxDeduction,String strPaymentEPolicy,String strTrueMoneySuccessCase){
		...
		if(lTrueMoneyIsYearly.length()<=0){
			lTrueMoneyIsYearly=THACriticalIllnessQuotationPaymentType.DEFAULT_TRUE_MONEY_IS_YEARLY
		}
		String lTrueMoneyInstalment=strTrueMoneyInstalment.trim()
		if(lTrueMoneyInstalment.length()<=0){
			lTrueMoneyInstalment=THACriticalIllnessQuotationPaymentType.DEFAULT_TRUE_MONEY_INSTALMENT
		}
		String lTrueMoneyMobileNo=strTrueMoneyMobileNo.trim()
		if(lTrueMoneyMobileNo.length()<=0){
			lTrueMoneyMobileNo=THACriticalIllnessQuotationPaymentType.DEFAULT_TRUE_MONEY_MOBILE_NO
		}
		String lTrueMoneyEmail=strTrueMoneyEmail.trim()
		if(lTrueMoneyEmail.length()<=0){
			lTrueMoneyEmail=THACriticalIllnessQuotationPaymentType.DEFAULT_TRUE_MONEY_EMAIL
		}
		String lPaymentAutoRenew=strPaymentAutoRenew.trim()
		if(lPaymentAutoRenew.length()<=0){
			lPaymentAutoRenew=THACriticalIllnessQuotationPaymentType.DEFAULT_PAYMENT_AUTO_RENEW
		...
		if(lTrueMoneySuccessCase.length()<=0){
			lTrueMoneySuccessCase=THACriticalIllnessQuotationPaymentType.DEFAULT_TRUE_MONEY_SUCCESS_CASE
		}
		String lTrueMoneyAmtYearly=''
		String lTrueMoneyAmtDeposit=''
		String lTrueMoneyAmtMonthly=''
	...
	public Map inputPaymentByRabbitLinePay(String strRabbitLinePayIsYearly,String strRabbitLinePayInstalment,String strRabbitLinePayMobileNo,String strRabbitLinePayEmail,String strPaymentAutoRenew,String strPaymentTaxDeduction,String strPaymentEPolicy,String strRabbitLinePaySuccessCase){
		...
		if(lRabbitLinePayIsYearly.length()<=0){
			lRabbitLinePayIsYearly=THACriticalIllnessQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_IS_YEARLY
		}
		String lRabbitLinePayInstalment=strRabbitLinePayInstalment.trim()
		if(lRabbitLinePayInstalment.length()<=0){
			lRabbitLinePayInstalment=THACriticalIllnessQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_INSTALMENT
		}
		String lRabbitLinePayMobileNo=strRabbitLinePayMobileNo.trim()
		if(lRabbitLinePayMobileNo.length()<=0){
			lRabbitLinePayMobileNo=THACriticalIllnessQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_MOBILE_NO
		}
		String lRabbitLinePayEmail=strRabbitLinePayEmail.trim()
		if(lRabbitLinePayEmail.length()<=0){
			lRabbitLinePayEmail=THACriticalIllnessQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_EMAIL
		}
		String lPaymentAutoRenew=strPaymentAutoRenew.trim()
		if(lPaymentAutoRenew.length()<=0){
			lPaymentAutoRenew=THACriticalIllnessQuotationPaymentType.DEFAULT_PAYMENT_AUTO_RENEW
		...
		if(lRabbitLinePaySuccessCase.length()<=0){
			lRabbitLinePaySuccessCase=THACriticalIllnessQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_SUCCESS_CASE
		}
		String lRabbitLinePayAmtYearly=''
		String lRabbitLinePayAmtDeposit=''
		String lRabbitLinePayAmtMonthly=''
	...
	public Map inputPaymentByMobileBanking(String strMobileBankingIsYearly,String strMobileBankingInstalment,String strMobileBankingCard,String strPaymentAutoRenew,String strPaymentTaxDeduction,String strPaymentEPolicy,String strMobileBankingSuccessCase){
		...
		if(lMobileBankingIsYearly.length()<=0){
			lMobileBankingIsYearly=THACriticalIllnessQuotationPaymentType.DEFAULT_MOBILE_BANKING_IS_YEARLY
		}
		String lMobileBankingInstalment=strMobileBankingInstalment.trim()
		if(lMobileBankingInstalment.length()<=0){
			lMobileBankingInstalment=THACriticalIllnessQuotationPaymentType.DEFAULT_MOBILE_BANKING_INSTALMENT
		}
		String lMobileBankingCard=strMobileBankingCard.trim()
		if(lMobileBankingCard.length()<=0){
			lMobileBankingCard=THACriticalIllnessQuotationPaymentType.DEFAULT_MOBILE_BANKING_CARD
		}
		String lPaymentAutoRenew=strPaymentAutoRenew.trim()
		if(lPaymentAutoRenew.length()<=0){
			lPaymentAutoRenew=THACriticalIllnessQuotationPaymentType.DEFAULT_PAYMENT_AUTO_RENEW
		...
		if(lMobileBankingSuccessCase.length()<=0){
			lMobileBankingSuccessCase=THACriticalIllnessQuotationPaymentType.DEFAULT_MOBILE_BANKING_SUCCESS_CASE
		}
		String lMobileBankingAmtYearly=''
		String lMobileBankingAmtDeposit=''
		String lMobileBankingAmtMonthly=''
		...
		try{
			...
			if(!lMobileBankingCardItem){
				lMobileBankingCardItem=lMobileBankingCardList.get(THACriticalIllnessQuotationPaymentType.DEFAULT_MOBILE_BANKING_CARD)
			}
			String lTagPaymentMethodModalForm=''
			String lLocatorPaymentMethodModalForm='#method-detail-modal'
			WebElement lElementPaymentMethodModalForm=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPaymentMethodModalForm)
	...
	public Map inputSalesforceProceedTodo(String strSalesforceProceedTodo){
		...
		if(lSalesforceProceedTodo.length()<=0){
			lSalesforceProceedTodo=THACriticalIllnessQuotationPaymentType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
	...
	public Map inputSalesforceProceedVerifyBefore(Map caseOutput){
		...
		try{
			...
			switch(THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_METHOD){
				case '2':
			...
			if(!IGNUemaHelper.checkStringContainString(THACriticalIllnessCoreType.CURRENT_QUOTATION_PAYMENT_METHOD,'3')){
				...
				if(lOutstandingBalance<1){
					for(Integer lIndex=0;lIndex<=5;lIndex++){
						...
						if(IGNUemaHelper.checkStringContainString(lQuoteLineItem,'FEE')){
	...
	public Map inputSalesforceProceedRetrieveTodo(String strSalesforceProceedRetrieveTodo,String strAccountNameAnonymous){
		...
		if(lSalesforceProceedRetrieveTodo.length()<=0){
			lSalesforceProceedRetrieveTodo=THACriticalIllnessQuotationPaymentType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
