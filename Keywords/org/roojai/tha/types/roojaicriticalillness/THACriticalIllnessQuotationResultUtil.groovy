...
public class THACriticalIllnessQuotationResultUtil{
	private WebDriver driver=null
	private WebDriver selenium=null
	public THACriticalIllnessQuotationResultUtil(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	public Boolean checkAnonymousAccountToGetActualOpportunityNumberByRedirecting(){
		...
		try{
			...
			if(lRedirected){
				...
				this.waitUntilLoaded()
				lRedirected=IGNUemaHelper.checkElementPresentByElementId(this.driver,'sci-packages',30)
				if(!lRedirected){
					WebUI.back()
					this.waitUntilLoaded()
				}
					...
					case 'A':
						...
						if(lIsActualResult){
							...
							if(lIsSalesforceProceedTodo){
								...
								THACriticalIllnessCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_GENERAL=caseOutput.SalesforceVerifyDataOpportunityObjQuoteAccessUrlC
								THACriticalIllnessCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_PAYMENT=caseOutput.SalesforceVerifyDataOpportunityObjPayOnlineUrlC
								THACriticalIllnessCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_SOCIAL=caseOutput.SalesforceVerifyDataOpportunityObjCustomerRatingUrlC
								lIsActualResult=lMapGetSalesforceDataByOpportunityNumberToMapCaseOutput.Result
								lActualResult=IGNUemaHelper.convertBooleanToString(lIsActualResult)
								caseOutput.ActualResult=lActualResult
							}
							...
							if(lIsActualResult){
								...
								if(lIsSalesforceProceedRetrieveTodo){
									Map lMapSalesforceProceedRetrieveWebFromUrl=this.inputSalesforceProceedRetrieveWebFromUrl(THACriticalIllnessCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_GENERAL)
	...
	public Map inputPositiveCase(String strPositiveCase){
		...
		if(lPositiveCase.length()<=0){
			lPositiveCase=THACriticalIllnessQuotationResultType.DEFAULT_POSITIVE_CASE
		}
		try{
	...
	public String checkAmount(String strText){
		String lreturn=''
		String lText=strText.trim()
		try{
			if(lText.contains('฿')){
				...
				if(lTextLength>0){
					String []lTextCheck=lText
					Integer lTextIndex=0
					for(lTextIndex=0;lTextIndex<=lTextLength;lTextIndex++){
						if(lTextCheck[lTextIndex]=='฿'){
							lText=lText.substring(lTextIndex+1)
							break
						}
					}
					if(lText.contains('฿')){
						Integer lTextIndex2=0
						lTextLength=lText.length()
						String []lTextCheck2=lText
						for(lTextIndex=0;lTextIndex2<=lTextLength;lTextIndex2++){
							if(lTextCheck2[lTextIndex2]=='฿'){
								lText=lText.substring(0,lTextIndex2)
								break
							}
						}
					}
	...
	public Boolean checkPaymentOptionCriticalIllnessSwitch(){
		Boolean lreturn=false
		try{
			String lTagPaymentOptionCriticalIllnessSwitchButton='paymentOptions'
			String lLocatorPaymentOptionCriticalIllnessSwitchButton=IGNUemaHelper.getTagDataSeleniumKey(lTagPaymentOptionCriticalIllnessSwitchButton)
			WebElement lElementPaymentOptionCriticalIllnessSwitchButton=IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,lTagPaymentOptionCriticalIllnessSwitchButton)
			if(lElementPaymentOptionCriticalIllnessSwitchButton){
				Boolean lIsPaymentOptionCriticalIllnessSwitchButtonDisabled=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPaymentOptionCriticalIllnessSwitchButton,'disabled')
				Boolean lIsPaymentOptionCriticalIllnessSwitchButtonAsMonthly=IGNUemaHelper.getDataSeleniumValueWebElement(this.driver,lElementPaymentOptionCriticalIllnessSwitchButton).toUpperCase().contains('NO DEPOSIT')
				if(!lIsPaymentOptionCriticalIllnessSwitchButtonDisabled){
					if(!lIsPaymentOptionCriticalIllnessSwitchButtonAsMonthly){
						IGNUemaHelper.webJsScrollToElement(this.driver,lElementPaymentOptionCriticalIllnessSwitchButton)
						lreturn=IGNUemaHelper.webJsClickWebElementFinalWithScroll(this.driver,lElementPaymentOptionCriticalIllnessSwitchButton)
					}
				}
				lElementPaymentOptionCriticalIllnessSwitchButton=IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,lTagPaymentOptionCriticalIllnessSwitchButton)
				Boolean lIsPaymentOptionCriticalIllnessSwitchButtonAsYearly=IGNUemaHelper.getDataSeleniumValueWebElement(this.driver,lElementPaymentOptionCriticalIllnessSwitchButton).toUpperCase().contains('YEARLY')
				if(!lIsPaymentOptionCriticalIllnessSwitchButtonAsYearly){
					IGNUemaHelper.webJsScrollToElement(this.driver,lElementPaymentOptionCriticalIllnessSwitchButton)
					lreturn=IGNUemaHelper.webJsClickWebElementFinalWithScroll(this.driver,lElementPaymentOptionCriticalIllnessSwitchButton)
				}
			}
	...
	public Map inputPostPrePackageA(){
		...
		try{
			...
			lreturn.put('PostPrePackagePremiumQuarterlyTotal','')
			lreturn.put('Result',lResult)
			String lTagPaymentOptionCancerSwitchButton='paymentOptions'
	...
	public Map inputLanguageA(String strLanguage){
		...
		if(lLanguage.length()<=0){
			lLanguage=THACriticalIllnessQuotationResultType.DEFAULT_LANGUAGE
		}
		...
		try{
			...
			if(!lLanguageItem){
				lLanguageItem=lLanguageList.get(THACriticalIllnessQuotationResultType.DEFAULT_LANGUAGE)
			}
	...
	public Map inputPackageOrganFailure(String strPackageOrganFailure){
		...
		try{
			...
			if(IGNUemaHelper.CURRENT_AB_TESTING_ENABLE){
				switch(IGNUemaHelper.CURRENT_AB_TESTING_VERSION){
					case 'A':
						lreturn=this.inputPackageOrganFailureA(strPackageOrganFailure)
						break
					case 'B':
						lreturn=this.inputPackageOrganFailureB(strPackageOrganFailure)
						break
					default:
						return lreturn
						break
	...
	public Map inputPackageOrganFailureA(String strPackageOrganFailure){
		Map lreturn=[:]
		String lPackageOrganFailure=strPackageOrganFailure.toUpperCase().trim()
		Map lPackageOrganFailureList=['1':'1','2':'2','3':'3','4':'4']
		try{
			Boolean lResult=false
			lreturn.put('PackageOrganFailure','')
			lreturn.put('Result',lResult)
			String lTagPackageOrganFailureButtonKey='package-Organ Failure'
			String lPackageOrganFailureItemDefault=lPackageOrganFailureList.get(THACriticalIllnessQuotationResultType.DEFAULT_PACKAGE_ORGAN_FAILURE.toUpperCase())
			String lPackageOrganFailureItemDefaultValue=lPackageOrganFailureItemDefault
			String lLocatorPackageOrganFailureDefaultButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageOrganFailureButtonKey,lPackageOrganFailureItemDefaultValue)
			WebElement lElementPackageOrganFailureDefaultButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageOrganFailureButtonKey,lPackageOrganFailureItemDefaultValue)
			if(!lElementPackageOrganFailureDefaultButton){
				lPackageOrganFailureItemDefaultValue='2'
				lLocatorPackageOrganFailureDefaultButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageOrganFailureButtonKey,lPackageOrganFailureItemDefaultValue)
				lElementPackageOrganFailureDefaultButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageOrganFailureButtonKey,lPackageOrganFailureItemDefaultValue)
			}
			if(!lElementPackageOrganFailureDefaultButton){
				lPackageOrganFailure='No Option'
			}else{
				String lPackageOrganFailureItem=lPackageOrganFailureList.get(lPackageOrganFailure)
				if(!lPackageOrganFailureItem){
					lPackageOrganFailureItem=lPackageOrganFailureItemDefaultValue
				}
				lPackageOrganFailure=''
				String lTagPackageOrganFailureButtonValue=lPackageOrganFailureItem
				String lLocatorPackageOrganFailureButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageOrganFailureButtonKey,lTagPackageOrganFailureButtonValue)
				WebElement lElementPackageOrganFailureButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageOrganFailureButtonKey,lTagPackageOrganFailureButtonValue)
				if(!lElementPackageOrganFailureButton){
					String lLocatorPackageOrganFailureActiveButton=IGNUemaHelper.getTagDataSeleniumKey(lTagPackageOrganFailureButtonKey)+'.active'
					WebElement lElementPackageOrganFailureActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageOrganFailureActiveButton)
					lElementPackageOrganFailureButton=lElementPackageOrganFailureActiveButton
				}
				if(!lElementPackageOrganFailureButton){
					lPackageOrganFailure='No Option'
				}else{
					IGNUemaHelper.webJsScrollToElement(this.driver,lElementPackageOrganFailureButton)
					IGNUemaHelper.webJsClickWebElementFinalWithScroll(this.driver,lElementPackageOrganFailureButton)
					lElementPackageOrganFailureButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageOrganFailureButtonKey,lTagPackageOrganFailureButtonValue)
					Boolean lIsElementPackageOrganFailureButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageOrganFailureButton,'active')
					if(lIsElementPackageOrganFailureButtonSelected){
						lPackageOrganFailure=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageOrganFailureButton).trim()
					}else{
						lPackageOrganFailure='No Option'
					}
				}
				lPackageOrganFailure=this.checkAmount(lPackageOrganFailure)
			}
			this.checkPriceChangeToYearlyPayment()
			lPackageOrganFailure=lPackageOrganFailure.trim()
	...
	public Map inputPackageOrganFailureB(String strPackageOrganFailure){
		Map lreturn=[:]
		String lPackageOrganFailure=strPackageOrganFailure.toUpperCase().trim()
		Map lPackageOrganFailureList=['1':'0','2':'1','3':'2','4':'3','5':'4']
		try{
			Boolean lResult=false
			lreturn.put('PackageOrganFailure','')
			lreturn.put('Result',lResult)
	...
	public Map inputPackageNeurological(String strPackageNeurological){
		...
		try{
			...
			if(IGNUemaHelper.CURRENT_AB_TESTING_ENABLE){
				switch(IGNUemaHelper.CURRENT_AB_TESTING_VERSION){
					case 'A':
						lreturn=this.inputPackageNeurologicalA(strPackageNeurological)
						break
					case 'B':
						lreturn=this.inputPackageNeurologicalB(strPackageNeurological)
						break
					default:
						return lreturn
						break
	...
	public Map inputPackageNeurologicalA(String strPackageNeurological){
		Map lreturn=[:]
		String lPackageNeurological=strPackageNeurological.toUpperCase().trim()
		Map lPackageNeurologicalList=['1':'1','2':'2','3':'3','4':'4']
		try{
			Boolean lResult=false
			lreturn.put('PackageNeurological','')
			lreturn.put('Result',lResult)
			String lTagPackageNeurologicalButtonKey='package-Neuro'
			String lPackageNeurologicalItemDefault=lPackageNeurologicalList.get(THACriticalIllnessQuotationResultType.DEFAULT_PACKAGE_NEUROLOGICAL.toUpperCase())
			String lPackageNeurologicalItemDefaultValue=lPackageNeurologicalItemDefault
			String lLocatorPackageNeurologicalDefaultButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageNeurologicalButtonKey,lPackageNeurologicalItemDefaultValue)
			WebElement lElementPackageNeurologicalDefaultButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageNeurologicalButtonKey,lPackageNeurologicalItemDefaultValue)
			if(!lElementPackageNeurologicalDefaultButton){
				lPackageNeurologicalItemDefaultValue='2'
				lLocatorPackageNeurologicalDefaultButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageNeurologicalButtonKey,lPackageNeurologicalItemDefaultValue)
				lElementPackageNeurologicalDefaultButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageNeurologicalButtonKey,lPackageNeurologicalItemDefaultValue)
			}
			if(!lElementPackageNeurologicalDefaultButton){
				lPackageNeurological='No Option'
			}else{
				String lPackageNeurologicalItem=lPackageNeurologicalList.get(lPackageNeurological)
				if(!lPackageNeurologicalItem){
					lPackageNeurologicalItem=lPackageNeurologicalItemDefaultValue
				}
				lPackageNeurological=''
				String lTagPackageNeurologicalButtonValue=lPackageNeurologicalItem
				String lLocatorPackageNeurologicalButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageNeurologicalButtonKey,lTagPackageNeurologicalButtonValue)
				WebElement lElementPackageNeurologicalButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageNeurologicalButtonKey,lTagPackageNeurologicalButtonValue)
				if(!lElementPackageNeurologicalButton){
					String lLocatorPackageNeurologicalActiveButton=IGNUemaHelper.getTagDataSeleniumKey(lTagPackageNeurologicalButtonKey)+'.active'
					WebElement lElementPackageNeurologicalActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageNeurologicalActiveButton)
					lElementPackageNeurologicalButton=lElementPackageNeurologicalActiveButton
				}
				if(!lElementPackageNeurologicalButton){
					lPackageNeurological='No Option'
				}else{
					IGNUemaHelper.webJsScrollToElement(this.driver,lElementPackageNeurologicalButton)
					IGNUemaHelper.webJsClickWebElementFinalWithScroll(this.driver,lElementPackageNeurologicalButton)
					lElementPackageNeurologicalButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageNeurologicalButtonKey,lTagPackageNeurologicalButtonValue)
					Boolean lIsElementPackageNeurologicalButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageNeurologicalButton,'active')
					if(lIsElementPackageNeurologicalButtonSelected){
						lPackageNeurological=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageNeurologicalButton).trim()
					}else{
						lPackageNeurological='No Option'
					}
				}
				lPackageNeurological=this.checkAmount(lPackageNeurological)
			}
			this.checkPriceChangeToYearlyPayment()
			lPackageNeurological=lPackageNeurological.trim()
	...
	public Map inputPackageNeurologicalB(String strPackageNeurological){
		Map lreturn=[:]
		String lPackageNeurological=strPackageNeurological.toUpperCase().trim()
		Map lPackageNeurologicalList=['1':'1','2':'2','3':'3','4':'4']
		try{
	...
	public Map inputPackageCardiovascular(String strPackageCardiovascular){
		...
		try{
			...
			if(IGNUemaHelper.CURRENT_AB_TESTING_ENABLE){
				switch(IGNUemaHelper.CURRENT_AB_TESTING_VERSION){
					case 'A':
						lreturn=this.inputPackageCardiovascularA(strPackageCardiovascular)
						break
					case 'B':
						lreturn=this.inputPackageCardiovascularB(strPackageCardiovascular)
						break
					default:
						return lreturn
						break
	...
	public Map inputPackageCardiovascularA(String strPackageCardiovascular){
		Map lreturn=[:]
		String lPackageCardiovascular=strPackageCardiovascular.toUpperCase().trim()
		Map lPackageCardiovascularList=['1':'1','2':'2','3':'3','4':'4']
		try{
			Boolean lResult=false
			lreturn.put('PackageCardiovascular','')
			lreturn.put('Result',lResult)
			String lTagPackageCardiovascularButtonKey='package-Cardiovascular'
			String lPackageCardiovascularItemDefault=lPackageCardiovascularList.get(THACriticalIllnessQuotationResultType.DEFAULT_PACKAGE_CARDIOVASCULAR.toUpperCase())
			String lPackageCardiovascularItemDefaultValue=lPackageCardiovascularItemDefault
			String lLocatorPackageCardiovascularDefaultButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageCardiovascularButtonKey,lPackageCardiovascularItemDefaultValue)
			WebElement lElementPackageCardiovascularDefaultButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageCardiovascularButtonKey,lPackageCardiovascularItemDefaultValue)
			if(!lElementPackageCardiovascularDefaultButton){
				lPackageCardiovascularItemDefaultValue='2'
				lLocatorPackageCardiovascularDefaultButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageCardiovascularButtonKey,lPackageCardiovascularItemDefaultValue)
				lElementPackageCardiovascularDefaultButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageCardiovascularButtonKey,lPackageCardiovascularItemDefaultValue)
			}
			if(!lElementPackageCardiovascularDefaultButton){
				lPackageCardiovascular='No Option'
			}else{
				String lPackageCardiovascularItem=lPackageCardiovascularList.get(lPackageCardiovascular)
				if(!lPackageCardiovascularItem){
					lPackageCardiovascularItem=lPackageCardiovascularItemDefaultValue
				}
				lPackageCardiovascular=''
				String lTagPackageCardiovascularButtonValue=lPackageCardiovascularItem
				String lLocatorPackageCardiovascularButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageCardiovascularButtonKey,lTagPackageCardiovascularButtonValue)
				WebElement lElementPackageCardiovascularButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageCardiovascularButtonKey,lTagPackageCardiovascularButtonValue)
				if(!lElementPackageCardiovascularButton){
					String lLocatorPackageCardiovascularActiveButton=IGNUemaHelper.getTagDataSeleniumKey(lTagPackageCardiovascularButtonKey)+'.active'
					WebElement lElementPackageCardiovascularActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageCardiovascularActiveButton)
					lElementPackageCardiovascularButton=lElementPackageCardiovascularActiveButton
				}
				if(!lElementPackageCardiovascularButton){
					lPackageCardiovascular='No Option'
				}else{
					IGNUemaHelper.webJsScrollToElement(this.driver,lElementPackageCardiovascularButton)
					IGNUemaHelper.webJsClickWebElementFinalWithScroll(this.driver,lElementPackageCardiovascularButton)
					lElementPackageCardiovascularButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageCardiovascularButtonKey,lTagPackageCardiovascularButtonValue)
					Boolean lIsElementPackageCardiovascularButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageCardiovascularButton,'active')
					if(lIsElementPackageCardiovascularButtonSelected){
						lPackageCardiovascular=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageCardiovascularButton).trim()
					}else{
						lPackageCardiovascular='No Option'
					}
				}
				lPackageCardiovascular=this.checkAmount(lPackageCardiovascular)
			}
			this.checkPriceChangeToYearlyPayment()
			lPackageCardiovascular=lPackageCardiovascular.trim()
	...
	public Map inputPackageCardiovascularB(String strPackageCardiovascular){
		Map lreturn=[:]
		String lPackageCardiovascular=strPackageCardiovascular.toUpperCase().trim()
		Map lPackageCardiovascularList=['1':'1','2':'2','3':'3','4':'4']
		try{
	...
	public Map inputPackageCancer(String strPackageCancer){
		...
		try{
			...
			if(IGNUemaHelper.CURRENT_AB_TESTING_ENABLE){
				switch(IGNUemaHelper.CURRENT_AB_TESTING_VERSION){
					case 'A':
						lreturn=this.inputPackageCancerA(strPackageCancer)
						break
					case 'B':
						lreturn=this.inputPackageCancerB(strPackageCancer)
						break
					default:
						return lreturn
						break
	...
	public Map inputPackageCancerA(String strPackageCancer){
		Map lreturn=[:]
		String lPackageCancer=strPackageCancer.toUpperCase().trim()
		Map lPackageCancerList=['1':'1','2':'2','3':'3','4':'4']
		try{
			Boolean lResult=false
			lreturn.put('PackageCancer','')
			lreturn.put('Result',lResult)
			String lTagPackageCancerButtonKey='package-Cancer'
			String lPackageCancerItemDefault=lPackageCancerList.get(THACriticalIllnessQuotationResultType.DEFAULT_PACKAGE_CANCER.toUpperCase())
			String lPackageCancerItemDefaultValue=lPackageCancerItemDefault
			String lLocatorPackageCancerDefaultButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageCancerButtonKey,lPackageCancerItemDefaultValue)
			WebElement lElementPackageCancerDefaultButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageCancerButtonKey,lPackageCancerItemDefaultValue)
			if(!lElementPackageCancerDefaultButton){
				lPackageCancerItemDefaultValue='2'
				lLocatorPackageCancerDefaultButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageCancerButtonKey,lPackageCancerItemDefaultValue)
				lElementPackageCancerDefaultButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageCancerButtonKey,lPackageCancerItemDefaultValue)
			}
			if(!lElementPackageCancerDefaultButton){
				lPackageCancer='No Option'
			}else{
				String lPackageCancerItem=lPackageCancerList.get(lPackageCancer)
				if(!lPackageCancerItem){
					lPackageCancerItem=lPackageCancerItemDefaultValue
				}
				lPackageCancer=''
				String lTagPackageCancerButtonValue=lPackageCancerItem
				String lLocatorPackageCancerButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagPackageCancerButtonKey,lTagPackageCancerButtonValue)
				WebElement lElementPackageCancerButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageCancerButtonKey,lTagPackageCancerButtonValue)
				if(!lElementPackageCancerButton){
					String lLocatorPackageCancerActiveButton=IGNUemaHelper.getTagDataSeleniumKey(lTagPackageCancerButtonKey)+'.active'
					WebElement lElementPackageCancerActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageCancerActiveButton)
					lElementPackageCancerButton=lElementPackageCancerActiveButton
				}
				if(!lElementPackageCancerButton){
					lPackageCancer='No Option'
				}else{
					IGNUemaHelper.webJsScrollToElement(this.driver,lElementPackageCancerButton)
					IGNUemaHelper.webJsClickWebElementFinalWithScroll(this.driver,lElementPackageCancerButton)
					lElementPackageCancerButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagPackageCancerButtonKey,lTagPackageCancerButtonValue)
					Boolean lIsElementPackageCancerButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageCancerButton,'active')
					if(lIsElementPackageCancerButtonSelected){
						lPackageCancer=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageCancerButton).trim()
					}else{
						lPackageCancer='No Option'
					}
				}
				lPackageCancer=this.checkAmount(lPackageCancer)
			}
			this.checkPriceChangeToYearlyPayment()
			lPackageCancer=lPackageCancer.trim()
	...
	public Map inputPackageCancerB(String strPackageCancer){
		Map lreturn=[:]
		String lPackageCancer=strPackageCancer.toUpperCase().trim()
		Map lPackageCancerList=['1':'1','2':'2','3':'3','4':'4']
		try{
	...
	public Map inputHospitalIncomeBenefitPlanA(String strHospitalIncomeBenefitPlan,String strQueryHospitalIncomeBenefitQuestion01,String strQueryHospitalIncomeBenefitQuestion02,String strQueryHospitalIncomeBenefitQuestion03,String strQueryHospitalIncomeBenefitQuestion04,String strQueryHospitalIncomeBenefitQuestion05,String strQueryHospitalIncomeBenefitQuestion06,String strQueryHospitalIncomeBenefitQuestion07,String strQueryHospitalIncomeBenefitQuestion08,String strQueryHospitalIncomeBenefitQuestion09,String strQueryHospitalIncomeBenefitQuestion10){
		...
		if(lHospitalIncomeBenefitPlan.length()<=0){
			lHospitalIncomeBenefitPlan=THACriticalIllnessQuotationResultType.DEFAULT_HOSPITAL_INCOME_BENEFIT_PLAN
		}
		...
		if(lQueryHospitalIncomeBenefitQuestion01.length()<=0){
			lQueryHospitalIncomeBenefitQuestion01=THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_01
		}
		...
		if(lQueryHospitalIncomeBenefitQuestion02.length()<=0){
			lQueryHospitalIncomeBenefitQuestion02=THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_02
		}
		...
		if(lQueryHospitalIncomeBenefitQuestion03.length()<=0){
			lQueryHospitalIncomeBenefitQuestion03=THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_03
		}
		...
		if(lQueryHospitalIncomeBenefitQuestion04.length()<=0){
			lQueryHospitalIncomeBenefitQuestion04=THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_04
		}
		...
		if(lQueryHospitalIncomeBenefitQuestion05.length()<=0){
			lQueryHospitalIncomeBenefitQuestion05=THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_05
		}
		...
		if(lQueryHospitalIncomeBenefitQuestion06.length()<=0){
			lQueryHospitalIncomeBenefitQuestion06=THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_06
		}
		...
		if(lQueryHospitalIncomeBenefitQuestion07.length()<=0){
			lQueryHospitalIncomeBenefitQuestion07=THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_07
		}
		...
		if(lQueryHospitalIncomeBenefitQuestion08.length()<=0){
			lQueryHospitalIncomeBenefitQuestion08=THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_08
		}
		...
		if(lQueryHospitalIncomeBenefitQuestion09.length()<=0){
			lQueryHospitalIncomeBenefitQuestion09=THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_09
		}
		...
		if(lQueryHospitalIncomeBenefitQuestion10.length()<=0){
			lQueryHospitalIncomeBenefitQuestion10=THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_10
		}
		...
		try{
			...
			String lHospitalIncomeBenefitPlanItemDefault=lHospitalIncomeBenefitPlanList.get(THACriticalIllnessQuotationResultType.DEFAULT_HOSPITAL_INCOME_BENEFIT_PLAN.toUpperCase())
			...
			if(!lElementHospitalIncomeBenefitPlanDefaultButton){
				lHospitalIncomeBenefitPlanItemDefaultValue='2'
			...
			if(lIsQueryHospitalIncomeBenefitToDo){
				...
				if(!lQueryHospitalIncomeBenefitQuestion01Item){
					lQueryHospitalIncomeBenefitQuestion01Item=lQueryHospitalIncomeBenefitToDoListAnswerTypeAnswerType1.get(THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_01.toUpperCase())
				}
				lQueryHospitalIncomeBenefitQuestion01=''
				...
				if(!lQueryHospitalIncomeBenefitQuestion02Item){
					lQueryHospitalIncomeBenefitQuestion02Item=lQueryHospitalIncomeBenefitToDoListAnswerTypeAnswerType1.get(THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_02.toUpperCase())
				}
				lQueryHospitalIncomeBenefitQuestion02=''
				...
				if(!lQueryHospitalIncomeBenefitQuestion03Item){
					lQueryHospitalIncomeBenefitQuestion03Item=lQueryHospitalIncomeBenefitToDoListAnswerTypeAnswerType1.get(THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_03.toUpperCase())
				}
				lQueryHospitalIncomeBenefitQuestion03=''
				...
				if(!lQueryHospitalIncomeBenefitQuestion04Item){
					lQueryHospitalIncomeBenefitQuestion04Item=lQueryHospitalIncomeBenefitToDoListAnswerTypeAnswerType1.get(THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_04.toUpperCase())
				}
				lQueryHospitalIncomeBenefitQuestion04=''
				...
				if(!lQueryHospitalIncomeBenefitQuestion05Item){
					lQueryHospitalIncomeBenefitQuestion05Item=lQueryHospitalIncomeBenefitToDoListAnswerTypeAnswerType1.get(THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_05.toUpperCase())
				}
				lQueryHospitalIncomeBenefitQuestion05=''
				...
				if(!lQueryHospitalIncomeBenefitQuestion06Item){
					lQueryHospitalIncomeBenefitQuestion06Item=lQueryHospitalIncomeBenefitToDoListAnswerTypeAnswerType1.get(THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_06.toUpperCase())
				}
				lQueryHospitalIncomeBenefitQuestion06=''
				...
				if(!lQueryHospitalIncomeBenefitQuestion07Item){
					lQueryHospitalIncomeBenefitQuestion07Item=lQueryHospitalIncomeBenefitToDoListAnswerTypeAnswerType1.get(THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_07.toUpperCase())
				}
				lQueryHospitalIncomeBenefitQuestion07=''
				...
				if(!lQueryHospitalIncomeBenefitQuestion08Item){
					lQueryHospitalIncomeBenefitQuestion08Item=lQueryHospitalIncomeBenefitToDoListAnswerTypeAnswerType1.get(THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_08.toUpperCase())
				}
				lQueryHospitalIncomeBenefitQuestion08=''
				...
				if(!lQueryHospitalIncomeBenefitQuestion09Item){
					lQueryHospitalIncomeBenefitQuestion09Item=lQueryHospitalIncomeBenefitToDoListAnswerTypeAnswerType1.get(THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_09.toUpperCase())
				}
				lQueryHospitalIncomeBenefitQuestion09=''
				...
				if(!lQueryHospitalIncomeBenefitQuestion10Item){
					lQueryHospitalIncomeBenefitQuestion10Item=lQueryHospitalIncomeBenefitToDoListAnswerTypeAnswerType1.get(THACriticalIllnessQuotationResultType.DEFAULT_QUERY_HOSPITAL_INCOME_BENEFIT_QUESTION_10.toUpperCase())
				}
				lQueryHospitalIncomeBenefitQuestion10=''
	...
	public Map inputQuoteMeA(String strQuoteBy,String strQuoteCallWithType,String strQuoteCallWithWhen,String strQuoteCallWithTime,String strQuotePhoneNo,String strQuoteEmail,String strQuoteNewsletter,String strQuoteCallMeNow,String strQuotePDFDownloadToDo){
		...
		if(lQuoteBy.length()<=0){
			lQuoteBy=THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_BY
		}
		String lQuoteCallWithType=strQuoteCallWithType.toUpperCase().trim()
		if(lQuoteCallWithType.length()<=0){
			lQuoteCallWithType=THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_CALL_WITH_TYPE
		}
		String lQuoteCallWithWhen=strQuoteCallWithWhen.toUpperCase().trim()
		if(lQuoteCallWithWhen.length()<=0){
			lQuoteCallWithWhen=THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_CALL_WITH_WHEN
		}
		String lQuoteCallWithTime=strQuoteCallWithTime.toUpperCase().trim()
		if(lQuoteCallWithTime.length()<=0){
			lQuoteCallWithTime=THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_CALL_WITH_TIME
		}
		String lQuotePhoneNo=strQuotePhoneNo.trim()
		if(lQuotePhoneNo.length()<=0){
			lQuotePhoneNo=THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_PHONE_NO
		}
		String lQuoteEmail=strQuoteEmail.trim()
		if(lQuoteEmail.length()<=0){
			lQuoteEmail=THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_EMAIL
		}
		String lQuoteNewsletter=strQuoteNewsletter.trim()
		if(lQuoteNewsletter.length()<=0){
			lQuoteNewsletter=THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_NEWSLETTER
		}
		String lQuoteCallMeNow=strQuoteCallMeNow.trim()
		if(lQuoteCallMeNow.length()<=0){
			lQuoteCallMeNow=THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_CALL_ME_NOW
		}
		String lQuotePDFDownloadToDo=strQuotePDFDownloadToDo.trim()
		if(lQuotePDFDownloadToDo.length()<=0){
			lQuotePDFDownloadToDo=THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_PDF_DOWNLOAD_TO_DO
		}
		String lQuotePDFDownloadDetail=''
		String lQuotePDFDownloadResult=''
		...
		try{
			...
			if(!lQuoteByItem){
				lQuoteByItem=lQuoteByList.get(THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_BY.toUpperCase())
			}
			lQuoteBy=''
			...
			if(THACriticalIllnessCoreType.CURRENT_QUOTATION_HEALTH_OPPORTUNITY_NUMBER.toUpperCase().contains('CI')){
				this.checkAnonymousAccountToGetActualOpportunityNumberByRedirecting()
			}
			if(lResult){
	...
	public Map inputSalesforceProceedTodo(String strSalesforceProceedTodo){
		...
		if(lSalesforceProceedTodo.length()<=0){
			lSalesforceProceedTodo=THACriticalIllnessQuotationResultType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
	...
	public Map inputAutomationDataBefore(){
		...
		try{
			...
			String lTagPackageOrganFailureActiveButton='package-Organ Failure'
			String lLocatorPackageOrganFailureActiveButton=IGNUemaHelper.getTagDataSeleniumKey(lTagPackageOrganFailureActiveButton)+'.selected'
			WebElement lElementPackageOrganFailureActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageOrganFailureActiveButton)
			if(!lElementPackageOrganFailureActiveButton){
				lAutomationPackageOrganFailure='No Option'
			}else{
				Boolean lIsElementPackageOrganFailureButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageOrganFailureActiveButton,'selected')
				if(lIsElementPackageOrganFailureButtonSelected){
					lAutomationPackageOrganFailure=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageOrganFailureActiveButton).trim()
					lAutomationPackageOrganFailure=this.wrapTextToGetAmountValue(lAutomationPackageOrganFailure,'฿',1)
				}else{
					lAutomationPackageOrganFailure='No Option'
				}
			}
			String lTagPackageNeurologicalActiveButton='package-Neuro'
			String lLocatorPackageNeurologicalActiveButton=IGNUemaHelper.getTagDataSeleniumKey(lTagPackageNeurologicalActiveButton)+'.selected'
			WebElement lElementPackageNeurologicalActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageNeurologicalActiveButton)
			if(!lElementPackageNeurologicalActiveButton){
				lAutomationPackageNeurological='No Option'
			}else{
				Boolean lIsElementPackageNeurologicalButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageNeurologicalActiveButton,'selected')
				if(lIsElementPackageNeurologicalButtonSelected){
					lAutomationPackageNeurological=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageNeurologicalActiveButton).trim()
					lAutomationPackageNeurological=this.wrapTextToGetAmountValue(lAutomationPackageNeurological,'฿',1)
				}else{
					lAutomationPackageNeurological='No Option'
				}
			}
			String lTagPackageCardiovascularActiveButton='package-Cardiovascular'
			String lLocatorPackageCardiovascularActiveButton=IGNUemaHelper.getTagDataSeleniumKey(lTagPackageCardiovascularActiveButton)+'.selected'
			WebElement lElementPackageCardiovascularActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageCardiovascularActiveButton)
			if(!lElementPackageCardiovascularActiveButton){
				lAutomationPackageCardiovascular='No Option'
			}else{
				Boolean lIsElementPackageCardiovascularButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageCardiovascularActiveButton,'selected')
				if(lIsElementPackageCardiovascularButtonSelected){
					lAutomationPackageCardiovascular=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageCardiovascularActiveButton).trim()
					lAutomationPackageCardiovascular=this.wrapTextToGetAmountValue(lAutomationPackageCardiovascular,'฿',1)
				}else{
					lAutomationPackageCardiovascular='No Option'
				}
			}
			String lTagPackageCancerActiveButton='package-Cancer'
			String lLocatorPackageCancerActiveButton=IGNUemaHelper.getTagDataSeleniumKey(lTagPackageCancerActiveButton)+'.selected'
			WebElement lElementPackageCancerActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageCancerActiveButton)
			if(!lElementPackageCancerActiveButton){
				lAutomationPackageCancer='No Option'
			}else{
				Boolean lIsElementPackageCancerButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageCancerActiveButton,'selected')
				if(lIsElementPackageCancerButtonSelected){
					lAutomationPackageCancer=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageCancerActiveButton).trim()
					lAutomationPackageCancer=this.wrapTextToGetAmountValue(lAutomationPackageCancer,'฿',1)
				}else{
					lAutomationPackageCancer='No Option'
				}
			}
			...
			if(!lElementQueryHospitalIncomeBenefitToDoButton){
				...
				lAutomationQueryHospitalIncomeBenefitQuestion06=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagQueryHospitalIncomeBenefitToDoButton,IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,'productQuestion-'))
				if(lAutomationQueryHospitalIncomeBenefitQuestion06.length()<=0){
					lAutomationQueryHospitalIncomeBenefitQuestion06='No Option'
				}
				lAutomationQueryHospitalIncomeBenefitQuestion07=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagQueryHospitalIncomeBenefitToDoButton,IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,'productQuestion-'))
				if(lAutomationQueryHospitalIncomeBenefitQuestion07.length()<=0){
					lAutomationQueryHospitalIncomeBenefitQuestion07='No Option'
				}
				lAutomationQueryHospitalIncomeBenefitQuestion08=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagQueryHospitalIncomeBenefitToDoButton,IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,'productQuestion-'))
				if(lAutomationQueryHospitalIncomeBenefitQuestion08.length()<=0){
					lAutomationQueryHospitalIncomeBenefitQuestion08='No Option'
				}
				lAutomationQueryHospitalIncomeBenefitQuestion09=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagQueryHospitalIncomeBenefitToDoButton,IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,'productQuestion-'))
				if(lAutomationQueryHospitalIncomeBenefitQuestion09.length()<=0){
					lAutomationQueryHospitalIncomeBenefitQuestion09='No Option'
				}
				lAutomationQueryHospitalIncomeBenefitQuestion10=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagQueryHospitalIncomeBenefitToDoButton,IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,'productQuestion-'))
	...
	public Map inputAutomationDataAfter(){
		...
		try{
			...
			WebElement lElementPackageOrganFailureActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageOrganFailureActiveButton)
			if(!lElementPackageOrganFailureActiveButton){
				lSalesforceRetrieveDataPackageOrganFailure='No Option'
			}else{
				Boolean lIsElementPackageOrganFailureButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageOrganFailureActiveButton,'selected')
				if(lIsElementPackageOrganFailureButtonSelected){
					lSalesforceRetrieveDataPackageOrganFailure=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageOrganFailureActiveButton).trim()
					lSalesforceRetrieveDataPackageOrganFailure=this.wrapTextToGetAmountValue(lSalesforceRetrieveDataPackageOrganFailure,'฿',1)
				}else{
					lSalesforceRetrieveDataPackageOrganFailure='No Option'
				}
			}
			...
			WebElement lElementPackageNeurologicalActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageNeurologicalActiveButton)
			if(!lElementPackageNeurologicalActiveButton){
				lSalesforceRetrieveDataPackageNeurological='No Option'
			}else{
				Boolean lIsElementPackageNeurologicalButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageNeurologicalActiveButton,'selected')
				if(lIsElementPackageNeurologicalButtonSelected){
					lSalesforceRetrieveDataPackageNeurological=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageNeurologicalActiveButton).trim()
					lSalesforceRetrieveDataPackageNeurological=this.wrapTextToGetAmountValue(lSalesforceRetrieveDataPackageNeurological,'฿',1)
				}else{
					lSalesforceRetrieveDataPackageNeurological='No Option'
				}
			}
			...
			WebElement lElementPackageCardiovascularActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageCardiovascularActiveButton)
			if(!lElementPackageCardiovascularActiveButton){
				lSalesforceRetrieveDataPackageCardiovascular='No Option'
			}else{
				Boolean lIsElementPackageCardiovascularButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageCardiovascularActiveButton,'selected')
				if(lIsElementPackageCardiovascularButtonSelected){
					lSalesforceRetrieveDataPackageCardiovascular=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageCardiovascularActiveButton).trim()
					lSalesforceRetrieveDataPackageCardiovascular=this.wrapTextToGetAmountValue(lSalesforceRetrieveDataPackageCardiovascular,'฿',1)
				}else{
					lSalesforceRetrieveDataPackageCardiovascular='No Option'
				}
			}
			...
			WebElement lElementPackageCancerActiveButton=IGNUemaHelper.getWebElementFromCssLocator(this.driver,lLocatorPackageCancerActiveButton)
			if(!lElementPackageCancerActiveButton){
				lSalesforceRetrieveDataPackageCancer='No Option'
			}else{
				Boolean lIsElementPackageCancerButtonSelected=IGNUemaHelper.checkElementContainClassNameByWebElement(this.driver,lElementPackageCancerActiveButton,'selected')
				if(lIsElementPackageCancerButtonSelected){
					lSalesforceRetrieveDataPackageCancer=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementPackageCancerActiveButton).trim()
					lSalesforceRetrieveDataPackageCancer=this.wrapTextToGetAmountValue(lSalesforceRetrieveDataPackageCancer,'฿',1)
				}else{
					lSalesforceRetrieveDataPackageCancer='No Option'
				}
			}
			...
			if(!lElementQueryHospitalIncomeBenefitToDoButton){
				...
				lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion06=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagQueryHospitalIncomeBenefitToDoButton,IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,'productQuestion-'))
				if(lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion06.length()<=0){
					lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion06='No Option'
				}
				lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion07=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagQueryHospitalIncomeBenefitToDoButton,IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,'productQuestion-'))
				if(lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion07.length()<=0){
					lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion07='No Option'
				}
				lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion08=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagQueryHospitalIncomeBenefitToDoButton,IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,'productQuestion-'))
				if(lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion08.length()<=0){
					lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion08='No Option'
				}
				lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion09=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagQueryHospitalIncomeBenefitToDoButton,IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,'productQuestion-'))
				if(lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion09.length()<=0){
					lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion09='No Option'
				}
				lSalesforceRetrieveDataQueryHospitalIncomeBenefitQuestion10=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagQueryHospitalIncomeBenefitToDoButton,IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,'productQuestion-'))
	...
	public Map inputSalesforceProceedVerifyBefore(Map caseOutput,Boolean isQuoteByNone){
		...
		try{
			...
			for(Integer lIndex=0;lIndex<=5;lIndex++){
				...
				if(IGNUemaHelper.checkStringContainString(lQuoteLineItem,'ORGANFAILURE')){
					String lAutomationPackageOrganFailure=caseOutput.AutomationPackageOrganFailure
					String lSalesforcePackageOrganFailure=caseOutput.('SalesforceVerifyDataQuoteLineItem'+lStrRoundPad+'DetailSumAssuredC')
					Float lAutomationPrice=IGNUemaHelper.convertStringToFloat(lAutomationPackageOrganFailure,0)
					Float lSalesforcePrice=IGNUemaHelper.convertStringToFloat(lSalesforcePackageOrganFailure,0)
					Float lPrice=Math.abs(lAutomationPrice-lSalesforcePrice)
					if(lPrice>1){
						lIsOK=false
						lText=IGNUemaHelper.concatMessageLeftRightByPipe('Package OrganFailure='+lAutomationPackageOrganFailure,lSalesforcePackageOrganFailure)
						lSalesforceVerifyCompareDetail=lSalesforceVerifyCompareDetail+lText+'\n'
					}
				}
				if(IGNUemaHelper.checkStringContainString(lQuoteLineItem,'NEURO')){
					String lAutomationPackageNeurological=caseOutput.AutomationPackageNeurological
					String lSalesforcePackageNeurological=caseOutput.('SalesforceVerifyDataQuoteLineItem'+lStrRoundPad+'DetailSumAssuredC')
					Float lAutomationPrice=IGNUemaHelper.convertStringToFloat(lAutomationPackageNeurological,0)
					Float lSalesforcePrice=IGNUemaHelper.convertStringToFloat(lSalesforcePackageNeurological,0)
					Float lPrice=Math.abs(lAutomationPrice-lSalesforcePrice)
					if(lPrice>1){
						lIsOK=false
						lText=IGNUemaHelper.concatMessageLeftRightByPipe('Package Neurological='+lAutomationPackageNeurological,lSalesforcePackageNeurological)
						lSalesforceVerifyCompareDetail=lSalesforceVerifyCompareDetail+lText+'\n'
					}
				}
				if(IGNUemaHelper.checkStringContainString(lQuoteLineItem,'CVD')){
					String lAutomationPackageCardiovascular=caseOutput.AutomationPackageCardiovascular
					String lSalesforcePackageCardiovascular=caseOutput.('SalesforceVerifyDataQuoteLineItem'+lStrRoundPad+'DetailSumAssuredC')
					Float lAutomationPrice=IGNUemaHelper.convertStringToFloat(lAutomationPackageCardiovascular,0)
					Float lSalesforcePrice=IGNUemaHelper.convertStringToFloat(lSalesforcePackageCardiovascular,0)
					Float lPrice=Math.abs(lAutomationPrice-lSalesforcePrice)
					if(lPrice>1){
						lIsOK=false
						lText=IGNUemaHelper.concatMessageLeftRightByPipe('Package Cardiovascular='+lAutomationPackageCardiovascular,lSalesforcePackageCardiovascular)
						lSalesforceVerifyCompareDetail=lSalesforceVerifyCompareDetail+lText+'\n'
					}
				}
				if(IGNUemaHelper.checkStringContainString(lQuoteLineItem,'CANCERCI')){
					String lAutomationPackageCancer=caseOutput.AutomationPackageCancer
					String lSalesforcePackageCancer=caseOutput.('SalesforceVerifyDataQuoteLineItem'+lStrRoundPad+'DetailSumAssuredC')
					Float lAutomationPrice=IGNUemaHelper.convertStringToFloat(lAutomationPackageCancer,0)
					Float lSalesforcePrice=IGNUemaHelper.convertStringToFloat(lSalesforcePackageCancer,0)
					Float lPrice=Math.abs(lAutomationPrice-lSalesforcePrice)
					if(lPrice>1){
						lIsOK=false
						lText=IGNUemaHelper.concatMessageLeftRightByPipe('Package Cancer='+lAutomationPackageCancer,lSalesforcePackageCancer)
						lSalesforceVerifyCompareDetail=lSalesforceVerifyCompareDetail+lText+'\n'
					}
				}
				if(IGNUemaHelper.checkStringContainString(lQuoteLineItem,'HIB')){
					...
					if(lSalesforceHospitalIncomeBenefitPlan.length()<=0){
						lIsOK=false
	...
	public Map inputSalesforceProceedVerifyAfter(Map caseOutput){
		...
		try{
			...
			String lAutomationPackageOrganFailure=caseOutput.AutomationPackageOrganFailure
			String lSalesforcePackageOrganFailure=caseOutput.SalesforceRetrieveDataPackageOrganFailure
			if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationPackageOrganFailure,lSalesforcePackageOrganFailure)){
				lIsOK=false
				lText=IGNUemaHelper.concatMessageLeftRightByPipe('Package OrganFailure='+lAutomationPackageOrganFailure,lSalesforcePackageOrganFailure)
				lSalesforceRetrieveCompareDetail=lSalesforceRetrieveCompareDetail+lText+'\n'
			}
			String lAutomationPackageNeurological=caseOutput.AutomationPackageNeurological
			String lSalesforcePackageNeurological=caseOutput.SalesforceRetrieveDataPackageNeurological
			if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationPackageNeurological,lSalesforcePackageNeurological)){
				lIsOK=false
				lText=IGNUemaHelper.concatMessageLeftRightByPipe('Package Neurological='+lAutomationPackageNeurological,lSalesforcePackageNeurological)
				lSalesforceRetrieveCompareDetail=lSalesforceRetrieveCompareDetail+lText+'\n'
			}
			String lAutomationPackageCardiovascular=caseOutput.AutomationPackageCardiovascular
			String lSalesforcePackageCardiovascular=caseOutput.SalesforceRetrieveDataPackageCardiovascular
			if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationPackageCardiovascular,lSalesforcePackageCardiovascular)){
				lIsOK=false
				lText=IGNUemaHelper.concatMessageLeftRightByPipe('Package Cardiovascular='+lAutomationPackageCardiovascular,lSalesforcePackageCardiovascular)
				lSalesforceRetrieveCompareDetail=lSalesforceRetrieveCompareDetail+lText+'\n'
			}
			String lAutomationPackageCancer=caseOutput.AutomationPackageCancer
			String lSalesforcePackageCancer=caseOutput.SalesforceRetrieveDataPackageCancer
			if(!IGNUemaHelper.checkStringContainStringRemoveWhitespace(lAutomationPackageCancer,lSalesforcePackageCancer)){
				lIsOK=false
				lText=IGNUemaHelper.concatMessageLeftRightByPipe('Package Cancer='+lAutomationPackageCancer,lSalesforcePackageCancer)
				lSalesforceRetrieveCompareDetail=lSalesforceRetrieveCompareDetail+lText+'\n'
			}
	...
	public Map inputSalesforceProceedRetrieveTodo(String strSalesforceProceedRetrieveTodo,String strAccountNameAnonymous){
		...
		if(lSalesforceProceedRetrieveTodo.length()<=0){
			lSalesforceProceedRetrieveTodo=THACriticalIllnessQuotationResultType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
	...
	public Map inputActualResultA(Boolean isPositiveCase,Boolean isQuoteByNone){
		...
		try{
			...
			this.checkPaymentOptionCriticalIllnessSwitch()
			this.checkPriceChangeToYearlyPayment()
			String lTagPaymentOptionCancerSwitchButton='paymentOptions'
			String lLocatorPaymentOptionCancerSwitchButton=IGNUemaHelper.getTagDataSeleniumKey(lTagPaymentOptionCancerSwitchButton)
			WebElement lElementPaymentOptionCancerSwitchButton=IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,lTagPaymentOptionCancerSwitchButton)
	...
	public Map inputQuoteCallWithTypeA(String strQuoteCallWithType){
		...
		try{
			...
			if(!lQuoteCallWithTypeItem){
				lQuoteCallWithTypeItem=lQuoteCallWithTypeList.get(THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_CALL_WITH_TYPE)
			}
	...
	public Map inputQuoteCallWithWhenA(String strQuoteCallWithWhen){
		...
		try{
			...
			if(!lQuoteCallWithWhenItem){
				lQuoteCallWithWhenItem=lQuoteCallWithWhenList.get(THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_CALL_WITH_WHEN)
			}
	...
	public Map inputQuoteCallWithTimeA(String strQuoteCallWithTime){
		...
		try{
			...
			if(!lQuoteCallWithTimeItem){
				lQuoteCallWithTimeItem=THACriticalIllnessQuotationResultType.DEFAULT_QUOTE_CALL_WITH_TIME
			}
