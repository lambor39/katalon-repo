...
import org.roojai.tha.types.electricmotorcar.THAElectricMotorCarQuotationSocialUtil
import org.roojai.tha.types.electricmotorcar.THAElectricMotorCarQuotationSocialUtilOld
public class THARGPAElectricMotorCarQuotationSocialUtil{
	private WebDriver driver=null
	private WebDriver selenium=null
	private def lTHAElectricMotorCarQuotationSocialUtil=null
	...
	public THARGPAElectricMotorCarQuotationSocialUtil(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	private THAElectricMotorCarQuotationSocialUtil getlTHAElectricMotorCarQuotationSocialUtil(){
		return this.lTHAElectricMotorCarQuotationSocialUtil
	}
	private void setlTHAElectricMotorCarQuotationSocialUtil(THAElectricMotorCarQuotationSocialUtil newlTHAElectricMotorCarQuotationSocialUtil){
		//Do Nothing
		//this.lTHAElectricMotorCarQuotationSocialUtil=newlTHAElectricMotorCarQuotationSocialUtil
	}
	public Boolean inputSocial(Map caseInput,Map caseOutput){
		...
		try{
			...
			if(THARGPAElectricMotorCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION){
				this.lTHAElectricMotorCarQuotationSocialUtil=new THAElectricMotorCarQuotationSocialUtil(this.driver,this.selenium)
			}else{
				this.lTHAElectricMotorCarQuotationSocialUtil=new THAElectricMotorCarQuotationSocialUtilOld(this.driver,this.selenium)
			}
			...
			if(!lIsPaymentPromptPayPageVisible){
				...
				String lPaymentMethod=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_METHOD
				String lQrSuccessCase=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_QR_CODE_SUCCESS_CASE
				String lTrueMoneyMobileNo=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_MOBILE_NO
				String lTrueMoneyEmail=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_EMAIL
				String lTrueMoneySuccessCase=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_SUCCESS_CASE
				String lRabbitLinePayMobileNo=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_MOBILE_NO
				String lRabbitLinePayEmail=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_EMAIL
				String lRabbitLinePaySuccessCase=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_SUCCESS_CASE
				String lMobileBankingCard=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_CARD
				String lMobileBankingSuccessCase=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_SUCCESS_CASE
				...
				if(lIsSalesforceProceedTodo){
					...
					if(lIsActualResult){
						...
						if(lIsSalesforceProceedRetrieveTodo){
							Map lMapSalesforceProceedRetrieveWebFromUrl=this.inputSalesforceProceedRetrieveWebFromUrl(THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_SOCIAL)
	...
	public Map inputLanguage(String strLanguage){
		...
		if(lLanguage.length()<=0){
			lLanguage=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_LANGUAGE
		}
		try{
			Boolean lResult=false
			lreturn.put('Language','')
			lreturn.put('Result',lResult)
			Map lMapLanguage=this.lTHAElectricMotorCarQuotationSocialUtil.inputLanguage(lLanguage)
			if(!lMapLanguage.Result){
				lLanguage=''
			}else{
				lLanguage=lMapLanguage.Language
	...
	public Map inputFeefoRating(String strPolicyInsurer,String strRatingForOverall,String strRatingForWebsite,String strRatingForCallCenter,String strRatingForProduct,String strRatingText,String strWithFacebook,String strFacebookLoginId,String strFacebookLoginPassword,String strShareFacebook){
		...
		if(lRatingForOverall.length()<=0){
			lRatingForOverall=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_RATING_FOR_OVERALL
		}
		String lRatingForWebsite=strRatingForWebsite.trim()
		if(lRatingForWebsite.length()<=0){
			lRatingForWebsite=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_RATING_FOR_WEBSITE
		}
		String lRatingForCallCenter=strRatingForCallCenter.trim()
		if(lRatingForCallCenter.length()<=0){
			lRatingForCallCenter=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_RATING_FOR_CALL_CENTER
		}
		String lRatingForProduct=strRatingForProduct.trim()
		if(lRatingForProduct.length()<=0){
			lRatingForProduct=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_RATING_FOR_PRODUCT
		}
		String lRatingText=strRatingText.trim()
		if(lRatingText.length()<=0){
			lRatingText=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_RATING_TEXT
		}
		String lWithFacebook=strWithFacebook.trim()
		if(lWithFacebook.length()<=0){
			lWithFacebook=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_WITH_FACEBOOK
		}
		String lFacebookLoginId=strFacebookLoginId.trim()
		if(lFacebookLoginId.length()<=0){
			lFacebookLoginId=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_FACEBOOK_LOGIN_ID
		}
		String lFacebookLoginPassword=strFacebookLoginPassword.trim()
		if(lFacebookLoginPassword.length()<=0){
			lFacebookLoginPassword=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_FACEBOOK_LOGIN_PASSWORD
		}
		String lShareFacebook=strShareFacebook.trim()
		if(lShareFacebook.length()<=0){
			lShareFacebook=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_SHARE_FACEBOOK
		}
					...
					if(lIsFoundFeefoWindow){
						...
						if(IGNUemaHelper.waitElementVisibleByLocator(this.driver,lLocatorRatingForOverall,30)){
							...
							if(lElementRatingForOverall){
								...
								if(THARGPAElectricMotorCarQuotationDetail2Type.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail2Output)){
									String lOpportunityNumber=lCaseDetail2Output.get('OpportunityNumber').toString()
									lRatingText=lOpportunityNumber+'|'+lRatingText
								}
								if(THARGPAElectricMotorCarCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
	...
	public Map inputPaymentByQR(String strQrSuccessCase){
		...
		if(lQrSuccessCase.length()<=0){
			lQrSuccessCase=THAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_QR_CODE_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public Map inputPaymentByTrueMoney(String strTrueMoneyMobileNo,String strTrueMoneyEmail,String strTrueMoneySuccessCase){
		...
		if(lTrueMoneyMobileNo.length()<=0){
			lTrueMoneyMobileNo=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_MOBILE_NO
		}
		String lTrueMoneyEmail=strTrueMoneyEmail.trim()
		if(lTrueMoneyEmail.length()<=0){
			lTrueMoneyEmail=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_EMAIL
		}
		String lTrueMoneySuccessCase=strTrueMoneySuccessCase.trim()
		if(lTrueMoneySuccessCase.length()<=0){
			lTrueMoneySuccessCase=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public Map inputPaymentByRabbitLinePay(String strRabbitLinePayMobileNo,String strRabbitLinePayEmail,String strRabbitLinePaySuccessCase){
		...
		if(lRabbitLinePayMobileNo.length()<=0){
			lRabbitLinePayMobileNo=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_MOBILE_NO
		}
		String lRabbitLinePayEmail=strRabbitLinePayEmail.trim()
		if(lRabbitLinePayEmail.length()<=0){
			lRabbitLinePayEmail=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_EMAIL
		}
		String lRabbitLinePaySuccessCase=strRabbitLinePaySuccessCase.trim()
		if(lRabbitLinePaySuccessCase.length()<=0){
			lRabbitLinePaySuccessCase=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public Map inputPaymentByMobileBanking(String strMobileBankingCard,String strMobileBankingSuccessCase){
		...
		if(lMobileBankingCard.length()<=0){
			lMobileBankingCard=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_CARD
		}
		String lMobileBankingSuccessCase=strMobileBankingSuccessCase.trim()
		if(lMobileBankingSuccessCase.length()<=0){
			lMobileBankingSuccessCase=THARGPAElectricMotorCarCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_SUCCESS_CASE
		}
		String lOpportunityNumber=''
		String lAmtTotalPrice=''
		try{
	...
	public String inputPositiveCase(String strPositiveCase){
		...
		try{
			String lText=''
			if(lPositiveCase.length()<=0){
				lPositiveCase=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_POSITIVE_CASE
			}
			Boolean lIsPositiveCase=IGNUemaHelper.convertStringToBoolean(lPositiveCase)
	...
	public Map inputPolicyInsurer(String strPolicyInsurer){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapPolicyInsurer=this.lTHAElectricMotorCarQuotationSocialUtil.inputPolicyInsurer(lPolicyInsurer)
			if(!lMapPolicyInsurer.Result){
				lPolicyInsurer=''
			}else{
				lPolicyInsurer=lMapPolicyInsurer.PolicyInsurer
	...
	public Map inputSalesforceProceedTodo(String strSalesforceProceedTodo){
		...
		if(lSalesforceProceedTodo.length()<=0){
			lSalesforceProceedTodo=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
			lreturn.put('SalesforceProceedTodo','')
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedTodo=this.lTHAElectricMotorCarQuotationSocialUtil.inputSalesforceProceedTodo(lSalesforceProceedTodo)
			if(!lMapSalesforceProceedTodo.Result){
				lSalesforceProceedTodo=''
			}else{
				lSalesforceProceedTodo=lMapSalesforceProceedTodo.SalesforceProceedTodo
	...
	public Map inputSalesforceProceedVerifyBefore(Map caseOutput){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedVerifyBefore=this.lTHAElectricMotorCarQuotationSocialUtil.inputSalesforceProceedVerifyBefore(caseOutput)
	...
	public Map inputSalesforceProceedVerifyAfter(Map caseOutput){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedVerifyAfter=this.lTHAElectricMotorCarQuotationSocialUtil.inputSalesforceProceedVerifyAfter(caseOutput)
	...
	public Map inputSalesforceProceedRetrieveTodo(String strSalesforceProceedRetrieveTodo,String strAccountNameAnonymous){
		...
		if(lSalesforceProceedRetrieveTodo.length()<=0){
			lSalesforceProceedRetrieveTodo=THARGPAElectricMotorCarQuotationSocialType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
			lreturn.put('SalesforceProceedRetrieveTodo','')
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedRetrieveTodo=this.lTHAElectricMotorCarQuotationSocialUtil.inputSalesforceProceedRetrieveTodo(lSalesforceProceedRetrieveTodo,strAccountNameAnonymous)
			if(!lMapSalesforceProceedRetrieveTodo.Result){
				lSalesforceProceedRetrieveTodo=''
			}else{
				lSalesforceProceedRetrieveTodo=lMapSalesforceProceedRetrieveTodo.SalesforceProceedRetrieveTodo
	...
	public Map inputSalesforceProceedRetrieveWebFromUrl(String strRetrievePageURL){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedRetrieveWebFromUrl=this.lTHAElectricMotorCarQuotationSocialUtil.inputSalesforceProceedRetrieveWebFromUrl(lRetrievePageURL)
