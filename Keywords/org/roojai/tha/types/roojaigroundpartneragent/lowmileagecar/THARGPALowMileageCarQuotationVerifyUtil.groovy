...
public class THARGPALowMileageCarQuotationVerifyUtil{
	private WebDriver driver=null
	private WebDriver selenium=null
	public THARGPALowMileageCarQuotationVerifyUtil(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	public Boolean inputVerify(Map caseInput,Map caseOutput){
		...
		try{
			...
			lReadOK=THARGPAMotorQuotationInputType.readInputFromDataFile(GlobalVariable.CaseNumber,lCaseInputInput)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THARGPAMotorQuotationInputType.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseInputOutput)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
			...
			lReadOK=THARGPAMotorQuotationResultType.readInputFromDataFile(GlobalVariable.CaseNumber,lCaseResultInput)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THARGPAMotorQuotationResultType.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseResultOutput)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
			...
			lReadOK=THARGPAMotorQuotationDetail1Type.readInputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail1Input)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THARGPAMotorQuotationDetail1Type.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail1Output)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
			...
			lReadOK=THARGPAMotorQuotationPaymentType.readInputFromDataFile(GlobalVariable.CaseNumber,lCasePaymentInput)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THARGPAMotorQuotationPaymentType.readOutputFromDataFile(GlobalVariable.CaseNumber,lCasePaymentOutput)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
			...
			lReadOK=THARGPAMotorQuotationDetail2Type.readInputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail2Input)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THARGPAMotorQuotationDetail2Type.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail2Output)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
			...
			lReadOK=THARGPAMotorQuotationSocialType.readInputFromDataFile(GlobalVariable.CaseNumber,lCaseSocialInput)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THARGPAMotorQuotationSocialType.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseSocialOutput)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
