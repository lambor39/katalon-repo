...
import org.roojai.tha.types.motorbike.THAMotorBikeQuotationPaymentUtil
import org.roojai.tha.types.motorbike.THAMotorBikeQuotationPaymentUtilOld
...
public class THARGPAMotorQuotationPaymentUtil{
	private WebDriver driver=null
	private WebDriver selenium=null
	private def lTHAMotorBikeQuotationPaymentUtil=null
	private def lTHAMotorCarQuotationPaymentUtil=null
	private Boolean lIsSalesforceProceedRetrieveWebFromUrlSuccess=false
	public THARGPAMotorQuotationPaymentUtil(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	private THAMotorBikeQuotationPaymentUtil getlTHAMotorBikeQuotationPaymentUtil(){
		return this.lTHAMotorBikeQuotationPaymentUtil
	}
	private void setlTHAMotorBikeQuotationPaymentUtil(THAMotorBikeQuotationPaymentUtil newlTHAMotorBikeQuotationPaymentUtil){
		//Do Nothing
		//this.lTHAMotorBikeQuotationPaymentUtil=newlTHAMotorBikeQuotationPaymentUtil
	}
	private THAMotorCarQuotationPaymentUtil getlTHAMotorCarQuotationPaymentUtil(){
		return this.lTHAMotorCarQuotationPaymentUtil
	}
	private void setlTHAMotorCarQuotationPaymentUtil(THAMotorCarQuotationPaymentUtil newlTHAMotorCarQuotationPaymentUtil){
		//Do Nothing
		//this.lTHAMotorCarQuotationPaymentUtil=newlTHAMotorCarQuotationPaymentUtil
	}
		...
		try{
			...
			if(THARGPAMotorCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION){
				if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
					this.lTHAMotorBikeQuotationPaymentUtil=new THAMotorBikeQuotationPaymentUtil(this.driver,this.selenium)
				}else{
					this.lTHAMotorCarQuotationPaymentUtil=new THAMotorCarQuotationPaymentUtil(this.driver,this.selenium)
				}
			}else{
				if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
					this.lTHAMotorBikeQuotationPaymentUtil=new THAMotorBikeQuotationPaymentUtilOld(this.driver,this.selenium)
				}else{
					this.lTHAMotorCarQuotationPaymentUtil=new THAMotorCarQuotationPaymentUtilOld(this.driver,this.selenium)
				}
			}
			...
			if(lIsSalesforceProceedTodo){
				...
				if(lIsActualResult){
					...
					if(lIsSalesforceProceedRetrieveTodo){
						Map lMapSalesforceProceedRetrieveWebFromUrl=this.inputSalesforceProceedRetrieveWebFromUrl(THARGPAMotorCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_PAYMENT)
			...
			THARGPAMotorCoreType.CURRENT_QUOTATION_PAYMENT_METHOD=lPaymentMethod
			...
			switch(lPaymentMethod){
				...
				case '3':
					...
					THARGPAMotorCoreType.CURRENT_QUOTATION_PAYMENT_QR_CODE_SUCCESS_CASE=lQrSuccessCase
					if(!lMapPaymentByQR.Result){
						return lreturn
					}
					break
				case '4':
					...
					THARGPAMotorCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_MOBILE_NO=lTrueMoneyMobileNo
					THARGPAMotorCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_EMAIL=lTrueMoneyEmail
					THARGPAMotorCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_SUCCESS_CASE=lTrueMoneySuccessCase
					if(!lMapPaymentByTrueMoney.Result){
						return lreturn
					}
					break
				case '5':
					...
					THARGPAMotorCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_MOBILE_NO=lRabbitLinePayMobileNo
					THARGPAMotorCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_EMAIL=lRabbitLinePayEmail
					THARGPAMotorCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_SUCCESS_CASE=lRabbitLinePaySuccessCase
					if(!lMapPaymentByRabbitLinePay.Result){
						return lreturn
					}
					break
				case '6':
					...
					THARGPAMotorCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_CARD=lMobileBankingCard
					THARGPAMotorCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_SUCCESS_CASE=lMobileBankingSuccessCase
					if(!lMapPaymentByMobileBanking.Result){
						return lreturn
					}
					break
	...
	public Map inputPositiveCase(String strPositiveCase){
		...
		if(lPositiveCase.length()<=0){
			lPositiveCase=THARGPAMotorQuotationPaymentType.DEFAULT_POSITIVE_CASE
		}
		try{
	...
	public Map inputActualResult(Boolean isPositiveCase){
		...
		try{
			...
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapActualResult=this.lTHAMotorBikeQuotationPaymentUtil.inputActualResult(isPositiveCase)
				if(!lMapActualResult.Result){
					lActualResult=''
				}else{
					lActualResult=lMapActualResult.ActualResult
				...
				Map lMapActualResult=this.lTHAMotorCarQuotationPaymentUtil.inputActualResult(isPositiveCase)
				if(!lMapActualResult.Result){
					lActualResult=''
				}else{
					lActualResult=lMapActualResult.ActualResult
	...
	public Map inputLanguage(String strLanguage){
		...
		if(lLanguage.length()<=0){
			lLanguage=THARGPAMotorQuotationPaymentType.DEFAULT_LANGUAGE
		}
		try{
			Boolean lResult=false
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapLanguage=this.lTHAMotorBikeQuotationPaymentUtil.inputLanguage(lLanguage)
				if(!lMapLanguage.Result){
					lLanguage=''
				}else{
					lLanguage=lMapLanguage.Language
				...
				Map lMapLanguage=this.lTHAMotorCarQuotationPaymentUtil.inputLanguage(lLanguage)
				if(!lMapLanguage.Result){
					lLanguage=''
				}else{
					lLanguage=lMapLanguage.Language
	...
	public Map inputOpportunityNumber(){
		...
		try{
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapOpportunityNumber=this.lTHAMotorBikeQuotationPaymentUtil.inputOpportunityNumber()
				if(!lMapOpportunityNumber.Result){
					lOpportunityNumber=''
				}else{
					lOpportunityNumber=lMapOpportunityNumber.OpportunityNumber
				...
				Map lMapOpportunityNumber=this.lTHAMotorCarQuotationPaymentUtil.inputOpportunityNumber()
				if(!lMapOpportunityNumber.Result){
					lOpportunityNumber=''
				}else{
					lOpportunityNumber=lMapOpportunityNumber.OpportunityNumber
	...
	public Map inputPaymentMethod(String strPaymentMethod){
		...
		if(lPaymentMethod.length()<=0){
			lPaymentMethod=THARGPAMotorQuotationPaymentType.DEFAULT_PAYMENT_METHOD
		}
		try{
			Boolean lResult=false
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapPaymentMethod=this.lTHAMotorBikeQuotationPaymentUtil.inputPaymentMethod(lPaymentMethod)
				if(!lMapPaymentMethod.Result){
					lPaymentMethod=''
				}else{
					lPaymentMethod=lMapPaymentMethod.PaymentMethod
				...
				Map lMapPaymentMethod=this.lTHAMotorCarQuotationPaymentUtil.inputPaymentMethod(lPaymentMethod)
				if(!lMapPaymentMethod.Result){
					lPaymentMethod=''
				}else{
					lPaymentMethod=lMapPaymentMethod.PaymentMethod
	...
	public Map inputPaymentByQR(String strQrIsYearly,String strQrInstalment,String strEPolicy,String strQrSuccessCase){
		...
		if(lQrIsYearly.length()<=0){
			lQrIsYearly=THARGPAMotorQuotationPaymentType.DEFAULT_QR_IS_YEARLY
		}
		String lQrInstalment=strQrInstalment.trim()
		if(lQrInstalment.length()<=0){
			lQrInstalment=THARGPAMotorQuotationPaymentType.DEFAULT_QR_INSTALMENT
		}
		String lEPolicy=strEPolicy.trim()
		if(lEPolicy.length()<=0){
			lEPolicy=THARGPAMotorQuotationPaymentType.DEFAULT_E_POLICY
		}
		String lQrSuccessCase=strQrSuccessCase.trim()
		if(lQrSuccessCase.length()<=0){
			lQrSuccessCase=THARGPAMotorQuotationPaymentType.DEFAULT_QR_SUCCESS_CASE
		}
		String lQrAmtYearly=''
		String lQrAmtDeposit=''
		String lQrAmtMonthly=''
		...
		try{
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapPaymentByQR=this.lTHAMotorBikeQuotationPaymentUtil.inputPaymentByQR(lQrIsYearly,lQrInstalment,lEPolicy,lQrSuccessCase)
				if(!lMapPaymentByQR.Result){
					lQrIsYearly=''
					lQrInstalment=''
					lEPolicy=''
				...
				Map lMapPaymentByQR=this.lTHAMotorCarQuotationPaymentUtil.inputPaymentByQR(lQrIsYearly,lQrInstalment,lEPolicy,lQrSuccessCase)
				if(!lMapPaymentByQR.Result){
					lQrIsYearly=''
					lQrInstalment=''
					lEPolicy=''
	...
	public Map inputPaymentByIB(String strIbBank,String strEPolicy,String strAutoRenew,String strIbSuccessCase){
		...
		if(lIbBank.length()<=0){
			lIbBank=THARGPAMotorQuotationPaymentType.DEFAULT_IB_BANK.toUpperCase()
		}
		String lEPolicy=strEPolicy.trim()
		if(lEPolicy.length()<=0){
			lEPolicy=THARGPAMotorQuotationPaymentType.DEFAULT_E_POLICY
		...
		if(lAutoRenew.length()<=0){
			lAutoRenew=THARGPAMotorQuotationPaymentType.DEFAULT_AUTO_RENEW
		}
		String lIbSuccessCase=strIbSuccessCase.trim()
		if(lIbSuccessCase.length()<=0){
			lIbSuccessCase=THARGPAMotorQuotationPaymentType.DEFAULT_IB_SUCCESS_CASE
		}
		String lIbAmtYearly=''
		try{
			Boolean lResult=false
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapPaymentByIB=this.lTHAMotorBikeQuotationPaymentUtil.inputPaymentByIB(lIbBank,lEPolicy,lAutoRenew,lIbSuccessCase)
				if(!lMapPaymentByIB.Result){
					lIbBank=''
					lEPolicy=''
					lAutoRenew=''
				...
				Map lMapPaymentByIB=this.lTHAMotorCarQuotationPaymentUtil.inputPaymentByIB(lIbBank,lEPolicy,lAutoRenew,lIbSuccessCase)
				if(!lMapPaymentByIB.Result){
					lIbBank=''
					lEPolicy=''
					lAutoRenew=''
	...
	public Map inputPaymentByCC(String strCcIsYearly,String strCcInstalment,String strCcNumber,String strCcName,String strCcExpiryMonth,String strCcExpiryYear,String strCcCvv,String strCcBank,String strEPolicy,String strAutoRenew,String strCcSuccessCase){
		...
		if(lCcIsYearly.length()<=0){
			lCcIsYearly=THARGPAMotorQuotationPaymentType.DEFAULT_CC_IS_YEARLY
		}
		String lCcInstalment=strCcInstalment.trim()
		if(lCcInstalment.length()<=0){
			lCcInstalment=THARGPAMotorQuotationPaymentType.DEFAULT_CC_INSTALMENT
		}
		String lCcNumber=strCcNumber.trim()
		if(lCcNumber.length()<=0){
			lCcNumber=THARGPAMotorQuotationPaymentType.DEFAULT_CC_NUMBER
		}
		String lCcName=strCcName.trim()
		if(lCcName.length()<=0){
			lCcName=THARGPAMotorQuotationPaymentType.DEFAULT_CC_NAME
		}
		String lCcExpiryMonth=strCcExpiryMonth.trim()
		if(lCcExpiryMonth.length()<=0){
			lCcExpiryMonth=THARGPAMotorQuotationPaymentType.DEFAULT_CC_EXPIRY_MONTH
		}
		String lCcExpiryYear=strCcExpiryYear.trim()
		if(lCcExpiryYear.length()<=0){
			lCcExpiryYear=THARGPAMotorQuotationPaymentType.DEFAULT_CC_EXPIRY_YEAR
		}
		String lCcCvv=strCcCvv.trim()
		if(lCcCvv.length()<=0){
			lCcCvv=THARGPAMotorQuotationPaymentType.DEFAULT_CC_CVV
		}
		String lCcBank=strCcBank.trim()
		if(lCcBank.length()<=0){
			lCcBank=THARGPAMotorQuotationPaymentType.DEFAULT_CC_BANK
		}
		String lEPolicy=strEPolicy.trim()
		if(lEPolicy.length()<=0){
			lEPolicy=THARGPAMotorQuotationPaymentType.DEFAULT_E_POLICY
		...
		if(lCcSuccessCase.length()<=0){
			lCcSuccessCase=THARGPAMotorQuotationPaymentType.DEFAULT_CC_SUCCESS_CASE
		}
		String lCcAmtYearly=''
		String lCcAmtDeposit=''
		String lCcAmtMonthly=''
		...
		try{
			...
			if(this.lIsSalesforceProceedRetrieveWebFromUrlSuccess){
				THARGPAMotorCoreType.CURRENT_SKIP_PAGE_DETAIL2=true
			}
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapPaymentByCC=this.lTHAMotorBikeQuotationPaymentUtil.inputPaymentByCC(lCcIsYearly,lCcInstalment,lCcNumber,lCcName,lCcExpiryMonth,lCcExpiryYear,lCcCvv,lCcBank,lEPolicy,lAutoRenew,lCcSuccessCase)
				if(!lMapPaymentByCC.Result){
					lCcIsYearly=''
					lCcInstalment=''
					lCcNumber=''
				...
				Map lMapPaymentByCC=this.lTHAMotorCarQuotationPaymentUtil.inputPaymentByCC(lCcIsYearly,lCcInstalment,lCcNumber,lCcName,lCcExpiryMonth,lCcExpiryYear,lCcCvv,lCcBank,lEPolicy,lAutoRenew,lCcSuccessCase)
				if(!lMapPaymentByCC.Result){
					lCcIsYearly=''
					lCcInstalment=''
					lCcNumber=''
	...
	public Map inputPaymentByTrueMoney(String strTrueMoneyIsYearly,String strTrueMoneyInstalment,String strTrueMoneyMobileNo,String strTrueMoneyEmail,String strEPolicy,String strAutoRenew,String strTrueMoneySuccessCase){
		...
		if(lTrueMoneyIsYearly.length()<=0){
			lTrueMoneyIsYearly=THARGPAMotorQuotationPaymentType.DEFAULT_TRUE_MONEY_IS_YEARLY
		}
		String lTrueMoneyInstalment=strTrueMoneyInstalment.trim()
		if(lTrueMoneyInstalment.length()<=0){
			lTrueMoneyInstalment=THARGPAMotorQuotationPaymentType.DEFAULT_TRUE_MONEY_INSTALMENT
		}
		String lTrueMoneyMobileNo=strTrueMoneyMobileNo.trim()
		if(lTrueMoneyMobileNo.length()<=0){
			lTrueMoneyMobileNo=THARGPAMotorQuotationPaymentType.DEFAULT_TRUE_MONEY_MOBILE_NO
		}
		String lTrueMoneyEmail=strTrueMoneyEmail.trim()
		if(lTrueMoneyEmail.length()<=0){
			lTrueMoneyEmail=THARGPAMotorQuotationPaymentType.DEFAULT_TRUE_MONEY_EMAIL
		}
		String lEPolicy=strEPolicy.trim()
		if(lEPolicy.length()<=0){
			lEPolicy=THARGPAMotorQuotationPaymentType.DEFAULT_E_POLICY
		...
		if(lTrueMoneySuccessCase.length()<=0){
			lTrueMoneySuccessCase=THARGPAMotorQuotationPaymentType.DEFAULT_TRUE_MONEY_SUCCESS_CASE
		}
		String lTrueMoneyAmtYearly=''
		String lTrueMoneyAmtDeposit=''
		String lTrueMoneyAmtMonthly=''
		...
		try{
			...
			/*************************************************************************************************************************************************/
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapPaymentByTrueMoney=this.lTHAMotorBikeQuotationPaymentUtil.inputPaymentByTrueMoney(lTrueMoneyIsYearly,lTrueMoneyInstalment,lTrueMoneyMobileNo,lTrueMoneyEmail,lEPolicy,lAutoRenew,lTrueMoneySuccessCase)
				if(!lMapPaymentByTrueMoney.Result){
					lTrueMoneyIsYearly=''
					lTrueMoneyInstalment=''
					lTrueMoneyMobileNo=''
				...
				Map lMapPaymentByTrueMoney=this.lTHAMotorCarQuotationPaymentUtil.inputPaymentByTrueMoney(lTrueMoneyIsYearly,lTrueMoneyInstalment,lTrueMoneyMobileNo,lTrueMoneyEmail,lEPolicy,lAutoRenew,lTrueMoneySuccessCase)
				if(!lMapPaymentByTrueMoney.Result){
					lTrueMoneyIsYearly=''
					lTrueMoneyInstalment=''
					lTrueMoneyMobileNo=''
	...
	public Map inputPaymentByRabbitLinePay(String strRabbitLinePayIsYearly,String strRabbitLinePayInstalment,String strRabbitLinePayMobileNo,String strRabbitLinePayEmail,String strEPolicy,String strAutoRenew,String strRabbitLinePaySuccessCase){
		...
		if(lRabbitLinePayIsYearly.length()<=0){
			lRabbitLinePayIsYearly=THARGPAMotorQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_IS_YEARLY
		}
		String lRabbitLinePayInstalment=strRabbitLinePayInstalment.trim()
		if(lRabbitLinePayInstalment.length()<=0){
			lRabbitLinePayInstalment=THARGPAMotorQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_INSTALMENT
		}
		String lRabbitLinePayMobileNo=strRabbitLinePayMobileNo.trim()
		if(lRabbitLinePayMobileNo.length()<=0){
			lRabbitLinePayMobileNo=THARGPAMotorQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_MOBILE_NO
		}
		String lRabbitLinePayEmail=strRabbitLinePayEmail.trim()
		if(lRabbitLinePayEmail.length()<=0){
			lRabbitLinePayEmail=THARGPAMotorQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_EMAIL
		}
		String lEPolicy=strEPolicy.trim()
		if(lEPolicy.length()<=0){
			lEPolicy=THARGPAMotorQuotationPaymentType.DEFAULT_E_POLICY
		...
		if(lRabbitLinePaySuccessCase.length()<=0){
			lRabbitLinePaySuccessCase=THARGPAMotorQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_SUCCESS_CASE
		}
		String lRabbitLinePayAmtYearly=''
		String lRabbitLinePayAmtDeposit=''
		String lRabbitLinePayAmtMonthly=''
		...
		try{
			...
			/*************************************************************************************************************************************************/
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapPaymentByRabbitLinePay=this.lTHAMotorBikeQuotationPaymentUtil.inputPaymentByRabbitLinePay(lRabbitLinePayIsYearly,lRabbitLinePayInstalment,lRabbitLinePayMobileNo,lRabbitLinePayEmail,lEPolicy,lAutoRenew,lRabbitLinePaySuccessCase)
				if(!lMapPaymentByRabbitLinePay.Result){
					lRabbitLinePayIsYearly=''
					lRabbitLinePayInstalment=''
					lRabbitLinePayMobileNo=''
				...
				Map lMapPaymentByRabbitLinePay=this.lTHAMotorCarQuotationPaymentUtil.inputPaymentByRabbitLinePay(lRabbitLinePayIsYearly,lRabbitLinePayInstalment,lRabbitLinePayMobileNo,lRabbitLinePayEmail,lEPolicy,lAutoRenew,lRabbitLinePaySuccessCase)
				if(!lMapPaymentByRabbitLinePay.Result){
					lRabbitLinePayIsYearly=''
					lRabbitLinePayInstalment=''
					lRabbitLinePayMobileNo=''
	...
	public Map inputPaymentByMobileBanking(String strMobileBankingIsYearly,String strMobileBankingInstalment,String strMobileBankingCard,String strEPolicy,String strAutoRenew,String strMobileBankingSuccessCase){
		...
		if(lMobileBankingIsYearly.length()<=0){
			lMobileBankingIsYearly=THARGPAMotorQuotationPaymentType.DEFAULT_MOBILE_BANKING_IS_YEARLY
		}
		String lMobileBankingInstalment=strMobileBankingInstalment.trim()
		if(lMobileBankingInstalment.length()<=0){
			lMobileBankingInstalment=THARGPAMotorQuotationPaymentType.DEFAULT_MOBILE_BANKING_INSTALMENT
		}
		String lMobileBankingCard=strMobileBankingCard.trim()
		if(lMobileBankingCard.length()<=0){
			lMobileBankingCard=THARGPAMotorQuotationPaymentType.DEFAULT_MOBILE_BANKING_CARD
		}
		String lEPolicy=strEPolicy.trim()
		if(lEPolicy.length()<=0){
			lEPolicy=THARGPAMotorQuotationPaymentType.DEFAULT_E_POLICY
		...
		if(lMobileBankingSuccessCase.length()<=0){
			lMobileBankingSuccessCase=THARGPAMotorQuotationPaymentType.DEFAULT_MOBILE_BANKING_SUCCESS_CASE
		}
		String lMobileBankingAmtYearly=''
		String lMobileBankingAmtDeposit=''
		String lMobileBankingAmtMonthly=''
		...
		try{
			...
			/*************************************************************************************************************************************************/
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapPaymentByMobileBanking=this.lTHAMotorBikeQuotationPaymentUtil.inputPaymentByMobileBanking(lMobileBankingIsYearly,lMobileBankingInstalment,lMobileBankingCard,lEPolicy,lAutoRenew,lMobileBankingSuccessCase)
				if(!lMapPaymentByMobileBanking.Result){
					lMobileBankingIsYearly=''
					lMobileBankingInstalment=''
					lMobileBankingCard=''
				...
				Map lMapPaymentByMobileBanking=this.lTHAMotorCarQuotationPaymentUtil.inputPaymentByMobileBanking(lMobileBankingIsYearly,lMobileBankingInstalment,lMobileBankingCard,lEPolicy,lAutoRenew,lMobileBankingSuccessCase)
				if(!lMapPaymentByMobileBanking.Result){
					lMobileBankingIsYearly=''
					lMobileBankingInstalment=''
					lMobileBankingCard=''
	...
	public Map inputPolicyInsurer(String strPolicyInsurer){
		...
		try{
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapPolicyInsurer=this.lTHAMotorBikeQuotationPaymentUtil.inputPolicyInsurer(lPolicyInsurer)
				if(!lMapPolicyInsurer.Result){
					lPolicyInsurer=''
				}else{
					lPolicyInsurer=lMapPolicyInsurer.PolicyInsurer
				...
				Map lMapPolicyInsurer=this.lTHAMotorCarQuotationPaymentUtil.inputPolicyInsurer(lPolicyInsurer)
				if(!lMapPolicyInsurer.Result){
					lPolicyInsurer=''
				}else{
					lPolicyInsurer=lMapPolicyInsurer.PolicyInsurer
	...
	public Map inputSalesforceProceedTodo(String strSalesforceProceedTodo){
		...
		if(lSalesforceProceedTodo.length()<=0){
			lSalesforceProceedTodo=THARGPAMotorQuotationPaymentType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapSalesforceProceedTodo=this.lTHAMotorBikeQuotationPaymentUtil.inputSalesforceProceedTodo(lSalesforceProceedTodo)
				if(!lMapSalesforceProceedTodo.Result){
					lSalesforceProceedTodo=''
				}else{
					lSalesforceProceedTodo=lMapSalesforceProceedTodo.SalesforceProceedTodo
				...
				Map lMapSalesforceProceedTodo=this.lTHAMotorCarQuotationPaymentUtil.inputSalesforceProceedTodo(lSalesforceProceedTodo)
				if(!lMapSalesforceProceedTodo.Result){
					lSalesforceProceedTodo=''
				}else{
					lSalesforceProceedTodo=lMapSalesforceProceedTodo.SalesforceProceedTodo
	...
	public Map inputAutomationDataBefore(){
		...
		try{
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapAutomationDataBefore=this.lTHAMotorBikeQuotationPaymentUtil.inputAutomationDataBefore()
				lResult=lMapAutomationDataBefore.Result
			}else{
				Map lMapAutomationDataBefore=this.lTHAMotorCarQuotationPaymentUtil.inputAutomationDataBefore()
				lResult=lMapAutomationDataBefore.Result
			}
			if(lResult){
				lreturn.put('Result',lResult)
	...
	public Map inputAutomationDataAfter(){
		...
		try{
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapAutomationDataAfter=this.lTHAMotorBikeQuotationPaymentUtil.inputAutomationDataAfter()
				if(!lMapAutomationDataAfter.Result){
					lSalesforceRetrieveDataOpportunityNumber=''
					lSalesforceRetrieveDataPremiumYearly=''
					lSalesforceRetrieveDataPremiumDeposit=''
				...
				Map lMapAutomationDataAfter=this.lTHAMotorCarQuotationPaymentUtil.inputAutomationDataAfter()
				if(!lMapAutomationDataAfter.Result){
					lSalesforceRetrieveDataOpportunityNumber=''
					lSalesforceRetrieveDataPremiumYearly=''
					lSalesforceRetrieveDataPremiumDeposit=''
	...
	public Map inputSalesforceProceedVerifyBefore(Map caseOutput){
		...
		try{
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapSalesforceProceedVerifyBefore=this.lTHAMotorBikeQuotationPaymentUtil.inputSalesforceProceedVerifyBefore(caseOutput)
				lResult=lMapSalesforceProceedVerifyBefore.Result
			}else{
				Map lMapSalesforceProceedVerifyBefore=this.lTHAMotorCarQuotationPaymentUtil.inputSalesforceProceedVerifyBefore(caseOutput)
				lResult=lMapSalesforceProceedVerifyBefore.Result
			}
			if(lResult){
				lreturn.put('Result',lResult)
	...
	public Map inputSalesforceProceedVerifyAfter(Map caseOutput){
		...
		try{
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapSalesforceProceedVerifyAfter=this.lTHAMotorBikeQuotationPaymentUtil.inputSalesforceProceedVerifyAfter(caseOutput)
				lResult=lMapSalesforceProceedVerifyAfter.Result
			}else{
				Map lMapSalesforceProceedVerifyAfter=this.lTHAMotorCarQuotationPaymentUtil.inputSalesforceProceedVerifyAfter(caseOutput)
				lResult=lMapSalesforceProceedVerifyAfter.Result
			}
			if(lResult){
				lreturn.put('Result',lResult)
	...
	public Map inputSalesforceProceedRetrieveTodo(String strSalesforceProceedRetrieveTodo,String strAccountNameAnonymous){
		...
		if(lSalesforceProceedRetrieveTodo.length()<=0){
			lSalesforceProceedRetrieveTodo=THARGPAMotorQuotationPaymentType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapSalesforceProceedRetrieveTodo=this.lTHAMotorBikeQuotationPaymentUtil.inputSalesforceProceedRetrieveTodo(lSalesforceProceedRetrieveTodo,strAccountNameAnonymous)
				if(!lMapSalesforceProceedRetrieveTodo.Result){
					lSalesforceProceedRetrieveTodo=''
				}else{
					lSalesforceProceedRetrieveTodo=lMapSalesforceProceedRetrieveTodo.SalesforceProceedRetrieveTodo
				...
				Map lMapSalesforceProceedRetrieveTodo=this.lTHAMotorCarQuotationPaymentUtil.inputSalesforceProceedRetrieveTodo(lSalesforceProceedRetrieveTodo,strAccountNameAnonymous)
				if(!lMapSalesforceProceedRetrieveTodo.Result){
					lSalesforceProceedRetrieveTodo=''
				}else{
					lSalesforceProceedRetrieveTodo=lMapSalesforceProceedRetrieveTodo.SalesforceProceedRetrieveTodo
	...
	public Map inputSalesforceProceedRetrieveWebFromUrl(String strRetrievePageURL){
		...
		try{
			...
			lreturn.put('Result',lResult)
			if(THARGPAMotorCoreType.CURRENT_PRODUCT_IS_MOTOR_BIKE){
				Map lMapSalesforceProceedRetrieveWebFromUrl=this.lTHAMotorBikeQuotationPaymentUtil.inputSalesforceProceedRetrieveWebFromUrl(lRetrievePageURL)
				lResult=lMapSalesforceProceedRetrieveWebFromUrl.Result
			}else{
				Map lMapSalesforceProceedRetrieveWebFromUrl=this.lTHAMotorCarQuotationPaymentUtil.inputSalesforceProceedRetrieveWebFromUrl(lRetrievePageURL)
				lResult=lMapSalesforceProceedRetrieveWebFromUrl.Result
			}
			if(lResult){
				lreturn.put('Result',lResult)
