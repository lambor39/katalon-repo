...
public class THARGPAMotorQuotationVerifyFactory{
	public Boolean IsValid=false
	private lCaseList=[]
	public Integer CurrentCase=0
	public THARGPAMotorQuotationVerifyFactory(TestData dataInput){
		super()
	...
	public Boolean validateDataInput(TestData dataInput){
		...
		try{
			...
			lreturn=(dataColNum>=THARGPAMotorQuotationVerifyType.INPUT_DATA_COL_BEGIN)&&(dataRowNum>=THARGPAMotorQuotationVerifyType.INPUT_DATA_ROW_END)
		}catch(Exception e){
			e.printStackTrace()
			return lreturn
		}
	...
	public void initCaseList(TestData dataInput){
		...
		try{
			...
			Integer dataColNum=dataInput.getColumnNumbers()
			Integer dataRowNum=dataInput.getRowNumbers()
			Integer dataColIndex=THARGPAMotorQuotationVerifyType.INPUT_DATA_COL_BEGIN
			Integer dataRowIndex=THARGPAMotorQuotationVerifyType.INPUT_DATA_ROW_BEGIN
			THARGPAMotorQuotationVerifyType dataCase
			Map caseInput=[:]
			caseInput.clear()
			String cellVal=''
			for(dataColIndex=THARGPAMotorQuotationVerifyType.INPUT_DATA_COL_BEGIN;dataColIndex<=dataColNum;dataColIndex++){
				dataCase=new THARGPAMotorQuotationVerifyType()
				dataCase.initInput()
				dataCase.initOutput()
				caseInput.clear()
				...
				dataRowIndex=THARGPAMotorQuotationVerifyType.INPUT_DATA_ROW_BEGIN
	...
	public THARGPAMotorQuotationVerifyType CaseData(){
	...
	public Boolean SaveOutput(){
		Boolean lreturn=false
		THARGPAMotorQuotationVerifyType caseData
		try{
			Integer dataColIndex=THARGPAMotorQuotationVerifyType.OUTPUT_EXCEL_COL_BEGIN+(this.CurrentCase-1)
			Integer dataRowIndex=THARGPAMotorQuotationVerifyType.OUTPUT_EXCEL_ROW_BEGIN
			if((this.CurrentCase>0)&&(this.lCaseList.size()>0)){
				caseData=this.lCaseList.get(this.CurrentCase-1)
				def xlsWorkbook=ExcelKeywords.getWorkbook(THARGPAMotorQuotationVerifyType.OUTPUT_EXCEL_FILE_NAME)
				def xlsSheet=ExcelKeywords.getExcelSheet(xlsWorkbook,THARGPAMotorQuotationVerifyType.OUTPUT_EXCEL_SHEET_NAME)
				...
				dataRowIndex+=1
				IGNUemaHelper.saveExcelWorkbookByFileName(THARGPAMotorQuotationVerifyType.OUTPUT_EXCEL_FILE_NAME,xlsWorkbook)
				lreturn=true
			}
		}catch(Exception e){
			e.printStackTrace()
