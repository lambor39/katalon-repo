...
import org.roojai.tha.types.roojaipersonalaccident.THAPersonalAccidentQuotationPaymentUtil
...
public class THARGPAPersonalAccidentQuotationPaymentUtil{
	private WebDriver driver=null
	private WebDriver selenium=null
	private THAPersonalAccidentQuotationPaymentUtil lTHAPersonalAccidentQuotationPaymentUtil=null
	public THARGPAPersonalAccidentQuotationPaymentUtil(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	private THAPersonalAccidentQuotationPaymentUtil getlTHAPersonalAccidentQuotationPaymentUtil(){
		return this.lTHAPersonalAccidentQuotationPaymentUtil
	}
	private void setlTHAPersonalAccidentQuotationPaymentUtil(THAPersonalAccidentQuotationPaymentUtil newlTHAPersonalAccidentQuotationPaymentUtil){
		//Do Nothing
		//this.lTHAPersonalAccidentQuotationPaymentUtil=newlTHAPersonalAccidentQuotationPaymentUtil
	}
	public Boolean inputPayment(Map caseInput,Map caseOutput){
		...
		try{
			...
			this.lTHAPersonalAccidentQuotationPaymentUtil=new THAPersonalAccidentQuotationPaymentUtil(this.driver,this.selenium)
			//Check ABTesting
			...
			this.lTHAPersonalAccidentQuotationPaymentUtil.waitUntilOpportunityNumberUpdated()
			...
			if(lIsActualResult){
				...
				if(lIsSalesforceProceedRetrieveTodo){
					Map lMapSalesforceProceedRetrieveWebFromUrl=this.inputSalesforceProceedRetrieveWebFromUrl(THARGPAPersonalAccidentCoreType.CURRENT_QUOTATION_RETRIEVAL_URL_PAGE_PAYMENT)
			...
			THARGPAPersonalAccidentCoreType.CURRENT_QUOTATION_PAYMENT_METHOD=lPaymentMethod
			...
			switch(lPaymentMethod){
				...
				case '4':
					...
					THARGPAPersonalAccidentCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_MOBILE_NO=lTrueMoneyMobileNo
					THARGPAPersonalAccidentCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_EMAIL=lTrueMoneyEmail
					THARGPAPersonalAccidentCoreType.CURRENT_QUOTATION_PAYMENT_TRUE_MONEY_SUCCESS_CASE=lTrueMoneySuccessCase
					if(!lMapPaymentByTrueMoney.Result){
						return lreturn
					}
					break
				case '5':
					...
					THARGPAPersonalAccidentCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_MOBILE_NO=lRabbitLinePayMobileNo
					THARGPAPersonalAccidentCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_EMAIL=lRabbitLinePayEmail
					THARGPAPersonalAccidentCoreType.CURRENT_QUOTATION_PAYMENT_RABBIT_LINE_PAY_SUCCESS_CASE=lRabbitLinePaySuccessCase
					if(!lMapPaymentByRabbitLinePay.Result){
						return lreturn
					}
					break
				case '6':
					...
					THARGPAPersonalAccidentCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_CARD=lMobileBankingCard
					THARGPAPersonalAccidentCoreType.CURRENT_QUOTATION_PAYMENT_MOBILE_BANKING_SUCCESS_CASE=lMobileBankingSuccessCase
					if(!lMapPaymentByMobileBanking.Result){
						return lreturn
					}
					break
	...
	public Map inputLanguage(String strLanguage){
		...
		if(lLanguage.length()<=0){
			lLanguage=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_LANGUAGE
		}
		try{
			Boolean lResult=false
			lreturn.put('Language','')
			lreturn.put('Result',lResult)
			Map lMapLanguage=this.lTHAPersonalAccidentQuotationPaymentUtil.inputLanguage(lLanguage)
			if(!lMapLanguage.Result){
				lLanguage=''
			}else{
				lLanguage=lMapLanguage.Language
	...
	public Map inputOpportunityNumber(){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapOpportunityNumber=this.lTHAPersonalAccidentQuotationPaymentUtil.inputOpportunityNumber()
			if(!lMapOpportunityNumber.Result){
				lOpportunityNumber=''
			}else{
				lOpportunityNumber=lMapOpportunityNumber.OpportunityNumber
	...
	public Map inputPaymentMethod(String strPaymentMethod){
		...
		if(lPaymentMethod.length()<=0){
			lPaymentMethod=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_PAYMENT_METHOD
		}
		try{
			Boolean lResult=false
			lreturn.put('PaymentMethod','')
			lreturn.put('Result',lResult)
			Map lMapPaymentMethod=this.lTHAPersonalAccidentQuotationPaymentUtil.inputPaymentMethod(lPaymentMethod)
			if(!lMapPaymentMethod.Result){
				lPaymentMethod=''
			}else{
				lPaymentMethod=lMapPaymentMethod.PaymentMethod
	...
	public Map inputPaymentByQR(String strPaymentTaxDeduction,String strPaymentEPolicy){
		...
		if(lPaymentTaxDeduction.length()<=0){
			lPaymentTaxDeduction=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_PAYMENT_TAX_DEDUCTION
		}
		String lPaymentEPolicy=strPaymentEPolicy.trim()
		if(lPaymentEPolicy.length()<=0){
			lPaymentEPolicy=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_PAYMENT_E_POLICY
		}
		String lQrAmtYearly=''
		try{
			Boolean lResult=false
			...
			lreturn.put('Result',lResult)
			Map lMapPaymentByQR=this.lTHAPersonalAccidentQuotationPaymentUtil.inputPaymentByQR(lPaymentTaxDeduction,lPaymentEPolicy)
			if(!lMapPaymentByQR.Result){
				lPaymentTaxDeduction=''
				lPaymentEPolicy=''
				lQrAmtYearly=''
	...
	public Map inputPaymentByIB(String strIbBank,String strPaymentAutoRenew,String strPaymentTaxDeduction,String strPaymentEPolicy,String strIbSuccessCase){
		...
		if(lIbBank.length()<=0){
			lIbBank=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_IB_BANK
		}
		String lPaymentAutoRenew=strPaymentAutoRenew.trim()
		if(lPaymentAutoRenew.length()<=0){
			lPaymentAutoRenew=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_PAYMENT_AUTO_RENEW
		}
		String lPaymentTaxDeduction=strPaymentTaxDeduction.trim()
		if(lPaymentTaxDeduction.length()<=0){
			lPaymentTaxDeduction=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_PAYMENT_TAX_DEDUCTION
		...
		if(lIbSuccessCase.length()<=0){
			lIbSuccessCase=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_IB_SUCCESS_CASE
		}
		String lIbAmtYearly=''
		try{
			Boolean lResult=false
			...
			lreturn.put('Result',lResult)
			Map lMapPaymentByIB=this.lTHAPersonalAccidentQuotationPaymentUtil.inputPaymentByIB(lIbBank,lPaymentAutoRenew,lPaymentTaxDeduction,lPaymentEPolicy,lIbSuccessCase)
			if(!lMapPaymentByIB.Result){
				lIbBank=''
				lPaymentAutoRenew=''
				lPaymentTaxDeduction=''
	...
	public Map inputPaymentByCC(String strCcIsYearly,String strCcIsInstalment,String strCcNumber,String strCcName,String strCcExpiryMonth,String strCcExpiryYear,String strCcCvv,String strCcBank,String strPaymentAutoRenew,String strPaymentTaxDeduction,String strPaymentEPolicy,String strCcSuccessCase){
		...
		if(lCcIsYearly.length()<=0){
			lCcIsYearly=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_CC_IS_YEARLY
		}
		String lCcIsInstalment=strCcIsInstalment.trim()
		if(lCcIsInstalment.length()<=0){
			lCcIsInstalment=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_CC_IS_INSTALMENT
		}
		String lCcNumber=strCcNumber.trim()
		if(lCcNumber.length()<=0){
			lCcNumber=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_CC_NUMBER
		}
		String lCcName=strCcName.trim()
		if(lCcName.length()<=0){
			lCcName=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_CC_NAME
		}
		String lCcExpiryMonth=strCcExpiryMonth.trim()
		if(lCcExpiryMonth.length()<=0){
			lCcExpiryMonth=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_CC_EXPIRY_MONTH
		}
		String lCcExpiryYear=strCcExpiryYear.trim()
		if(lCcExpiryYear.length()<=0){
			lCcExpiryYear=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_CC_EXPIRY_YEAR
		}
		String lCcCvv=strCcCvv.trim()
		if(lCcCvv.length()<=0){
			lCcCvv=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_CC_CVV
		}
		String lCcBank=strCcBank.trim()
		if(lCcBank.length()<=0){
			lCcBank=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_CC_BANK
		}
		String lPaymentAutoRenew=strPaymentAutoRenew.trim()
		if(lPaymentAutoRenew.length()<=0){
			lPaymentAutoRenew=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_PAYMENT_AUTO_RENEW
		...
		if(lCcSuccessCase.length()<=0){
			lCcSuccessCase=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_CC_SUCCESS_CASE
		}
		String lCcAmtYearly=''
		String lCcAmtDeposit=''
		String lCcAmtMonthly=''
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapPaymentByCC=this.lTHAPersonalAccidentQuotationPaymentUtil.inputPaymentByCC(lCcIsYearly,lCcIsInstalment,lCcNumber,lCcName,lCcExpiryMonth,lCcExpiryYear,lCcCvv,lCcBank,lPaymentAutoRenew,lPaymentTaxDeduction,lPaymentEPolicy,lCcSuccessCase)
			if(!lMapPaymentByCC.Result){
				lCcIsYearly=''
				lCcIsInstalment=''
				lCcNumber=''
	...
	public Map inputPaymentByTrueMoney(String strTrueMoneyIsYearly,String strTrueMoneyInstalment,String strTrueMoneyMobileNo,String strTrueMoneyEmail,String strPaymentAutoRenew,String strPaymentTaxDeduction,String strPaymentEPolicy,String strTrueMoneySuccessCase){
		...
		if(lTrueMoneyIsYearly.length()<=0){
			lTrueMoneyIsYearly=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_TRUE_MONEY_IS_YEARLY
		}
		String lTrueMoneyInstalment=strTrueMoneyInstalment.trim()
		if(lTrueMoneyInstalment.length()<=0){
			lTrueMoneyInstalment=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_TRUE_MONEY_INSTALMENT
		}
		String lTrueMoneyMobileNo=strTrueMoneyMobileNo.trim()
		if(lTrueMoneyMobileNo.length()<=0){
			lTrueMoneyMobileNo=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_TRUE_MONEY_MOBILE_NO
		}
		String lTrueMoneyEmail=strTrueMoneyEmail.trim()
		if(lTrueMoneyEmail.length()<=0){
			lTrueMoneyEmail=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_TRUE_MONEY_EMAIL
		}
		String lPaymentAutoRenew=strPaymentAutoRenew.trim()
		if(lPaymentAutoRenew.length()<=0){
			lPaymentAutoRenew=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_PAYMENT_AUTO_RENEW
		...
		if(lTrueMoneySuccessCase.length()<=0){
			lTrueMoneySuccessCase=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_TRUE_MONEY_SUCCESS_CASE
		}
		String lTrueMoneyAmtYearly=''
		String lTrueMoneyAmtDeposit=''
		String lTrueMoneyAmtMonthly=''
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapPaymentByTrueMoney=this.lTHAPersonalAccidentQuotationPaymentUtil.inputPaymentByTrueMoney(lTrueMoneyIsYearly,lTrueMoneyInstalment,lTrueMoneyMobileNo,lTrueMoneyEmail,lPaymentAutoRenew,lPaymentTaxDeduction,lPaymentEPolicy,lTrueMoneySuccessCase)
			if(!lMapPaymentByTrueMoney.Result){
				lTrueMoneyIsYearly=''
				lTrueMoneyInstalment=''
				lTrueMoneyMobileNo=''
	...
	public Map inputPaymentByRabbitLinePay(String strRabbitLinePayIsYearly,String strRabbitLinePayInstalment,String strRabbitLinePayMobileNo,String strRabbitLinePayEmail,String strPaymentAutoRenew,String strPaymentTaxDeduction,String strPaymentEPolicy,String strRabbitLinePaySuccessCase){
		...
		if(lRabbitLinePayIsYearly.length()<=0){
			lRabbitLinePayIsYearly=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_IS_YEARLY
		}
		String lRabbitLinePayInstalment=strRabbitLinePayInstalment.trim()
		if(lRabbitLinePayInstalment.length()<=0){
			lRabbitLinePayInstalment=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_INSTALMENT
		}
		String lRabbitLinePayMobileNo=strRabbitLinePayMobileNo.trim()
		if(lRabbitLinePayMobileNo.length()<=0){
			lRabbitLinePayMobileNo=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_MOBILE_NO
		}
		String lRabbitLinePayEmail=strRabbitLinePayEmail.trim()
		if(lRabbitLinePayEmail.length()<=0){
			lRabbitLinePayEmail=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_EMAIL
		}
		String lPaymentAutoRenew=strPaymentAutoRenew.trim()
		if(lPaymentAutoRenew.length()<=0){
			lPaymentAutoRenew=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_PAYMENT_AUTO_RENEW
		...
		if(lRabbitLinePaySuccessCase.length()<=0){
			lRabbitLinePaySuccessCase=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_RABBIT_LINE_PAY_SUCCESS_CASE
		}
		String lRabbitLinePayAmtYearly=''
		String lRabbitLinePayAmtDeposit=''
		String lRabbitLinePayAmtMonthly=''
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapPaymentByRabbitLinePay=this.lTHAPersonalAccidentQuotationPaymentUtil.inputPaymentByRabbitLinePay(lRabbitLinePayIsYearly,lRabbitLinePayInstalment,lRabbitLinePayMobileNo,lRabbitLinePayEmail,lPaymentAutoRenew,lPaymentTaxDeduction,lPaymentEPolicy,lRabbitLinePaySuccessCase)
			if(!lMapPaymentByRabbitLinePay.Result){
				lRabbitLinePayIsYearly=''
				lRabbitLinePayInstalment=''
				lRabbitLinePayMobileNo=''
	...
	public Map inputPaymentByMobileBanking(String strMobileBankingIsYearly,String strMobileBankingInstalment,String strMobileBankingCard,String strPaymentAutoRenew,String strPaymentTaxDeduction,String strPaymentEPolicy,String strMobileBankingSuccessCase){
		...
		if(lMobileBankingIsYearly.length()<=0){
			lMobileBankingIsYearly=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_MOBILE_BANKING_IS_YEARLY
		}
		String lMobileBankingInstalment=strMobileBankingInstalment.trim()
		if(lMobileBankingInstalment.length()<=0){
			lMobileBankingInstalment=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_MOBILE_BANKING_INSTALMENT
		}
		String lMobileBankingCard=strMobileBankingCard.trim()
		if(lMobileBankingCard.length()<=0){
			lMobileBankingCard=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_MOBILE_BANKING_CARD
		}
		String lPaymentAutoRenew=strPaymentAutoRenew.trim()
		if(lPaymentAutoRenew.length()<=0){
			lPaymentAutoRenew=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_PAYMENT_AUTO_RENEW
		...
		if(lMobileBankingSuccessCase.length()<=0){
			lMobileBankingSuccessCase=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_MOBILE_BANKING_SUCCESS_CASE
		}
		String lMobileBankingAmtYearly=''
		String lMobileBankingAmtDeposit=''
		String lMobileBankingAmtMonthly=''
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapPaymentByMobileBanking=this.lTHAPersonalAccidentQuotationPaymentUtil.inputPaymentByMobileBanking(lMobileBankingIsYearly,lMobileBankingInstalment,lMobileBankingCard,lPaymentAutoRenew,lPaymentTaxDeduction,lPaymentEPolicy,lMobileBankingSuccessCase)
			if(!lMapPaymentByMobileBanking.Result){
				lMobileBankingIsYearly=''
				lMobileBankingInstalment=''
				lMobileBankingCard=''
	...
	public Map inputSalesforceProceedTodo(String strSalesforceProceedTodo){
		...
		if(lSalesforceProceedTodo.length()<=0){
			lSalesforceProceedTodo=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
			lreturn.put('SalesforceProceedTodo','')
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedTodo=this.lTHAPersonalAccidentQuotationPaymentUtil.inputSalesforceProceedTodo(lSalesforceProceedTodo)
			if(!lMapSalesforceProceedTodo.Result){
				lSalesforceProceedTodo=''
			}else{
				lSalesforceProceedTodo=lMapSalesforceProceedTodo.SalesforceProceedTodo
	...
	public Map inputAutomationDataBefore(){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapAutomationDataBefore=this.lTHAPersonalAccidentQuotationPaymentUtil.inputAutomationDataBefore()
			if(!lMapAutomationDataBefore.Result){
				lAutomationOpportunityNumber=''
				lAutomationPremiumYearly=''
				lAutomationPremiumDeposit=''
	...
	public Map inputAutomationDataAfter(){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapAutomationDataAfter=this.lTHAPersonalAccidentQuotationPaymentUtil.inputAutomationDataAfter()
			if(!lMapAutomationDataAfter.Result){
				lSalesforceRetrieveDataOpportunityNumber=''
				lSalesforceRetrieveDataPremiumYearly=''
				lSalesforceRetrieveDataPremiumDeposit=''
	...
	public Map inputSalesforceProceedVerifyBefore(Map caseOutput){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result','')
			Map lMapSalesforceProceedVerifyBefore=this.lTHAPersonalAccidentQuotationPaymentUtil.inputSalesforceProceedVerifyBefore(caseOutput)
	...
	public Map inputSalesforceProceedVerifyAfter(Map caseOutput){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result','')
			Map lMapSalesforceProceedVerifyAfter=this.lTHAPersonalAccidentQuotationPaymentUtil.inputSalesforceProceedVerifyAfter(caseOutput)
	...
	public Map inputSalesforceProceedRetrieveTodo(String strSalesforceProceedRetrieveTodo,String strAccountNameAnonymous){
		...
		if(lSalesforceProceedRetrieveTodo.length()<=0){
			lSalesforceProceedRetrieveTodo=THARGPAPersonalAccidentQuotationPaymentType.DEFAULT_SALESFORCE_PROCEED_TODO
		}
		try{
			Boolean lResult=false
			lreturn.put('SalesforceProceedRetrieveTodo','')
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedRetrieveTodo=this.lTHAPersonalAccidentQuotationPaymentUtil.inputSalesforceProceedRetrieveTodo(lSalesforceProceedRetrieveTodo,strAccountNameAnonymous)
			if(!lMapSalesforceProceedRetrieveTodo.Result){
				lSalesforceProceedRetrieveTodo=''
			}else{
				lSalesforceProceedRetrieveTodo=lMapSalesforceProceedRetrieveTodo.SalesforceProceedRetrieveTodo
	...
	public Map inputSalesforceProceedRetrieveWebFromUrl(String strRetrievePageURL){
		...
		try{
			Boolean lResult=false
			lreturn.put('Result',lResult)
			Map lMapSalesforceProceedRetrieveWebFromUrl=this.lTHAPersonalAccidentQuotationPaymentUtil.inputSalesforceProceedRetrieveWebFromUrl(strRetrievePageURL)
	...
	public Map inputActualResult(Boolean isPositiveCase){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapActualResult=this.lTHAPersonalAccidentQuotationPaymentUtil.inputActualResult(isPositiveCase)
			if(!lMapActualResult.Result){
				lActualResult=''
			}else{
				lActualResult=lMapActualResult.ActualResult
	...
	public Map inputPolicyInsurer(String strPolicyInsurer){
		...
		try{
			...
			lreturn.put('Result',lResult)
			Map lMapPolicyInsurer=this.lTHAPersonalAccidentQuotationPaymentUtil.inputPolicyInsurer(lPolicyInsurer)
			if(!lMapPolicyInsurer.Result){
				lPolicyInsurer=''
			}else{
				lPolicyInsurer=lMapPolicyInsurer.PolicyInsurer
