...
public class THARGPAExtendedWarrantyQuotationPaymentFactory{
	public Boolean IsValid=false
	private lCaseList=[]
	public Integer CurrentCase=0
	public THARGPAExtendedWarrantyQuotationPaymentFactory(TestData dataInput){
		super()
	...
	public Boolean validateDataInput(TestData dataInput){
		...
		try{
			...
			lreturn=(dataColNum>=THARGPAExtendedWarrantyQuotationPaymentType.INPUT_DATA_COL_BEGIN)&&(dataRowNum>=THARGPAExtendedWarrantyQuotationPaymentType.INPUT_DATA_ROW_END)
		}catch(Exception e){
			e.printStackTrace()
			return lreturn
		}
	...
	public void initCaseList(TestData dataInput){
		...
		try{
			...
			Integer dataColNum=dataInput.getColumnNumbers()
			Integer dataRowNum=dataInput.getRowNumbers()
			Integer dataColIndex=THARGPAExtendedWarrantyQuotationPaymentType.INPUT_DATA_COL_BEGIN
			Integer dataRowIndex=THARGPAExtendedWarrantyQuotationPaymentType.INPUT_DATA_ROW_BEGIN
			THARGPAExtendedWarrantyQuotationPaymentType dataCase
			Map caseInput=[:]
			caseInput.clear()
			String cellVal=''
			for(dataColIndex=THARGPAExtendedWarrantyQuotationPaymentType.INPUT_DATA_COL_BEGIN;dataColIndex<=dataColNum;dataColIndex++){
				dataCase=new THARGPAExtendedWarrantyQuotationPaymentType()
				dataCase.initInput()
				dataCase.initOutput()
				caseInput.clear()
				...
				dataRowIndex=THARGPAExtendedWarrantyQuotationPaymentType.INPUT_DATA_ROW_BEGIN
	...
	public THARGPAExtendedWarrantyQuotationPaymentType CaseData(){
	...
	public Boolean SaveOutput(){
		Boolean lreturn=false
		THARGPAExtendedWarrantyQuotationPaymentType caseData
		try{
			Integer dataColIndex=THARGPAExtendedWarrantyQuotationPaymentType.OUTPUT_EXCEL_COL_BEGIN+(this.CurrentCase-1)
			Integer dataRowIndex=THARGPAExtendedWarrantyQuotationPaymentType.OUTPUT_EXCEL_ROW_BEGIN
			if((this.CurrentCase>0)&&(this.lCaseList.size()>0)){
				caseData=this.lCaseList.get(this.CurrentCase-1)
				def xlsWorkbook=ExcelKeywords.getWorkbook(THARGPAExtendedWarrantyQuotationPaymentType.OUTPUT_EXCEL_FILE_NAME)
				def xlsSheet=ExcelKeywords.getExcelSheet(xlsWorkbook,THARGPAExtendedWarrantyQuotationPaymentType.OUTPUT_EXCEL_SHEET_NAME)
				...
				dataRowIndex+=1
				IGNUemaHelper.saveExcelWorkbookByFileName(THARGPAExtendedWarrantyQuotationPaymentType.OUTPUT_EXCEL_FILE_NAME,xlsWorkbook)
				lreturn=true
			}
		}catch(Exception e){
			e.printStackTrace()
