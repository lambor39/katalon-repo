...
public class THALowMileageCarQuotationInputFactory{
	public Boolean IsValid=false
	private lCaseList=[]
	public Integer CurrentCase=0
	public THALowMileageCarQuotationInputFactory(TestData dataInput){
		super()
	...
	public Boolean validateDataInput(TestData dataInput){
		...
		try{
			...
			lreturn=(dataColNum>=THALowMileageCarQuotationInputType.INPUT_DATA_COL_BEGIN)&&(dataRowNum>=THALowMileageCarQuotationInputType.INPUT_DATA_ROW_END)
		}catch(Exception e){
			e.printStackTrace()
			return lreturn
		}
	...
	public void initCaseList(TestData dataInput){
		...
		try{
			...
			Integer dataColNum=dataInput.getColumnNumbers()
			Integer dataRowNum=dataInput.getRowNumbers()
			Integer dataColIndex=THALowMileageCarQuotationInputType.INPUT_DATA_COL_BEGIN
			Integer dataRowIndex=THALowMileageCarQuotationInputType.INPUT_DATA_ROW_BEGIN
			THALowMileageCarQuotationInputType dataCase
			Map caseInput=[:]
			caseInput.clear()
			String cellVal=''
			for(dataColIndex=THALowMileageCarQuotationInputType.INPUT_DATA_COL_BEGIN;dataColIndex<=dataColNum;dataColIndex++){
				dataCase=new THALowMileageCarQuotationInputType()
				dataCase.initInput()
				dataCase.initOutput()
				caseInput.clear()
				...
				dataRowIndex=THALowMileageCarQuotationInputType.INPUT_DATA_ROW_BEGIN
	...
	public THALowMileageCarQuotationInputType CaseData(){
	...
	public Boolean SaveOutput(){
		Boolean lreturn=false
		THALowMileageCarQuotationInputType caseData
		try{
			Integer dataColIndex=THALowMileageCarQuotationInputType.OUTPUT_EXCEL_COL_BEGIN+(this.CurrentCase-1)
			Integer dataRowIndex=THALowMileageCarQuotationInputType.OUTPUT_EXCEL_ROW_BEGIN
			if((this.CurrentCase>0)&&(this.lCaseList.size()>0)){
				caseData=this.lCaseList.get(this.CurrentCase-1)
				def xlsWorkbook=ExcelKeywords.getWorkbook(THALowMileageCarQuotationInputType.OUTPUT_EXCEL_FILE_NAME)
				def xlsSheet=ExcelKeywords.getExcelSheet(xlsWorkbook,THALowMileageCarQuotationInputType.OUTPUT_EXCEL_SHEET_NAME)
				...
				dataRowIndex+=1
				IGNUemaHelper.saveExcelWorkbookByFileName(THALowMileageCarQuotationInputType.OUTPUT_EXCEL_FILE_NAME,xlsWorkbook)
				lreturn=true
			}
		}catch(Exception e){
			e.printStackTrace()
