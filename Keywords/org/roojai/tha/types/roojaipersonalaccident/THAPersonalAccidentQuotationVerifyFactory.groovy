...
public class THAPersonalAccidentQuotationVerifyFactory{
	public Boolean IsValid=false
	private lCaseList=[]
	public Integer CurrentCase=0
	public THAPersonalAccidentQuotationVerifyFactory(TestData dataInput){
		super()
	...
	public Boolean validateDataInput(TestData dataInput){
		...
		try{
			...
			lreturn=(dataColNum>=THAPersonalAccidentQuotationVerifyType.INPUT_DATA_COL_BEGIN)&&(dataRowNum>=THAPersonalAccidentQuotationVerifyType.INPUT_DATA_ROW_END)
		}catch(Exception e){
			e.printStackTrace()
			return lreturn
		}
	...
	public void initCaseList(TestData dataInput){
		...
		try{
			...
			Integer dataColNum=dataInput.getColumnNumbers()
			Integer dataRowNum=dataInput.getRowNumbers()
			Integer dataColIndex=THAPersonalAccidentQuotationVerifyType.INPUT_DATA_COL_BEGIN
			Integer dataRowIndex=THAPersonalAccidentQuotationVerifyType.INPUT_DATA_ROW_BEGIN
			THAPersonalAccidentQuotationVerifyType dataCase
			Map caseInput=[:]
			caseInput.clear()
			String cellVal=''
			for(dataColIndex=THAPersonalAccidentQuotationVerifyType.INPUT_DATA_COL_BEGIN;dataColIndex<=dataColNum;dataColIndex++){
				dataCase=new THAPersonalAccidentQuotationVerifyType()
				dataCase.initInput()
				dataCase.initOutput()
				caseInput.clear()
				...
				dataRowIndex=THAPersonalAccidentQuotationVerifyType.INPUT_DATA_ROW_BEGIN
	...
	public THAPersonalAccidentQuotationVerifyType CaseData(){
	...
	public Boolean SaveOutput(){
		Boolean lreturn=false
		THAPersonalAccidentQuotationVerifyType caseData
		try{
			Integer dataColIndex=THAPersonalAccidentQuotationVerifyType.OUTPUT_EXCEL_COL_BEGIN+(this.CurrentCase-1)
			Integer dataRowIndex=THAPersonalAccidentQuotationVerifyType.OUTPUT_EXCEL_ROW_BEGIN
			if((this.CurrentCase>0)&&(this.lCaseList.size()>0)){
				caseData=this.lCaseList.get(this.CurrentCase-1)
				def xlsWorkbook=ExcelKeywords.getWorkbook(THAPersonalAccidentQuotationVerifyType.OUTPUT_EXCEL_FILE_NAME)
				def xlsSheet=ExcelKeywords.getExcelSheet(xlsWorkbook,THAPersonalAccidentQuotationVerifyType.OUTPUT_EXCEL_SHEET_NAME)
				...
				dataRowIndex+=1
				IGNUemaHelper.saveExcelWorkbookByFileName(THAPersonalAccidentQuotationVerifyType.OUTPUT_EXCEL_FILE_NAME,xlsWorkbook)
				lreturn=true
			}
		}catch(Exception e){
			e.printStackTrace()
