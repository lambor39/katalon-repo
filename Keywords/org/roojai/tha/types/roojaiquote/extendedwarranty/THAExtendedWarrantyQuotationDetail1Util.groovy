...
public class THAExtendedWarrantyQuotationDetail1Util{
	private WebDriver driver=null
	private WebDriver selenium=null
	public THAExtendedWarrantyQuotationDetail1Util(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	public Boolean waitUntilOpportunityNumberUpdated(){
		...
		try{
			...
			String lOpportunityNumberOld=THAExtendedWarrantyCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER.trim()
			...
			if(lElementOpportunityNumberNew){
				for(Integer lIndex=1;lIndex<=5;lIndex++){
	...
	public Boolean inputDetail1(Map caseInput,Map caseOutput){
		...
		String lBlueBookWarantyExpiryDate=caseInput.BlueBookWarantyExpiryDate.toString().trim()
		String lBlueBookWarantyExpiryOffsetDate=caseInput.BlueBookWarantyExpiryOffsetDate.toString().trim()
		String lBlueBookWarantyOdometer=caseInput.BlueBookWarantyOdometer.toString().trim()
		...
		try{
			...
			lMapDetail1Input.BlueBookWarantyExpiryDate=lBlueBookWarantyExpiryDate
			lMapDetail1Input.BlueBookWarantyExpiryOffsetDate=lBlueBookWarantyExpiryOffsetDate
			lMapDetail1Input.BlueBookWarantyOdometer=lBlueBookWarantyOdometer
			...
			lBlueBookWarantyExpiryDate=lMapInputDetail1.BlueBookWarantyExpiryDate
			lBlueBookWarantyExpiryOffsetDate=lMapInputDetail1.BlueBookWarantyExpiryOffsetDate
			lBlueBookWarantyOdometer=lMapInputDetail1.BlueBookWarantyOdometer
			...
			caseOutput.BlueBookWarantyExpiryDate=lBlueBookWarantyExpiryDate
			caseOutput.BlueBookWarantyExpiryOffsetDate=lBlueBookWarantyExpiryOffsetDate
			caseOutput.BlueBookWarantyOdometer=lBlueBookWarantyOdometer
	...
	public Map initDetail1Input(){
		...
		try{
			...
			lreturn.put('BlueBookWarantyExpiryDate','')
			lreturn.put('BlueBookWarantyExpiryOffsetDate','')
			lreturn.put('BlueBookWarantyOdometer','')
	...
	public Map inputDetail1A(Map mapInput,Boolean isPositiveCase){
		...
		String lBlueBookWarantyExpiryDate=mapInput.BlueBookWarantyExpiryDate.toString().trim()
		String lBlueBookWarantyExpiryOffsetDate=mapInput.BlueBookWarantyExpiryOffsetDate.toString().trim()
		String lBlueBookWarantyOdometer=mapInput.BlueBookWarantyOdometer.toString().trim()
		...
		try{
			...
			if(!lIsHolderCallMeNowToDo){
				...
				Map lMapBlueBookWarantyExpiryDate=this.inputBlueBookWarantyExpiryDate(lBlueBookWarantyExpiryDate,lBlueBookWarantyExpiryOffsetDate)
				if(!lMapBlueBookWarantyExpiryDate.Result){
					lBlueBookWarantyExpiryDate=''
					lBlueBookWarantyExpiryOffsetDate=''
				}else{
					lBlueBookWarantyExpiryDate=lMapBlueBookWarantyExpiryDate.BlueBookWarantyExpiryDate
					lBlueBookWarantyExpiryOffsetDate=lMapBlueBookWarantyExpiryDate.BlueBookWarantyExpiryOffsetDate
				}
				lreturn.BlueBookWarantyExpiryDate=lBlueBookWarantyExpiryDate
				lreturn.BlueBookWarantyExpiryOffsetDate=lBlueBookWarantyExpiryOffsetDate
				if(!lMapBlueBookWarantyExpiryDate.Result){
					return lreturn
				}
				Map lMapBlueBookWarantyOdometer=this.inputBlueBookWarantyOdometer(lBlueBookWarantyOdometer)
				if(!lMapBlueBookWarantyOdometer.Result){
					lBlueBookWarantyOdometer=''
				}else{
					lBlueBookWarantyOdometer=lMapBlueBookWarantyOdometer.BlueBookWarantyOdometer
				}
				lreturn.BlueBookWarantyOdometer=lBlueBookWarantyOdometer
				if(lBlueBookWarantyOdometer.length()<=0){
					return lreturn
				}
				if(!lIsHolderIsPerson){
					...
					if(lIsHolderIsMain){
						Boolean lIsMainDisplayed=IGNUemaHelper.checkElementVisibleByLocatorJs(this.driver,IGNUemaHelper.getTagDataSeleniumKey('mainDriverEdit'))
						if(lIsMainDisplayed){
	...
	public Map inputPositiveCase(String strPositiveCase){
		...
		if(lPositiveCase.length()<=0){
			lPositiveCase=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_POSITIVE_CASE
		}
		try{
	...
	public Map inputLanguageA(String strLanguage){
		...
		try{
			...
			if(!lLanguageItem){
				lLanguageItem=lLanguageList.get(THAExtendedWarrantyQuotationDetail1Type.DEFAULT_LANGUAGE.toUpperCase())
			}
			...
			if(lLanguage.toUpperCase().contains('NO OPTION')){
				lLanguage=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_LANGUAGE
			}
	...
	public Map inputSubmitCallMeBackA(String strSubmitCallMeBack,String strCallMeBackEmail,String strCallMeBackMobile){
		...
		if(lSubmitCallMeBack.length()<=0){
			lSubmitCallMeBack=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_SUBMIT_CALL_ME_BACK
		}
		String lCallMeBackEmail=strCallMeBackEmail.trim()
		if(lCallMeBackEmail.length()<=0){
			lCallMeBackEmail=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_CALL_ME_BACK_EMAIL
		}
		String lCallMeBackMobile=strCallMeBackMobile.trim()
		if(lCallMeBackMobile.length()<=0){
			lCallMeBackMobile=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_CALL_ME_BACK_MOBILE
		}
		try{
			Boolean lResult=false
			lreturn.put('SubmitCallMeBack','')
	...
	public Map inputHolderIsMainA(String strHolderIsMain){
		...
		if(lHolderIsMain.length()<=0){
			lHolderIsMain=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_IS_MAIN
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderIsMain','')
	...
	public Map inputHolderEmailA(String strHolderEmail){
		...
		if(lHolderEmail.length()<=0){
			lHolderEmail=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_EMAIL
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderEmail','')
	...
	public Map inputHolderNewsletterA(String strHolderNewsletter){
		...
		if(lHolderNewsletter.length()<=0){
			lHolderNewsletter=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_NEWSLETTER
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderNewsletter','')
			...
			String lTagButton='ew-pd1-userAcceptConsent'
			String lLocatorButton=IGNUemaHelper.getTagDataSeleniumKey(lTagButton)
			...
			if(!lElementButton){
				...
				lStageStr=IGNUemaHelper.getDataSeleniumValueWebElement(this.driver,lElementButton)
				lStageBool=IGNUemaHelper.convertStringToBoolean(lStageStr)
				lStageStr=IGNUemaHelper.convertBooleanToYesNoString(lStageBool)
	...
	public Map inputHolderPhoneNoA(String strHolderPhoneNo){
		...
		if(lHolderPhoneNo.length()<=0){
			lHolderPhoneNo=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_PHONE_NO
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderPhoneNo','')
	...
	public Map inputHolderCallMeNowToDoA(String strHolderCallMeNowToDo){
		...
		if(lHolderCallMeNowToDo.length()<=0){
			lHolderCallMeNowToDo=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_CALL_ME_NOW_TO_DO
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderCallMeNowToDo','')
	...
	public Map inputCallMeNowA(String strCallMeNow){
		...
		if(lCallMeNow.length()<=0){
			lCallMeNow=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_CALL_ME_NOW
		}
		...
		try{
			...
			lCallMeNow=''
			if(!lCallMeNowItem){
				lCallMeNowItem=lCallMeNowList.get(THAExtendedWarrantyQuotationDetail1Type.DEFAULT_CALL_ME_NOW)
			}
	...
	public Map inputCarForCommercialA(String strCarForCommercial){
		...
		if(lCarForCommercial.length()<=0){
			lCarForCommercial=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_CAR_FOR_COMMERCIAL
		}
		try{
			Boolean lResult=false
			lreturn.put('CarForCommercial','')
	...
	public Map inputHolderIsPersonA(String strHolderIsPerson){
		...
		if(lHolderIsPerson.length()<=0){
			lHolderIsPerson=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_IS_PERSON
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderIsPerson','')
	...
	public Map inputHolderCompNameA(String strHolderCompName){
		...
		if(lHolderCompName.length()<=0){
			lHolderCompName=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_COMP_NAME
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderCompName','')
	...
	public Map inputHolderCompIdA(String strHolderCompId){
		...
		if(lHolderCompId.length()<=0){
			lHolderCompId=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_COMP_ID
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderCompId','')
	...
	public Map inputCarPlateNumberA(String strCarPlateNumber){
		...
		if(lCarPlateNumber.length()<=0){
			lCarPlateNumber=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_CAR_PLATE_NUMBER
		}
		if(lCarPlateNumber.length()<=0){
	...
	public Map inputCarPlateProvinceA(String strCarPlateProvince){
		...
		if(lCarPlateProvince.length()<=0){
			lCarPlateProvince=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_CAR_PLATE_PROVINCE
		}
		try{
			Boolean lResult=false
			lreturn.put('CarPlateProvince','')
	...
	public Map inputBlueBookWarantyExpiryDate(String strBlueBookWarantyExpiryDate,String strBlueBookWarantyExpiryOffsetDate){
		Map lreturn=[:]
		try{
			Boolean lResult=false
			lreturn.put('BlueBookWarantyExpiryDate','')
			...
			if(IGNUemaHelper.CURRENT_AB_TESTING_ENABLE){
				switch(IGNUemaHelper.CURRENT_AB_TESTING_VERSION){
					case 'A':
						lreturn=this.inputBlueBookWarantyExpiryDateA(strBlueBookWarantyExpiryDate,strBlueBookWarantyExpiryOffsetDate)
						break
					case 'B':
						lreturn=this.inputBlueBookWarantyExpiryDateB(strBlueBookWarantyExpiryDate,strBlueBookWarantyExpiryOffsetDate)
						break
					default:
						return lreturn
						break
	...
	public Map inputBlueBookWarantyExpiryDateA(String strBlueBookWarantyExpiryDate,String strBlueBookWarantyExpiryOffsetDate){
		Map lreturn=[:]
		String lBlueBookWarantyExpiryDate=strBlueBookWarantyExpiryDate.trim()
		String lBlueBookWarantyExpiryOffsetDate=strBlueBookWarantyExpiryOffsetDate.trim()
		if(lBlueBookWarantyExpiryDate.length()<=0){
			lBlueBookWarantyExpiryDate=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_BLUE_BOOK_WARANTY_EXPIRY_DATE
		}
		String lFormatDate='dd/MM/yyyy'
		if(lBlueBookWarantyExpiryDate.length()<=0){
			lBlueBookWarantyExpiryDate=IGNUemaHelper.convertLocalDateToString(lFormatDate)
		}
		if(lBlueBookWarantyExpiryOffsetDate.length()<=0){
			lBlueBookWarantyExpiryOffsetDate=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_BLUE_BOOK_WARANTY_EXPIRY_OFFSET_DATE
		}
		Integer lBlueBookWarantyExpiryOffsetDateNum=IGNUemaHelper.convertStringToInteger(lBlueBookWarantyExpiryOffsetDate,0)
		try{
			Boolean lResult=false
			lreturn.put('BlueBookWarantyExpiryDate','')
			lreturn.put('BlueBookWarantyExpiryOffsetDate','')
			lreturn.put('Result',lResult)
			String lTagBlueBookWarantyExpiryDate='warrantyExpiryDate'
			String lLocatorBlueBookWarantyExpiryDate=IGNUemaHelper.getTagDataSeleniumKey(lTagBlueBookWarantyExpiryDate)
			Boolean lIsBlueBookWarantyExpiryDate=IGNUemaHelper.checkElementVisibleByLocatorKs(this.driver,lLocatorBlueBookWarantyExpiryDate)
			if(!lIsBlueBookWarantyExpiryDate){
				lBlueBookWarantyExpiryDate='No Option'
				lBlueBookWarantyExpiryOffsetDate='No Option'
			}else{
				lBlueBookWarantyExpiryOffsetDate=lBlueBookWarantyExpiryOffsetDateNum.toString()
				lBlueBookWarantyExpiryDate=IGNUemaHelper.offsetDayByDateString(lFormatDate,lBlueBookWarantyExpiryDate,lBlueBookWarantyExpiryOffsetDateNum)
				String lBlueBookWarantyExpiryDateDD=IGNUemaHelper.extractStringDayFromDateStringSlashDDMMYYYY(lBlueBookWarantyExpiryDate)
				String lBlueBookWarantyExpiryDateMM=IGNUemaHelper.extractStringMonthFromDateStringSlashDDMMYYYY(lBlueBookWarantyExpiryDate)
				String lBlueBookWarantyExpiryDateYYYY=IGNUemaHelper.extractStringYearFromDateStringSlashDDMMYYYY(lBlueBookWarantyExpiryDate)
				lBlueBookWarantyExpiryDate=''
				String lTagInputTextDD='warrantyExpiryDate-dd'
				...
				if(!lElementInputTextDD){
					return lreturn
				}
				lBlueBookWarantyExpiryDateDD=IGNUemaHelper.typeTextByWebElement(this.driver,lElementInputTextDD,lBlueBookWarantyExpiryDateDD)
				String lTagInputTextMM='warrantyExpiryDate-mm'
				...
				lBlueBookWarantyExpiryDateMM=IGNUemaHelper.typeTextByWebElement(this.driver,lElementInputTextMM,lBlueBookWarantyExpiryDateMM)
				String lTagInputTextYYYY='warrantyExpiryDate-yyyy'
				...
				lBlueBookWarantyExpiryDateYYYY=IGNUemaHelper.typeTextByWebElement(this.driver,lElementInputTextYYYY,lBlueBookWarantyExpiryDateYYYY)
				lBlueBookWarantyExpiryDate=lBlueBookWarantyExpiryDateDD+'/'+lBlueBookWarantyExpiryDateMM+'/'+lBlueBookWarantyExpiryDateYYYY
				String lTagButtonNext='warrantyExpiryDate-next'
			...
			lResult=lBlueBookWarantyExpiryDate.length()>0
			if(lResult){
				lreturn.put('BlueBookWarantyExpiryDate',lBlueBookWarantyExpiryDate)
				lreturn.put('BlueBookWarantyExpiryOffsetDate',lBlueBookWarantyExpiryOffsetDate)
				lreturn.put('Result',lResult)
			}
		}catch(Exception e){
			e.printStackTrace()
	...
	public Map inputBlueBookWarantyExpiryDateB(String strBlueBookWarantyExpiryDate,String strBlueBookWarantyExpiryOffsetDate){
		Map lreturn=[:]
		String lBlueBookWarantyExpiryDate=strBlueBookWarantyExpiryDate.trim()
		String lBlueBookWarantyExpiryOffsetDate=strBlueBookWarantyExpiryOffsetDate.trim()
		if(lBlueBookWarantyExpiryDate.length()<=0){
	...
	public Map inputBlueBookWarantyOdometer(String strBlueBookWarantyOdometer){
		Map lreturn=[:]
		try{
			Boolean lResult=false
			lreturn.put('BlueBookWarantyOdometer','')
			...
			if(IGNUemaHelper.CURRENT_AB_TESTING_ENABLE){
				switch(IGNUemaHelper.CURRENT_AB_TESTING_VERSION){
					case 'A':
						lreturn=this.inputBlueBookWarantyOdometerA(strBlueBookWarantyOdometer)
						break
					case 'B':
						lreturn=this.inputBlueBookWarantyOdometerB(strBlueBookWarantyOdometer)
						break
					default:
						return lreturn
						break
	...
	public Map inputBlueBookWarantyOdometerA(String strBlueBookWarantyOdometer){
		Map lreturn=[:]
		String lBlueBookWarantyOdometer=strBlueBookWarantyOdometer.trim()
		if(lBlueBookWarantyOdometer.length()<=0){
			lBlueBookWarantyOdometer=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_BLUE_BOOK_WARANTY_ODOMETER
		}
		try{
			Boolean lResult=false
			lreturn.put('BlueBookWarantyOdometer','')
			lreturn.put('Result',lResult)
			String lTagBlueBookWarantyOdometerInputText='warrantyOdometerCovered'
			String lLocatorBlueBookWarantyOdometerInputText=IGNUemaHelper.getTagDataSeleniumKey(lTagBlueBookWarantyOdometerInputText)
			Boolean lIsBlueBookWarantyOdometerInputText=IGNUemaHelper.checkElementVisibleByLocatorKs(this.driver,lLocatorBlueBookWarantyOdometerInputText)
			if(!lIsBlueBookWarantyOdometerInputText){
				lBlueBookWarantyOdometer='No Option'
			}else{
				String lInputText=lBlueBookWarantyOdometer
				lBlueBookWarantyOdometer=''
				lBlueBookWarantyOdometer=IGNUemaHelper.typeTextWithEnterByLocator(this.driver,lLocatorBlueBookWarantyOdometerInputText,lInputText)
				IGNUemaHelper.selectButtonByLocator(this.driver,IGNUemaHelper.getTagDataSeleniumKey('warrantyodometercovered-next'))
			}
			lResult=lBlueBookWarantyOdometer.length()>0
			if(lResult){
				lreturn.put('BlueBookWarantyOdometer',lBlueBookWarantyOdometer)
				lreturn.put('Result',lResult)
			}
		}catch(Exception e){
			e.printStackTrace()
	...
	public Map inputBlueBookWarantyOdometerB(String strBlueBookWarantyOdometer){
		Map lreturn=[:]
		String lBlueBookWarantyOdometer=strBlueBookWarantyOdometer.trim()
		if(lBlueBookWarantyOdometer.length()<=0){
			lBlueBookWarantyOdometer=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_BLUE_BOOK_WARANTY_ODOMETER
	...
	public Map inputHolderStatusA(String strHolderStatus){
		...
		Map lHolderStatusList=['1':'1','2':'2','3':'3','4':'4']
		Integer lHolderStatusDefaultMin=1
		Integer lHolderStatusDefaultMax=4
		try{
			Boolean lResult=false
			lreturn.put('HolderStatus','')
			lreturn.put('Result',lResult)
			String lHolderStatusItem=lHolderStatusList.get(lHolderStatus)
			if(!lHolderStatusItem){
				lHolderStatusItem=lHolderStatusDefaultMin.toString()
			}
			String lTagHolderStatus='ownerEdit'
			String lLocatorHolderStatus=IGNUemaHelper.getTagDataSeleniumKey(lTagHolderStatus)
			IGNUemaHelper.selectButtonByLocator(this.driver,lLocatorHolderStatus)
			lHolderStatus=''
			switch(lHolderStatusItem){
				case '2':
					String lTagMaleMarriedButtonKey='ownerGender'
					String lTagMaleMarriedButtonValue='Male-Married'
					String lLocatorMaleMarriedButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagMaleMarriedButtonKey,lTagMaleMarriedButtonValue)
					WebElement lElementMaleMarriedButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagMaleMarriedButtonKey,lTagMaleMarriedButtonValue)
					if(!lElementMaleMarriedButton){
						return lreturn
					}else{
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementMaleMarriedButton)
						String lMaleMarriedText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementMaleMarriedButton).trim()
						lHolderStatus=lMaleMarriedText
					}
					break
				case '3':
					String lTagFemaleSingleButtonKey='ownerGender'
					String lTagFemaleSingleButtonValue='Female-Single'
					String lLocatorFemaleSingleButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagFemaleSingleButtonKey,lTagFemaleSingleButtonValue)
					WebElement lElementFemaleSingleButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagFemaleSingleButtonKey,lTagFemaleSingleButtonValue)
					if(!lElementFemaleSingleButton){
						return lreturn
					}else{
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementFemaleSingleButton)
						String lFemaleSingleText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementFemaleSingleButton).trim()
						lHolderStatus=lFemaleSingleText
					}
					break
				case '4':
					String lTagFemaleMarriedButtonKey='ownerGender'
					String lTagFemaleMarriedButtonValue='Female-Married'
					String lLocatorFemaleMarriedButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagFemaleMarriedButtonKey,lTagFemaleMarriedButtonValue)
					WebElement lElementFemaleMarriedButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagFemaleMarriedButtonKey,lTagFemaleMarriedButtonValue)
					if(!lElementFemaleMarriedButton){
						return lreturn
					}else{
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementFemaleMarriedButton)
						String lFemaleMarriedText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementFemaleMarriedButton).trim()
						lHolderStatus=lFemaleMarriedText
					}
					break
				default:
					String lTagMaleSingleButtonKey='ownerGender'
					String lTagMaleSingleButtonValue='Male-Single'
					String lLocatorMaleSingleButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagMaleSingleButtonKey,lTagMaleSingleButtonValue)
					WebElement lElementMaleSingleButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagMaleSingleButtonKey,lTagMaleSingleButtonValue)
					if(!lElementMaleSingleButton){
						return lreturn
					}else{
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementMaleSingleButton)
						String lMaleSingleText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementMaleSingleButton).trim()
						lHolderStatus=lMaleSingleText
					}
					break
			}
	...
	public Map inputHolderDobA(String strHolderDob){
		...
		if(lHolderDob.length()<=0){
			lHolderDob=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_DOB
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderDob','')
			lreturn.put('Result',lResult)
			String lTagHolderDob='DOBMD'
			String lLocatorHolderDob=IGNUemaHelper.getTagDataSeleniumKey(lTagHolderDob)
			WebElement lElementHolderDob=IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,lTagHolderDob)
			if(!lElementHolderDob){
				return lreturn
			}else{
	...
	public Map inputHolderIsThaiA(String strHolderIsThai){
		...
		if(lHolderIsThai.length()<=0){
			lHolderIsThai=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_IS_THAI
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderIsThai','')
	...
	public Map inputHolderThaiIdA(String strHolderThaiId){
		...
		if(lHolderThaiId.length()<=0){
			lHolderThaiId=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_THAI_ID
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderThaiId','')
	...
	public Map inputHolderOccupationA(String strHolderOccupation){
		...
		if(lHolderOccupation.length()<=0){
			lHolderOccupation=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_OCCUPATION
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderOccupation','')
	...
	public Map inputHolderPassportA(String strHolderPassport){
		...
		if(lHolderPassport.length()<=0){
			lHolderPassport=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_PASSPORT
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderPassport','')
			...
			String lLocatorInputText=IGNUemaHelper.getTagDataSeleniumKey(lTagInputText)
			lHolderPassport=IGNUemaHelper.typeTextByLocator(this.driver,lLocatorInputText,lHolderPassport)
	...
	public Map inputHolderCountryA(String strHolderCountry){
		...
		if(lHolderCountry.length()<=0){
			lHolderCountry=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_COUNTRY
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderCountry','')
	...
	public Map inputHolderNameA(String strHolderName){
		...
		if(lHolderName.length()<=0){
			lHolderName=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_NAME
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderName','')
	...
	public Map inputHolderSurnameA(String strHolderSurname){
		...
		if(lHolderSurname.length()<=0){
			lHolderSurname=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_HOLDER_SURNAME
		}
		try{
			Boolean lResult=false
			lreturn.put('HolderSurname','')
	...
	public Map inputMainStatusA(String strMainStatus){
		...
		Map lMainStatusList=['1':'1','2':'2','3':'3','4':'4']
		Integer lMainStatusDefaultMin=1
		Integer lMainStatusDefaultMax=4
		try{
			Boolean lResult=false
			lreturn.put('MainStatus','')
			lreturn.put('Result',lResult)
			String lMainStatusItem=lMainStatusList.get(lMainStatus)
			if(!lMainStatusItem){
				lMainStatusItem=lMainStatusDefaultMin.toString()
			}
			String lTagMainStatus='mainDriverEdit'
			String lLocatorMainStatus=IGNUemaHelper.getTagDataSeleniumKey(lTagMainStatus)
			IGNUemaHelper.selectButtonByLocator(this.driver,lLocatorMainStatus)
			lMainStatus=''
			switch(lMainStatusItem){
				case '2':
					String lTagMaleMarriedButtonKey='mainDriverGender'
					String lTagMaleMarriedButtonValue='Male-Married'
					String lLocatorMaleMarriedButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagMaleMarriedButtonKey,lTagMaleMarriedButtonValue)
					WebElement lElementMaleMarriedButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagMaleMarriedButtonKey,lTagMaleMarriedButtonValue)
					if(!lElementMaleMarriedButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementMaleMarriedButton)
						String lMaleMarriedText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementMaleMarriedButton).trim()
						lMainStatus=lMaleMarriedText
					}
					break
				case '3':
					String lTagFemaleSingleButtonKey='mainDriverGender'
					String lTagFemaleSingleButtonValue='Female-Single'
					String lLocatorFemaleSingleButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagFemaleSingleButtonKey,lTagFemaleSingleButtonValue)
					WebElement lElementFemaleSingleButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagFemaleSingleButtonKey,lTagFemaleSingleButtonValue)
					if(!lElementFemaleSingleButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementFemaleSingleButton)
						String lFemaleSingleText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementFemaleSingleButton).trim()
						lMainStatus=lFemaleSingleText
					}
					break
				case '4':
					String lTagFemaleMarriedButtonKey='mainDriverGender'
					String lTagFemaleMarriedButtonValue='Female-Married'
					String lLocatorFemaleMarriedButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagFemaleMarriedButtonKey,lTagFemaleMarriedButtonValue)
					WebElement lElementFemaleMarriedButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagFemaleMarriedButtonKey,lTagFemaleMarriedButtonValue)
					if(!lElementFemaleMarriedButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementFemaleMarriedButton)
						String lFemaleMarriedText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementFemaleMarriedButton).trim()
						lMainStatus=lFemaleMarriedText
					}
					break
				default:
					String lTagMaleSingleButtonKey='mainDriverGender'
					String lTagMaleSingleButtonValue='Male-Single'
					String lLocatorMaleSingleButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagMaleSingleButtonKey,lTagMaleSingleButtonValue)
					WebElement lElementMaleSingleButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagMaleSingleButtonKey,lTagMaleSingleButtonValue)
					if(!lElementMaleSingleButton){
	...
	public Map inputMainDobA(String strMainDob){
		...
		if(lMainDob.length()<=0){
			lMainDob=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_MAIN_DOB
		}
		try{
			Boolean lResult=false
			lreturn.put('MainDob','')
			lreturn.put('Result',lResult)
			String lTagMainDob='DOBMD'
			String lLocatorMainDob=IGNUemaHelper.getTagDataSeleniumKey(lTagMainDob)
			WebElement lElementMainDob=IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,lTagMainDob)
			if(!lElementMainDob){
				return lreturn
			}else{
				...
				String lTagMainDobButtonNext='mainDriverinfo-next'
				String lLocatorMainDobButtonNext=IGNUemaHelper.getTagDataSeleniumKey(lTagMainDobButtonNext)
				WebElement lElementMainDobButtonNext=IGNUemaHelper.getWebElementFromDataSeleniumKey(this.driver,lTagMainDobButtonNext)
				IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementMainDobButtonNext)
			}
	...
	public Map inputMainIsThaiA(String strMainIsThai){
		...
		if(lMainIsThai.length()<=0){
			lMainIsThai=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_MAIN_IS_THAI
		}
		try{
			Boolean lResult=false
			lreturn.put('MainIsThai','')
	...
	public Map inputMainThaiIdA(String strMainThaiId){
		...
		if(lMainThaiId.length()<=0){
			lMainThaiId=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_MAIN_THAI_ID
		}
		try{
			Boolean lResult=false
			lreturn.put('MainThaiId','')
	...
	public Map inputMainOccupationA(String strMainOccupation){
		...
		if(lMainOccupation.length()<=0){
			lMainOccupation=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_MAIN_OCCUPATION
		}
		try{
			Boolean lResult=false
			lreturn.put('MainOccupation','')
	...
	public Map inputMainPassportA(String strMainPassport){
		...
		if(lMainPassport.length()<=0){
			lMainPassport=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_MAIN_PASSPORT
		}
		try{
			Boolean lResult=false
			lreturn.put('MainPassport','')
			...
			String lLocatorInputText=IGNUemaHelper.getTagDataSeleniumKey(lTagInputText)
			lMainPassport=IGNUemaHelper.typeTextByLocator(this.driver,lLocatorInputText,lMainPassport)
	...
	public Map inputMainCountryA(String strMainCountry){
		...
		if(lMainCountry.length()<=0){
			lMainCountry=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_MAIN_COUNTRY
		}
		try{
			Boolean lResult=false
			lreturn.put('MainCountry','')
	...
	public Map inputMainNameA(String strMainName){
		...
		if(lMainName.length()<=0){
			lMainName=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_MAIN_NAME
		}
		try{
			Boolean lResult=false
			lreturn.put('MainName','')
	...
	public Map inputMainSurnameA(String strMainSurname){
		...
		if(lMainSurname.length()<=0){
			lMainSurname=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_MAIN_SURNAME
		}
		try{
			Boolean lResult=false
			lreturn.put('MainSurname','')
	...
	public Map inputMainEmailA(String strMainEmail){
		...
		if(lMainEmail.length()<=0){
			lMainEmail=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_MAIN_EMAIL
		}
		try{
			Boolean lResult=false
			lreturn.put('MainEmail','')
	...
	public Map inputMainPhoneNoA(String strMainPhoneNo){
		...
		if(lMainPhoneNo.length()<=0){
			lMainPhoneNo=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_MAIN_PHONE_NO
		}
		try{
			Boolean lResult=false
			lreturn.put('MainPhoneNo','')
	...
	public Map inputAdditionDriversA(String strAdditionDrivers){
		...
		if(lAdditionDrivers.length()<=0){
			lAdditionDrivers=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_ADDITION_DRIVERS
		}
		try{
			Boolean lResult=false
			lreturn.put('AdditionDrivers','')
			lreturn.put('Result',lResult)
			String lTagAdditionDrivers=''
			String lLocatorAdditionDrivers='#section-additional-drivers'
			Boolean lIsAdditionDrivers=IGNUemaHelper.checkElementVisibleByLocatorKs(this.driver,lLocatorAdditionDrivers)
			if(!lIsAdditionDrivers){
				lAdditionDrivers='No Option'
			}else{
				Integer lNumAdditionDrivers=IGNUemaHelper.convertStringToInteger(lAdditionDrivers,0)
	...
	public Map inputDriversHaveLicenseA(String strDriversHaveLicense){
		...
		if(lDriversHaveLicense.length()<=0){
			lDriversHaveLicense=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVERS_HAVE_LICENSE
		}
		try{
			Boolean lResult=false
			lreturn.put('DriversHaveLicense','')
	...
	public Map inputDriver1StatusA(String strDriver1Status){
		...
		Map lDriver1StatusList=['1':'1','2':'2','3':'3','4':'4']
		Integer lDriver1StatusDefaultMin=1
		Integer lDriver1StatusDefaultMax=4
		try{
			Boolean lResult=false
			lreturn.put('Driver1Status','')
			lreturn.put('Result',lResult)
			String lDriver1StatusItem=lDriver1StatusList.get(lDriver1Status)
			if(!lDriver1StatusItem){
				lDriver1StatusItem=lDriver1StatusDefaultMin.toString()
			}
			lDriver1Status=''
			switch(lDriver1StatusItem){
				case '2':
					String lTagMaleMarriedButtonKey='add-driver-0-gender'
					String lTagMaleMarriedButtonValue='Male-Married'
					String lLocatorMaleMarriedButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagMaleMarriedButtonKey,lTagMaleMarriedButtonValue)
					WebElement lElementMaleMarriedButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagMaleMarriedButtonKey,lTagMaleMarriedButtonValue)
					if(!lElementMaleMarriedButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementMaleMarriedButton)
						String lMaleMarriedText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementMaleMarriedButton).trim()
						lDriver1Status=lMaleMarriedText
					}
					break
				case '3':
					String lTagFemaleSingleButtonKey='add-driver-0-gender'
					String lTagFemaleSingleButtonValue='Female-Single'
					String lLocatorFemaleSingleButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagFemaleSingleButtonKey,lTagFemaleSingleButtonValue)
					WebElement lElementFemaleSingleButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagFemaleSingleButtonKey,lTagFemaleSingleButtonValue)
					if(!lElementFemaleSingleButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementFemaleSingleButton)
						String lFemaleSingleText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementFemaleSingleButton).trim()
						lDriver1Status=lFemaleSingleText
					}
					break
				case '4':
					String lTagFemaleMarriedButtonKey='add-driver-0-gender'
					String lTagFemaleMarriedButtonValue='Female-Married'
					String lLocatorFemaleMarriedButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagFemaleMarriedButtonKey,lTagFemaleMarriedButtonValue)
					WebElement lElementFemaleMarriedButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagFemaleMarriedButtonKey,lTagFemaleMarriedButtonValue)
					if(!lElementFemaleMarriedButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementFemaleMarriedButton)
						String lFemaleMarriedText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementFemaleMarriedButton).trim()
						lDriver1Status=lFemaleMarriedText
					}
					break
				default:
					String lTagMaleSingleButtonKey='add-driver-0-gender'
					String lTagMaleSingleButtonValue='Male-Single'
					String lLocatorMaleSingleButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagMaleSingleButtonKey,lTagMaleSingleButtonValue)
					WebElement lElementMaleSingleButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagMaleSingleButtonKey,lTagMaleSingleButtonValue)
					if(!lElementMaleSingleButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementMaleSingleButton)
						String lMaleSingleText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementMaleSingleButton).trim()
						lDriver1Status=lMaleSingleText
					}
					break
			}
	...
	public Map inputDriver1DobA(String strDriver1Dob){
		...
		if(lDriver1Dob.length()<=0){
			lDriver1Dob=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER1_DOB
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver1Dob','')
	...
	public Map inputDriver1IsThaiA(String strDriver1IsThai){
		...
		if(lDriver1IsThai.length()<=0){
			lDriver1IsThai=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER1_IS_THAI
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver1IsThai','')
	...
	public Map inputDriver1ThaiIdA(String strDriver1ThaiId){
		...
		if(lDriver1ThaiId.length()<=0){
			lDriver1ThaiId=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER1_THAI_ID
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver1ThaiId','')
	...
	public Map inputDriver1PassportA(String strDriver1Passport){
		...
		if(lDriver1Passport.length()<=0){
			lDriver1Passport=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER1_PASSPORT
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver1Passport','')
	...
	public Map inputDriver1CountryA(String strDriver1Country){
		...
		if(lDriver1Country.length()<=0){
			lDriver1Country=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER1_COUNTRY
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver1Country','')
	...
	public Map inputDriver1NameA(String strDriver1Name){
		...
		if(lDriver1Name.length()<=0){
			lDriver1Name=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER1_NAME
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver1Name','')
	...
	public Map inputDriver1SurnameA(String strDriver1Surname){
		...
		if(lDriver1Surname.length()<=0){
			lDriver1Surname=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER1_SURNAME
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver1Surname','')
	...
	public Map inputDriver1YearDrivingA(String strDriver1YearDriving){
		...
		if(lDriver1YearDriving.length()<=0){
			lDriver1YearDriving=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER1_YEAR_DRIVING
		}
	...
	public Map inputDriver1ClaimsNoA(String strDriver1ClaimsNo){
		...
		if(lDriver1ClaimsNo.length()<=0){
			lDriver1ClaimsNo=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER1_CLAIMS_NO
		}
	...
	public Map inputDriver1OccupationA(String strDriver1Occupation){
		...
		if(lDriver1Occupation.length()<=0){
			lDriver1Occupation=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER1_OCCUPATION
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver1Occupation','')
	...
	public Map inputDriver2StatusA(String strDriver2Status){
		...
		Map lDriver2StatusList=['1':'1','2':'2','3':'3','4':'4']
		Integer lDriver2StatusDefaultMin=1
		Integer lDriver2StatusDefaultMax=4
		try{
			Boolean lResult=false
			lreturn.put('Driver2Status','')
			lreturn.put('Result',lResult)
			String lDriver2StatusItem=lDriver2StatusList.get(lDriver2Status)
			if(!lDriver2StatusItem){
				lDriver2StatusItem=lDriver2StatusDefaultMin.toString()
			}
			lDriver2Status=''
			switch(lDriver2StatusItem){
				case '2':
					String lTagMaleMarriedButtonKey='add-driver-1-gender'
					String lTagMaleMarriedButtonValue='Male-Married'
					String lLocatorMaleMarriedButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagMaleMarriedButtonKey,lTagMaleMarriedButtonValue)
					WebElement lElementMaleMarriedButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagMaleMarriedButtonKey,lTagMaleMarriedButtonValue)
					if(!lElementMaleMarriedButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementMaleMarriedButton)
						String lMaleMarriedText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementMaleMarriedButton).trim()
						lDriver2Status=lMaleMarriedText
					}
					break
				case '3':
					String lTagFemaleSingleButtonKey='add-driver-1-gender'
					String lTagFemaleSingleButtonValue='Female-Single'
					String lLocatorFemaleSingleButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagFemaleSingleButtonKey,lTagFemaleSingleButtonValue)
					WebElement lElementFemaleSingleButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagFemaleSingleButtonKey,lTagFemaleSingleButtonValue)
					if(!lElementFemaleSingleButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementFemaleSingleButton)
						String lFemaleSingleText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementFemaleSingleButton).trim()
						lDriver2Status=lFemaleSingleText
					}
					break
				case '4':
					String lTagFemaleMarriedButtonKey='add-driver-1-gender'
					String lTagFemaleMarriedButtonValue='Female-Married'
					String lLocatorFemaleMarriedButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagFemaleMarriedButtonKey,lTagFemaleMarriedButtonValue)
					WebElement lElementFemaleMarriedButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagFemaleMarriedButtonKey,lTagFemaleMarriedButtonValue)
					if(!lElementFemaleMarriedButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementFemaleMarriedButton)
						String lFemaleMarriedText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementFemaleMarriedButton).trim()
						lDriver2Status=lFemaleMarriedText
					}
					break
				default:
					String lTagMaleSingleButtonKey='add-driver-1-gender'
					String lTagMaleSingleButtonValue='Male-Single'
					String lLocatorMaleSingleButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagMaleSingleButtonKey,lTagMaleSingleButtonValue)
					WebElement lElementMaleSingleButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagMaleSingleButtonKey,lTagMaleSingleButtonValue)
					if(!lElementMaleSingleButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementMaleSingleButton)
						String lMaleSingleText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementMaleSingleButton).trim()
						lDriver2Status=lMaleSingleText
					}
					break
			}
	...
	public Map inputDriver2DobA(String strDriver2Dob){
		...
		if(lDriver2Dob.length()<=0){
			lDriver2Dob=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER2_DOB
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver2Dob','')
	...
	public Map inputDriver2IsThaiA(String strDriver2IsThai){
		...
		if(lDriver2IsThai.length()<=0){
			lDriver2IsThai=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER2_IS_THAI
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver2IsThai','')
	...
	public Map inputDriver2ThaiIdA(String strDriver2ThaiId){
		...
		if(lDriver2ThaiId.length()<=0){
			lDriver2ThaiId=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER2_THAI_ID
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver2ThaiId','')
	...
	public Map inputDriver2PassportA(String strDriver2Passport){
		...
		if(lDriver2Passport.length()<=0){
			lDriver2Passport=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER2_PASSPORT
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver2Passport','')
	...
	public Map inputDriver2CountryA(String strDriver2Country){
		...
		if(lDriver2Country.length()<=0){
			lDriver2Country=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER2_COUNTRY
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver2Country','')
	...
	public Map inputDriver2NameA(String strDriver2Name){
		...
		if(lDriver2Name.length()<=0){
			lDriver2Name=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER2_NAME
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver2Name','')
	...
	public Map inputDriver2SurnameA(String strDriver2Surname){
		...
		if(lDriver2Surname.length()<=0){
			lDriver2Surname=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER2_SURNAME
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver2Surname','')
	...
	public Map inputDriver2YearDrivingA(String strDriver2YearDriving){
		...
		if(lDriver2YearDriving.length()<=0){
			lDriver2YearDriving=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER2_YEAR_DRIVING
		}
	...
	public Map inputDriver2ClaimsNoA(String strDriver2ClaimsNo){
		...
		if(lDriver2ClaimsNo.length()<=0){
			lDriver2ClaimsNo=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER2_CLAIMS_NO
		}
	...
	public Map inputDriver2OccupationA(String strDriver2Occupation){
		...
		if(lDriver2Occupation.length()<=0){
			lDriver2Occupation=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER2_OCCUPATION
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver2Occupation','')
	...
	public Map inputDriver3StatusA(String strDriver3Status){
		...
		Map lDriver3StatusList=['1':'1','2':'2','3':'3','4':'4']
		Integer lDriver3StatusDefaultMin=1
		Integer lDriver3StatusDefaultMax=4
		try{
			Boolean lResult=false
			lreturn.put('Driver3Status','')
			lreturn.put('Result',lResult)
			String lDriver3StatusItem=lDriver3StatusList.get(lDriver3Status)
			if(!lDriver3StatusItem){
				lDriver3StatusItem=lDriver3StatusDefaultMin.toString()
			}
			lDriver3Status=''
			switch(lDriver3StatusItem){
				case '2':
					String lTagMaleMarriedButtonKey='add-driver-2-gender'
					String lTagMaleMarriedButtonValue='Male-Married'
					String lLocatorMaleMarriedButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagMaleMarriedButtonKey,lTagMaleMarriedButtonValue)
					WebElement lElementMaleMarriedButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagMaleMarriedButtonKey,lTagMaleMarriedButtonValue)
					if(!lElementMaleMarriedButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementMaleMarriedButton)
						String lMaleMarriedText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementMaleMarriedButton).trim()
						lDriver3Status=lMaleMarriedText
					}
					break
				case '3':
					String lTagFemaleSingleButtonKey='add-driver-2-gender'
					String lTagFemaleSingleButtonValue='Female-Single'
					String lLocatorFemaleSingleButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagFemaleSingleButtonKey,lTagFemaleSingleButtonValue)
					WebElement lElementFemaleSingleButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagFemaleSingleButtonKey,lTagFemaleSingleButtonValue)
					if(!lElementFemaleSingleButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementFemaleSingleButton)
						String lFemaleSingleText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementFemaleSingleButton).trim()
						lDriver3Status=lFemaleSingleText
					}
					break
				case '4':
					String lTagFemaleMarriedButtonKey='add-driver-2-gender'
					String lTagFemaleMarriedButtonValue='Female-Married'
					String lLocatorFemaleMarriedButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagFemaleMarriedButtonKey,lTagFemaleMarriedButtonValue)
					WebElement lElementFemaleMarriedButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagFemaleMarriedButtonKey,lTagFemaleMarriedButtonValue)
					if(!lElementFemaleMarriedButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementFemaleMarriedButton)
						String lFemaleMarriedText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementFemaleMarriedButton).trim()
						lDriver3Status=lFemaleMarriedText
					}
					break
				default:
					String lTagMaleSingleButtonKey='add-driver-2-gender'
					String lTagMaleSingleButtonValue='Male-Single'
					String lLocatorMaleSingleButton=IGNUemaHelper.getTagDataSeleniumKeyValueEqual(lTagMaleSingleButtonKey,lTagMaleSingleButtonValue)
					WebElement lElementMaleSingleButton=IGNUemaHelper.getWebElementFromDataSeleniumKeyValueEqual(this.driver,lTagMaleSingleButtonKey,lTagMaleSingleButtonValue)
					if(!lElementMaleSingleButton){
						...
						IGNUemaHelper.clickByWebElementWithScroll(this.driver,lElementMaleSingleButton)
						String lMaleSingleText=IGNUemaHelper.getChildTextByWebElement(this.driver,lElementMaleSingleButton).trim()
						lDriver3Status=lMaleSingleText
					}
					break
			}
	...
	public Map inputDriver3DobA(String strDriver3Dob){
		...
		if(lDriver3Dob.length()<=0){
			lDriver3Dob=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER3_DOB
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver3Dob','')
	...
	public Map inputDriver3IsThaiA(String strDriver3IsThai){
		...
		if(lDriver3IsThai.length()<=0){
			lDriver3IsThai=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER3_IS_THAI
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver3IsThai','')
	...
	public Map inputDriver3ThaiIdA(String strDriver3ThaiId){
		...
		if(lDriver3ThaiId.length()<=0){
			lDriver3ThaiId=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER3_THAI_ID
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver3ThaiId','')
	...
	public Map inputDriver3PassportA(String strDriver3Passport){
		...
		if(lDriver3Passport.length()<=0){
			lDriver3Passport=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER3_PASSPORT
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver3Passport','')
	...
	public Map inputDriver3CountryA(String strDriver3Country){
		...
		if(lDriver3Country.length()<=0){
			lDriver3Country=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER3_COUNTRY
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver3Country','')
	...
	public Map inputDriver3NameA(String strDriver3Name){
		...
		if(lDriver3Name.length()<=0){
			lDriver3Name=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER3_NAME
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver3Name','')
	...
	public Map inputDriver3SurnameA(String strDriver3Surname){
		...
		if(lDriver3Surname.length()<=0){
			lDriver3Surname=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER3_SURNAME
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver3Surname','')
	...
	public Map inputDriver3YearDrivingA(String strDriver3YearDriving){
		...
		if(lDriver3YearDriving.length()<=0){
			lDriver3YearDriving=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER3_YEAR_DRIVING
		}
	...
	public Map inputDriver3ClaimsNoA(String strDriver3ClaimsNo){
		...
		if(lDriver3ClaimsNo.length()<=0){
			lDriver3ClaimsNo=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER3_CLAIMS_NO
		}
	...
	public Map inputDriver3OccupationA(String strDriver3Occupation){
		...
		if(lDriver3Occupation.length()<=0){
			lDriver3Occupation=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_DRIVER3_OCCUPATION
		}
		try{
			Boolean lResult=false
			lreturn.put('Driver3Occupation','')
	...
	public Map inputPolicyStartDateA(String strPolicyStartDate,String strPolicyStartDayOffset){
		...
		if(lPolicyStartDate.length()<=0){
			lPolicyStartDate=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_POLICY_START_DATE
		}
		String lFormatDate='dd/MM/yyyy'
		if(lPolicyStartDate.length()<=0){
		...
		if(lPolicyStartDayOffset.length()<=0){
			lPolicyStartDayOffset=THAExtendedWarrantyQuotationDetail1Type.DEFAULT_POLICY_START_DAY_OFFSET
		}
	...
	public Map inputPolicyInsurerA(String strPolicyInsurer){
		...
		try{
			...
			this.waitUntilOpportunityNumberUpdated()
			String lOpportunityNumber=THAExtendedWarrantyCoreType.CURRENT_QUOTATION_VEHICLE_OPPORTUNITY_NUMBER.trim()
	...
	public Map inputActualResultA(Boolean isPositiveCase,String strHolderPhoneNo,Boolean isSkipOTP){
		...
		try{
			...
			if(!lValid1){
				...
				String lLocatorPleaseWait='#myPleaseWait'
				...
				if(!lValid2){
					...
					if(lValid3){
						...
						IGNUemaHelper.delayThreadSecond(2)
						String lTagPriceChanged=''
						String lLocatorPriceChanged='#proof2screen'
							...
							if(!lValid5){
								...
								if(lOtpRes.length()>0){
									...
									if(IGNUemaHelper.selectButtonByLocator(this.driver,IGNUemaHelper.concatCssLocatorParentChild(lTagOTPDialog,lLocatorOTPButton))){
										IGNUemaHelper.delayThreadSecond(1)
										Boolean lRedirected=IGNUemaHelper.checkElementPresentByLocator(this.driver,lLocatorPaymentDocument,20)
										if(isPositiveCase){
