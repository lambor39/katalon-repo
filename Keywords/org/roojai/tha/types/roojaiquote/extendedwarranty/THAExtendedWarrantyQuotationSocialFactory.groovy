...
public class THAExtendedWarrantyQuotationSocialFactory{
	public Boolean IsValid=false
	private lCaseList=[]
	public Integer CurrentCase=0
	public THAExtendedWarrantyQuotationSocialFactory(TestData dataInput){
		super()
	...
	public Boolean validateDataInput(TestData dataInput){
		...
		try{
			...
			lreturn=(dataColNum>=THAExtendedWarrantyQuotationSocialType.INPUT_DATA_COL_BEGIN)&&(dataRowNum>=THAExtendedWarrantyQuotationSocialType.INPUT_DATA_ROW_END)
		}catch(Exception e){
			e.printStackTrace()
			return lreturn
		}
	...
	public void initCaseList(TestData dataInput){
		...
		try{
			...
			Integer dataColNum=dataInput.getColumnNumbers()
			Integer dataRowNum=dataInput.getRowNumbers()
			Integer dataColIndex=THAExtendedWarrantyQuotationSocialType.INPUT_DATA_COL_BEGIN
			Integer dataRowIndex=THAExtendedWarrantyQuotationSocialType.INPUT_DATA_ROW_BEGIN
			THAExtendedWarrantyQuotationSocialType dataCase
			Map caseInput=[:]
			caseInput.clear()
			String cellVal=''
			for(dataColIndex=THAExtendedWarrantyQuotationSocialType.INPUT_DATA_COL_BEGIN;dataColIndex<=dataColNum;dataColIndex++){
				dataCase=new THAExtendedWarrantyQuotationSocialType()
				dataCase.initInput()
				dataCase.initOutput()
				caseInput.clear()
				...
				dataRowIndex=THAExtendedWarrantyQuotationSocialType.INPUT_DATA_ROW_BEGIN
	...
	public THAExtendedWarrantyQuotationSocialType CaseData(){
	...
	public Boolean SaveOutput(){
		Boolean lreturn=false
		THAExtendedWarrantyQuotationSocialType caseData
		try{
			Integer dataColIndex=THAExtendedWarrantyQuotationSocialType.OUTPUT_EXCEL_COL_BEGIN+(this.CurrentCase-1)
			Integer dataRowIndex=THAExtendedWarrantyQuotationSocialType.OUTPUT_EXCEL_ROW_BEGIN
			if((this.CurrentCase>0)&&(this.lCaseList.size()>0)){
				caseData=this.lCaseList.get(this.CurrentCase-1)
				def xlsWorkbook=ExcelKeywords.getWorkbook(THAExtendedWarrantyQuotationSocialType.OUTPUT_EXCEL_FILE_NAME)
				def xlsSheet=ExcelKeywords.getExcelSheet(xlsWorkbook,THAExtendedWarrantyQuotationSocialType.OUTPUT_EXCEL_SHEET_NAME)
				...
				dataRowIndex+=1
				IGNUemaHelper.saveExcelWorkbookByFileName(THAExtendedWarrantyQuotationSocialType.OUTPUT_EXCEL_FILE_NAME,xlsWorkbook)
				lreturn=true
			}
		}catch(Exception e){
			e.printStackTrace()
