...
public class THAExtendedWarrantyQuotationVerifyUtil{
	private WebDriver driver=null
	private WebDriver selenium=null
	public THAExtendedWarrantyQuotationVerifyUtil(WebDriver newDriver,WebDriver newSelenium){
		super()
	...
	public Boolean inputVerify(Map caseInput,Map caseOutput){
		...
		try{
			...
			lReadOK=THAExtendedWarrantyQuotationInputType.readInputFromDataFile(GlobalVariable.CaseNumber,lCaseInputInput)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THAExtendedWarrantyQuotationInputType.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseInputOutput)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
			...
			lReadOK=THAExtendedWarrantyQuotationResultType.readInputFromDataFile(GlobalVariable.CaseNumber,lCaseResultInput)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THAExtendedWarrantyQuotationResultType.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseResultOutput)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
			...
			lReadOK=THAExtendedWarrantyQuotationDetail1Type.readInputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail1Input)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THAExtendedWarrantyQuotationDetail1Type.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail1Output)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
			...
			lReadOK=THAExtendedWarrantyQuotationPaymentType.readInputFromDataFile(GlobalVariable.CaseNumber,lCasePaymentInput)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THAExtendedWarrantyQuotationPaymentType.readOutputFromDataFile(GlobalVariable.CaseNumber,lCasePaymentOutput)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
			...
			lReadOK=THAExtendedWarrantyQuotationDetail2Type.readInputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail2Input)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THAExtendedWarrantyQuotationDetail2Type.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseDetail2Output)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
			...
			lReadOK=THAExtendedWarrantyQuotationSocialType.readInputFromDataFile(GlobalVariable.CaseNumber,lCaseSocialInput)
			IGNUemaHelper.delayThreadSecond(1)
			lReadOK=lReadOK&&THAExtendedWarrantyQuotationSocialType.readOutputFromDataFile(GlobalVariable.CaseNumber,lCaseSocialOutput)
			IGNUemaHelper.delayThreadSecond(1)
			if(!lReadOK){
				return lreturn
			}else{
