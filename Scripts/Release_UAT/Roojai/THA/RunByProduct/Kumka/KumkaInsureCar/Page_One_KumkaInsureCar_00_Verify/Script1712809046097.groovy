...
TestData dataInput=TestDataFactory.findTestData(THAKumkaInsureCarVerifyType.INPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
TestData dataOutput=TestDataFactory.findTestData(THAKumkaInsureCarVerifyType.OUTPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
THAKumkaInsureCarVerifyFactory caseFactory=new THAKumkaInsureCarVerifyFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAKumkaInsureCarVerifyType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAKumkaInsureCarVerifyUtil caseUtil=null
Boolean runNext=false
/*Input
 caseInput.KumkaInsureCar01Input
 caseInput.KumkaInsureCar02Result
 caseInput.ActualResult
 */
/*Output
 caseOutput.KumkaInsureCar01Input
 caseOutput.KumkaInsureCar02Result
...
if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
	...
	if(GlobalVariable.BrowserInit){
		...
		selenium=driver
		caseUtil=new THAKumkaInsureCarVerifyUtil(driver,selenium)
