...
import org.roojai.tha.types.kumka.insurecar.THAKumkaInsureCarInputFactory
import org.roojai.tha.types.kumka.insurecar.THAKumkaInsureCarInputType
import org.roojai.tha.types.kumka.insurecar.THAKumkaInsureCarInputUtil
...
TestData dataInput=TestDataFactory.findTestData(THAKumkaInsureCarInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAKumkaInsureCarInputType.OUTPUT_DATA_FILE_NAME)
THAKumkaInsureCarInputFactory caseFactory=new THAKumkaInsureCarInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAKumkaInsureCarInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAKumkaInsureCarInputUtil caseUtil=null
...
/*Input
 ...
 caseInput.CustomerPhoneNo
 caseInput.Customer MaritalStatusSex
 caseInput.CustomerDobPositiveCase
 caseInput.CustomerDob
 caseInput.CustomerYearOfDriving
 caseInput.CustomerNumberOfClaim
 caseInput.CustomerPostalCodePositiveCase
 caseInput.CustomerPostalCode
 caseInput.CarOdometer
 caseInput.CarUsage
 caseInput.CarDashCamera
 caseInput.CarInsuranceDue
 caseInput.CarNcbRate
 caseInput.CarCurrentInsurer
 caseInput.CarFinancing
...
/*Output
 ...
 caseOutput.CustomerPhoneNo
 caseOutput.Customer MaritalStatusSex
 caseOutput.CustomerDobPositiveCase
 caseOutput.CustomerDob
 caseOutput.CustomerYearOfDriving
 caseOutput.CustomerNumberOfClaim
 caseOutput.CustomerPostalCodePositiveCase
 caseOutput.CustomerPostalCode
 caseOutput.CarOdometer
 caseOutput.CarUsage
 caseOutput.CarDashCamera
 caseOutput.CarInsuranceDue
 caseOutput.CarNcbRate
 caseOutput.CarCurrentInsurer
 caseOutput.CarFinancing
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THAKumkaInsureCarInputUtil(driver,selenium)
			//TestName
			String lTestName='KumkaInsureCar_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
