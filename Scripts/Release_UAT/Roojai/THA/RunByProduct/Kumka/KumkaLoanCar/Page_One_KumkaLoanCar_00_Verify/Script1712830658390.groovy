...
TestData dataInput=TestDataFactory.findTestData(THAKumkaLoanCarVerifyType.INPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
TestData dataOutput=TestDataFactory.findTestData(THAKumkaLoanCarVerifyType.OUTPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
THAKumkaLoanCarVerifyFactory caseFactory=new THAKumkaLoanCarVerifyFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAKumkaLoanCarVerifyType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAKumkaLoanCarVerifyUtil caseUtil=null
Boolean runNext=false
/*Input
 caseInput.KumkaLoanCar01Input
 caseInput.KumkaLoanCar02Result
 caseInput.ActualResult
 */
/*Output
 caseOutput.KumkaLoanCar01Input
 caseOutput.KumkaLoanCar02Result
...
if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
	...
	if(GlobalVariable.BrowserInit){
		...
		selenium=driver
		caseUtil=new THAKumkaLoanCarVerifyUtil(driver,selenium)
