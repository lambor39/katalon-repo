...
import org.roojai.tha.types.kumka.loancar.THAKumkaLoanCarInputFactory
import org.roojai.tha.types.kumka.loancar.THAKumkaLoanCarInputType
import org.roojai.tha.types.kumka.loancar.THAKumkaLoanCarInputUtil
...
TestData dataInput=TestDataFactory.findTestData(THAKumkaLoanCarInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAKumkaLoanCarInputType.OUTPUT_DATA_FILE_NAME)
THAKumkaLoanCarInputFactory caseFactory=new THAKumkaLoanCarInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAKumkaLoanCarInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAKumkaLoanCarInputUtil caseUtil=null
...
/*Input
 ...
 caseInput.CustomerOccupation
 caseInput.CustomerSalaryPositiveCase
 caseInput.CustomerSalary
 caseInput.CustomerWorkExperience
 caseInput.CustomerDobPositiveCase
 caseInput.CustomerDob
 caseInput.CustomerAddressPositiveCase
 caseInput.CustomerAddress
 caseInput.CarRegisterUnder
 caseInput.CarHaveInsurance
 caseInput.CarInsuranceExpire
 caseInput.CarInsuranceOffer
 caseInput.CarInsuranceCallMeBackToDo
 caseInput.CarInsuranceCallMeBackNumber
 caseInput.CarOwnership
 caseInput.CarInstalmentPositiveCase
 caseInput.CarInstalmentPaidTerm
 caseInput.CarInstalmentAmount
 caseInput.LoanProduct01HomeLoan
 caseInput.LoanProduct02CreditCard
 caseInput.LoanProduct03PersonalLoan
 caseInput.LoanProduct04None
 caseInput.LatePayment
...
/*Output
 ...
 caseOutput.CustomerOccupation
 caseOutput.CustomerSalaryPositiveCase
 caseOutput.CustomerSalary
 caseOutput.CustomerWorkExperience
 caseOutput.CustomerDobPositiveCase
 caseOutput.CustomerDob
 caseOutput.CustomerAddressPositiveCase
 caseOutput.CustomerAddress
 caseOutput.CarRegisterUnder
 caseOutput.CarHaveInsurance
 caseOutput.CarInsuranceExpire
 caseOutput.CarInsuranceOffer
 caseOutput.CarInsuranceCallMeBackToDo
 caseOutput.CarInsuranceCallMeBackNumber
 caseOutput.CarOwnership
 caseOutput.CarInstalmentPositiveCase
 caseOutput.CarInstalmentPaidTerm
 caseOutput.CarInstalmentAmount
 caseOutput.LoanProduct01HomeLoan
 caseOutput.LoanProduct02CreditCard
 caseOutput.LoanProduct03PersonalLoan
 caseOutput.LoanProduct04None
 caseOutput.LatePayment
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THAKumkaLoanCarInputUtil(driver,selenium)
			//TestName
			String lTestName='KumkaLoanCar_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
			...
			String lLocatorToCheck2=IGNUemaHelper.getTagDataSeleniumKey('brand-select')
