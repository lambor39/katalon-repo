...
import org.roojai.tha.types.myaccount.newbiz.THAMyAccountNewbizInputFactory
import org.roojai.tha.types.myaccount.newbiz.THAMyAccountNewbizInputType
...
TestData dataInput=TestDataFactory.findTestData(THAMyAccountNewbizInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAMyAccountNewbizInputType.OUTPUT_DATA_FILE_NAME)
THAMyAccountNewbizInputFactory caseFactory=new THAMyAccountNewbizInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAMyAccountNewbizInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAMyAccountNewbizInputUtil caseUtil=null
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THAMyAccountNewbizInputUtil(driver,selenium)
			//TestName
			String lTestName='MyAccountNewbiz_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
