...
TestData dataInput=TestDataFactory.findTestData(THAMyAccountRenewalVerifyType.INPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
TestData dataOutput=TestDataFactory.findTestData(THAMyAccountRenewalVerifyType.OUTPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
THAMyAccountRenewalVerifyFactory caseFactory=new THAMyAccountRenewalVerifyFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAMyAccountRenewalVerifyType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAMyAccountRenewalVerifyUtil caseUtil=null
Boolean runNext=false
/*Input
 caseInput.MyAccountRenewal01Input
 caseInput.MyAccountRenewal02Main
 caseInput.ActualResult
 */
/*Output
 caseOutput.MyAccountRenewal01Input
 caseOutput.MyAccountRenewal02Main
...
if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
	...
	if(GlobalVariable.BrowserInit){
		...
		selenium=driver
		caseUtil=new THAMyAccountRenewalVerifyUtil(driver,selenium)
