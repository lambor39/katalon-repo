...
import org.roojai.tha.types.myaccount.renewal.THAMyAccountRenewalInputFactory
import org.roojai.tha.types.myaccount.renewal.THAMyAccountRenewalInputType
import org.roojai.tha.types.myaccount.renewal.THAMyAccountRenewalInputUtil
...
TestData dataInput=TestDataFactory.findTestData(THAMyAccountRenewalInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAMyAccountRenewalInputType.OUTPUT_DATA_FILE_NAME)
THAMyAccountRenewalInputFactory caseFactory=new THAMyAccountRenewalInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAMyAccountRenewalInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAMyAccountRenewalInputUtil caseUtil=null
...
/*Input
 ...
 caseInput.Version
 caseInput.Language
 caseInput.LoginMethod
 caseInput.LoginEmailThaiIdPassportNo
 caseInput.LoginFacebookUser
 caseInput.LoginFacebookPassword
 caseInput.OtpMethod
 caseInput.OtpEmail
 caseInput.OtpPhoneNo
 caseInput.ExpectedAccountName
...
/*Output
 ...
 caseOutput.Version
 caseOutput.Language
 caseOutput.LoginMethod
 caseOutput.LoginEmailThaiIdPassportNo
 caseOutput.LoginFacebookUser
 caseOutput.LoginFacebookPassword
 caseOutput.OtpMethod
 caseOutput.OtpEmail
 caseOutput.OtpPhoneNo
 caseOutput.ExpectedAccountName
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THAMyAccountRenewalInputUtil(driver,selenium)
			//TestName
			String lTestName='MyAccountRenewal_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
			...
			String lLocatorToCheck1='#app .form'
			...
			String lLocatorToCheck2=IGNUemaHelper.getTagDataSeleniumKey('select-language')
