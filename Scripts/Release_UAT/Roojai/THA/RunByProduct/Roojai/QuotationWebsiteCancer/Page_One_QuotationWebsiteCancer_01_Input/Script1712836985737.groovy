...
TestData dataInput=TestDataFactory.findTestData(THACancerQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THACancerQuotationInputType.OUTPUT_DATA_FILE_NAME)
THACancerQuotationInputFactory caseFactory=new THACancerQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THACancerQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THACancerQuotationInputUtil caseUtil=null
...
/*Input
 ...
 caseInput.QueryTravelledHighRiskCountry
 caseInput.QueryHealthCondition
 caseInput.QueryInsuredDeniedCoverage
 caseInput.QueryDiedFromCancer
...
/*Output
 ...
 caseOutput.CmsHeight
 caseOutput.CmsWeight
 caseOutput.CmsInsuredOccupation
 caseOutput.CmsInsuredSalary
 caseOutput.CmsSmokingHabbit
 ...
 caseOutput.QueryTravelledHighRiskCountry
 caseOutput.QueryHealthCondition
 caseOutput.QueryInsuredDeniedCoverage
 caseOutput.QueryDiedFromCancer
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(lVersion.length()<=0){
			lVersion=THACancerQuotationInputType.DEFAULT_VERSION
		}
		lVersion.toUpperCase()
		String lABTesting='&abtesting='+lVersion
		if(!(GlobalVariable.BrowserInit)){
			...
			if(lIsStartFromCmsFormMain){
				...
				if(lIsStartFromCmsFormMini){
					...
					GlobalVariable.BrowserInit=IGNBrowserConfig.openBrowser(caseInput.BaseUrl+lABTesting,IGNUemaHelper.convertStringToBoolean(caseInput.MaximizeMode),IGNUemaHelper.convertStringToBoolean(caseInput.WithAuth),caseInput.AuthUser,caseInput.AuthPassword)
				}
			}
			if(GlobalVariable.BrowserInit){
				if(lIsStartFromCmsFormMain){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THACancerQuotationInputUtil(driver,selenium)
			//TestName
			String lTestName='Cancer_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
			...
			if(lIsStartFromCmsFormMain){
				...
				if(lIsStartFromCmsFormMini){
					...
					ready1=IGNUemaHelper.checkElementPresentByElementId(driver,'illness-basicinfo-b',10)
					ready2=IGNUemaHelper.checkElementPresentByLocator(driver,'#illness-basicinfo-b',10)
				}
			}
