...
TestData dataInput=TestDataFactory.findTestData(THACriticalIllnessQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THACriticalIllnessQuotationInputType.OUTPUT_DATA_FILE_NAME)
THACriticalIllnessQuotationInputFactory caseFactory=new THACriticalIllnessQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THACriticalIllnessQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THACriticalIllnessQuotationInputUtil caseUtil=null
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(lVersion.length()<=0){
			lVersion=THACriticalIllnessQuotationInputType.DEFAULT_VERSION
		}
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THACriticalIllnessQuotationInputUtil(driver,selenium)
			//TestName
			String lTestName='CriticalIllness_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
			...
			if(lIsStartFromCmsFormMain){
				...
				if(lIsStartFromCmsFormMini){
					...
					ready1=IGNUemaHelper.checkElementPresentByElementId(driver,'ci-basicinfo',10)
					ready2=IGNUemaHelper.checkElementPresentByLocator(driver,'#ci-basicinfo',10)
				}
			}
