...
import org.roojai.tha.types.electricmotorcar.THAElectricMotorCarCoreType
import org.roojai.tha.types.electricmotorcar.THAElectricMotorCarQuotationInputFactory
import org.roojai.tha.types.electricmotorcar.THAElectricMotorCarQuotationInputType
...
TestData dataInput=TestDataFactory.findTestData(THAElectricMotorCarQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAElectricMotorCarQuotationInputType.OUTPUT_DATA_FILE_NAME)
THAElectricMotorCarQuotationInputFactory caseFactory=new THAElectricMotorCarQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAElectricMotorCarQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAElectricMotorCarQuotationInputUtil caseUtil=null
THAElectricMotorCarQuotationInputUtilOld caseUtilOld=null
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		Boolean lIsStartFromDirectElectricMotorCar=IGNUemaHelper.convertStringToBoolean(lStartFromDirectElectricMotorCar)
		lStartFromDirectElectricMotorCar=IGNUemaHelper.convertBooleanToYesNoString(lIsStartFromDirectElectricMotorCar)
		caseOutput.StartFromDirectElectricMotorCar=lStartFromDirectElectricMotorCar
		...
		if(!(GlobalVariable.BrowserInit)){
			...
			if(lIsStartFromCmsAffiliateMain){
				...
				if(lIsStartFromCmsFormMain){
					...
					if(lIsStartFromDirectElectricMotorCar){
						GlobalVariable.BrowserInit=IGNBrowserConfig.openBrowser(caseInput.BaseUrlElectricMotorCar,IGNUemaHelper.convertStringToBoolean(caseInput.MaximizeMode),IGNUemaHelper.convertStringToBoolean(caseInput.WithAuth),caseInput.AuthUser,caseInput.AuthPassword)
					}else{
						GlobalVariable.BrowserInit=IGNBrowserConfig.openBrowser(caseInput.BaseUrlMotorCar,IGNUemaHelper.convertStringToBoolean(caseInput.MaximizeMode),IGNUemaHelper.convertStringToBoolean(caseInput.WithAuth),caseInput.AuthUser,caseInput.AuthPassword)
					}
				}
			}
			if(GlobalVariable.BrowserInit){
				if(lIsStartFromCmsAffiliateMain){
					...
					if(lIsStartFromCmsFormMain){
						...
						caseOutput.BaseUrlMotorCar=caseInput.BaseUrlMotorCar
						caseOutput.BaseUrlElectricMotorCar=caseInput.BaseUrlElectricMotorCar
		...
		if(GlobalVariable.BrowserInit){
			...
			caseUtilOld=null
			//TestName
			String lTestName='MotorCar_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
			String lBaseUrl=''
			if(lIsStartFromCmsAffiliateMain){
				...
				if(lIsStartFromCmsFormMain){
					...
					if(lIsStartFromDirectElectricMotorCar){
						lBaseUrl=caseOutput.BaseUrlElectricMotorCar.toString()
					}else{
						lBaseUrl=caseOutput.BaseUrlMotorCar.toString()
					}
				}
			}
			...
			if(lIsStartFromCmsAffiliateMain){
				...
				if(lIsStartFromCmsFormMain){
					...
					if(lIsStartFromDirectElectricMotorCar){
						lBaseUrl=caseInput.BaseUrlElectricMotorCar
					}else{
						lBaseUrl=caseInput.BaseUrlMotorCar
					}
				}
			}
			if(IGNUemaHelper.CURRENT_BROWSER_IS_SAFARI){
			...
			if(lIsStartFromCmsAffiliateMain){
				...
				if(lIsStartFromCmsFormMain){
					...
					if(lIsStartFromDirectElectricMotorCar){
						ready1=IGNUemaHelper.checkElementPresentByElementId(driver,'evquote2dorothy',5)
						ready2=IGNUemaHelper.checkElementPresentByLocator(driver,'#evquote2dorothy',5)
					}else{
			...
			if(!runNext){
				...
				if(lIsStartFromCmsAffiliateMain){
					THAElectricMotorCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION=false
				}else{
					if(lIsStartFromCmsFormMain){
						THAElectricMotorCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION=false
					}else{
						THAElectricMotorCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION=IGNUemaHelper.checkWebDriverURLTargetUnderQuoteApplication(driver)
					}
				}
				if(THAElectricMotorCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION){
					caseUtil=new THAElectricMotorCarQuotationInputUtil(driver,selenium)
				}else{
					caseUtilOld=new THAElectricMotorCarQuotationInputUtilOld(driver,selenium)
				}
				//Check CmsLoaded
				if(lIsStartFromCmsFormMain){
					...
					if(THAElectricMotorCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION){
						...
						WebUI.refresh()
						IGNUemaHelper.delayThreadSecond(2)
						lMapCmsCarLanguage=caseUtilOld.inputCmsLanguage('EN')
						if(!lMapCmsCarLanguage.Result){
							WebUI.navigateToUrl(lBaseUrl)
							lMapCmsCarLanguage=caseUtilOld.inputCmsLanguage('EN')
						}else{
				...
				if(THAElectricMotorCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION){
					...
					if(lIsStartFromCmsAffiliateMain){
						runNext=caseUtilOld.inputInputFromCmsAffiliate(caseInput,caseOutput)
					}else{
						if(lIsStartFromCmsFormMain){
							runNext=caseUtilOld.inputInputFromCmsRoojai(caseInput,caseOutput)
						}else{
							runNext=caseUtilOld.inputInputFromProduct(caseInput,caseOutput)
						}
					}
				}
