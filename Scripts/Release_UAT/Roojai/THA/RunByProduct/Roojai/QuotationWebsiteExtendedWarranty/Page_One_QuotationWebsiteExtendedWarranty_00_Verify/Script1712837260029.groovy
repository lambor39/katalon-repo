...
TestData dataInput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationVerifyType.INPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
TestData dataOutput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationVerifyType.OUTPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
THAExtendedWarrantyQuotationVerifyFactory caseFactory=new THAExtendedWarrantyQuotationVerifyFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAExtendedWarrantyQuotationVerifyType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAExtendedWarrantyQuotationVerifyUtil caseUtil=null
Boolean runNext=false
/*Input
 caseInput.Quotation01Input
 caseInput.Quotation02Result
 caseInput.Quotation03Detail1
 caseInput.Quotation04Payment
 caseInput.Quotation05Detail2
 caseInput.Quotation06Social
 caseInput.ActualResult
 */
/*Output
 caseOutput.Quotation01Input
 caseOutput.Quotation02Result
 caseOutput.Quotation03Detail1
 caseOutput.Quotation04Payment
 caseOutput.Quotation05Detail2
 caseOutput.Quotation06Social
...
if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
	...
	if(GlobalVariable.BrowserInit){
		...
		selenium=driver
		caseUtil=new THAExtendedWarrantyQuotationVerifyUtil(driver,selenium)
