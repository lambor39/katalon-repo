...
TestData dataInput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationInputType.OUTPUT_DATA_FILE_NAME)
THAExtendedWarrantyQuotationInputFactory caseFactory=new THAExtendedWarrantyQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAExtendedWarrantyQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAExtendedWarrantyQuotationInputUtil caseUtil=null
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			caseUtil=null
			//TestName
			String lTestName='ExtendedWarranty_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
			String lBaseUrl=''
			if(lIsStartFromCmsFormMain){
			...
			if(lIsStartFromCmsFormMain){
				...
				if(lIsStartFromCmsFormMini){
					lLocatorToCheck1='#main .section-header a'
					lBaseUrl=caseInput.CmsBaseUrlFormMain
				}else{
			...
			if(lIsStartFromCmsFormMain){
				...
				if(lIsStartFromCmsFormMini){
					...
					ready1=IGNUemaHelper.checkElementPresentByElementId(driver,'extendedWarranty-basicinfo',5)
					ready2=IGNUemaHelper.checkElementPresentByLocator(driver,'#extendedWarranty-basicinfo',5)
				}
			}
			...
			if(!runNext){
				...
				caseFactory.SaveOutput()
				caseUtil=new THAExtendedWarrantyQuotationInputUtil(driver,selenium)
				//Check ABTesting
