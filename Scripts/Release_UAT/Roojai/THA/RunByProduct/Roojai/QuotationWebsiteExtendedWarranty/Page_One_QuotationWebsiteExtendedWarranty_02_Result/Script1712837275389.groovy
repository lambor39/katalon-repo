...
import org.roojai.tha.types.roojaiquote.extendedwarranty.THAExtendedWarrantyQuotationResultFactory
import org.roojai.tha.types.roojaiquote.extendedwarranty.THAExtendedWarrantyQuotationResultType
...
TestData dataInput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationResultType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationResultType.OUTPUT_DATA_FILE_NAME)
THAExtendedWarrantyQuotationResultFactory caseFactory=new THAExtendedWarrantyQuotationResultFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAExtendedWarrantyQuotationResultType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAExtendedWarrantyQuotationResultUtil caseUtil=null
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			if(!runNext){
				...
				caseFactory.SaveOutput()
				caseUtil=new THAExtendedWarrantyQuotationResultUtil(driver,selenium)
				//Check ABTesting
