...
import org.roojai.tha.types.roojaiquote.extendedwarranty.THAExtendedWarrantyCoreType
import org.roojai.tha.types.roojaiquote.extendedwarranty.THAExtendedWarrantyQuotationPaymentFactory
import org.roojai.tha.types.roojaiquote.extendedwarranty.THAExtendedWarrantyQuotationPaymentType
...
TestData dataInput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationPaymentType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationPaymentType.OUTPUT_DATA_FILE_NAME)
THAExtendedWarrantyQuotationPaymentFactory caseFactory=new THAExtendedWarrantyQuotationPaymentFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAExtendedWarrantyQuotationPaymentType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAExtendedWarrantyQuotationPaymentUtil caseUtil=null
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THAExtendedWarrantyQuotationPaymentUtil(driver,selenium)
