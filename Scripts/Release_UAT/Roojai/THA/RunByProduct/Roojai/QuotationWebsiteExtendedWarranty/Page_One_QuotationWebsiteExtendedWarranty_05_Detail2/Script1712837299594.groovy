...
import org.roojai.tha.types.roojaiquote.extendedwarranty.THAExtendedWarrantyQuotationDetail2Factory
import org.roojai.tha.types.roojaiquote.extendedwarranty.THAExtendedWarrantyQuotationDetail2Type
...
TestData dataInput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationDetail2Type.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationDetail2Type.OUTPUT_DATA_FILE_NAME)
THAExtendedWarrantyQuotationDetail2Factory caseFactory=new THAExtendedWarrantyQuotationDetail2Factory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAExtendedWarrantyQuotationDetail2Type caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAExtendedWarrantyQuotationDetail2Util caseUtil=null
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THAExtendedWarrantyQuotationDetail2Util(driver,selenium)
			...
			if(!runNext){
				...
				if(caseOutput.PositiveCase.toString().length()>0){
					if(!IGNUemaHelper.convertStringToBoolean(caseOutput.PositiveCase.toString())){
						...
						Map lMapPolicyInsurer=[:]
						if(THAExtendedWarrantyCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION){
							lMapPolicyInsurer=caseUtil.inputPolicyInsurer(lPolicyInsurer)
						}
