...
import org.roojai.tha.types.roojaiquote.extendedwarranty.THAExtendedWarrantyQuotationSocialFactory
import org.roojai.tha.types.roojaiquote.extendedwarranty.THAExtendedWarrantyQuotationSocialType
...
TestData dataInput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationSocialType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAExtendedWarrantyQuotationSocialType.OUTPUT_DATA_FILE_NAME)
THAExtendedWarrantyQuotationSocialFactory caseFactory=new THAExtendedWarrantyQuotationSocialFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAExtendedWarrantyQuotationSocialType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAExtendedWarrantyQuotationSocialUtil caseUtil=null
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THAExtendedWarrantyQuotationSocialUtil(driver,selenium)
