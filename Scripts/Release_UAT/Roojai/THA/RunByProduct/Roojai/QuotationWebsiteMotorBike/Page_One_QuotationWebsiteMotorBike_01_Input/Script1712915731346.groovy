...
import org.roojai.tha.types.motorbike.THAMotorBikeCoreType
...
TestData dataInput=TestDataFactory.findTestData(THAMotorBikeQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAMotorBikeQuotationInputType.OUTPUT_DATA_FILE_NAME)
THAMotorBikeQuotationInputFactory caseFactory=new THAMotorBikeQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAMotorBikeQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAMotorBikeQuotationInputUtil caseUtil=null
THAMotorBikeQuotationInputUtilOld caseUtilOld=null
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			caseUtilOld=null
			//TestName
			String lTestName='MotorBike_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
			String lBaseUrl=''
			if(lIsStartFromCmsAffiliateMain){
			...
			if(lIsStartFromCmsAffiliateMain){
				...
				if(lIsStartFromCmsFormMain){
					...
					ready1=IGNUemaHelper.checkElementPresentByElementId(driver,'quotation-input-q2-2021',5)
					ready2=IGNUemaHelper.checkElementPresentByLocator(driver,'#quotation-input-q2-2021',5)
					runNext=(ready1&&ready2)
					if(!runNext){
						ready1=IGNUemaHelper.checkElementPresentByElementId(driver,'quotation-v3',5)
						ready2=IGNUemaHelper.checkElementPresentByLocator(driver,'#quotation-v3',5)
					}
				}
			}
			runNext=(ready1&&ready2)
			...
			if(!runNext){
				...
				if(lIsStartFromCmsAffiliateMain){
					THAMotorBikeCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION=false
				}else{
					if(lIsStartFromCmsFormMain){
						THAMotorBikeCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION=false
					}else{
						THAMotorBikeCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION=IGNUemaHelper.checkWebDriverURLTargetUnderQuoteApplication(driver)
					}
				}
				if(THAMotorBikeCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION){
					caseUtil=new THAMotorBikeQuotationInputUtil(driver,selenium)
				}else{
					caseUtilOld=new THAMotorBikeQuotationInputUtilOld(driver,selenium)
				}
				//Check CmsLoaded
				if(lIsStartFromCmsFormMain){
