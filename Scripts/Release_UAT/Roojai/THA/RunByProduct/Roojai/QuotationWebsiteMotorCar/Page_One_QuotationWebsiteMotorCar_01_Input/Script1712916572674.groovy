...
TestData dataInput=TestDataFactory.findTestData(THAMotorCarQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THAMotorCarQuotationInputType.OUTPUT_DATA_FILE_NAME)
THAMotorCarQuotationInputFactory caseFactory=new THAMotorCarQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THAMotorCarQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THAMotorCarQuotationInputUtil caseUtil=null
THAMotorCarQuotationInputUtilOld caseUtilOld=null
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			if(!runNext){
				...
				if(lIsStartFromCmsAffiliateMain){
					THAMotorCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION=false
				}else{
					if(lIsStartFromCmsFormMain){
						THAMotorCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION=false
					}else{
				...
				if(THAMotorCarCoreType.CURRENT_URL_UNDER_QUOTE_APPLICATION){
					caseUtil=new THAMotorCarQuotationInputUtil(driver,selenium)
				}else{
					caseUtilOld=new THAMotorCarQuotationInputUtilOld(driver,selenium)
				}
				//Check CmsLoaded
				if(lIsStartFromCmsFormMain){
