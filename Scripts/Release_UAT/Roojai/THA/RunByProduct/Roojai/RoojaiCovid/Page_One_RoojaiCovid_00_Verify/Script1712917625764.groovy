...
TestData dataInput=TestDataFactory.findTestData(THARoojaiCovidQuotationVerifyType.INPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
TestData dataOutput=TestDataFactory.findTestData(THARoojaiCovidQuotationVerifyType.OUTPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
THARoojaiCovidQuotationVerifyFactory caseFactory=new THARoojaiCovidQuotationVerifyFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARoojaiCovidQuotationVerifyType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARoojaiCovidQuotationVerifyUtil caseUtil=null
Boolean runNext=false
/*Input
 caseInput.RoojaiCovidInput
 caseInput.RoojaiCovidDetail
 caseInput.RoojaiCovidPayment
 caseInput.ActualResult
 */
/*Output
 caseOutput.RoojaiCovidInput
 caseOutput.RoojaiCovidDetail
 caseOutput.RoojaiCovidPayment
...
if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
	...
	if(GlobalVariable.BrowserInit){
		...
		selenium=driver
		caseUtil=new THARoojaiCovidQuotationVerifyUtil(driver,selenium)
