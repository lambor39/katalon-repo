...
import org.roojai.tha.types.roojaicovid.THARoojaiCovidQuotationInputFactory
import org.roojai.tha.types.roojaicovid.THARoojaiCovidQuotationInputType
import org.roojai.tha.types.roojaicovid.THARoojaiCovidQuotationInputUtil
...
TestData dataInput=TestDataFactory.findTestData(THARoojaiCovidQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THARoojaiCovidQuotationInputType.OUTPUT_DATA_FILE_NAME)
THARoojaiCovidQuotationInputFactory caseFactory=new THARoojaiCovidQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARoojaiCovidQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARoojaiCovidQuotationInputUtil caseUtil=null
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THARoojaiCovidQuotationInputUtil(driver,selenium)
			//TestName
			String lTestName='RoojaiCovid_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
			...
			String lLocatorToCheck1='#collapsingNavbar'
			if(IGNUemaHelper.CURRENT_BROWSER_IS_SAFARI){
				IGNUemaHelper.switchSmartWaitDelay(true)
			...
			Boolean ready1=IGNUemaHelper.checkElementPresentByElementId(driver,'collapsingNavbar',5)
			Boolean ready2=IGNUemaHelper.checkElementPresentByLocator(driver,'#collapsingNavbar',5)
			...
			if(!runNext){
				...
				caseFactory.SaveOutput()
				caseOutput.Language=caseUtil.inputLanguage(caseInput.Language.toString())
				runNext=caseOutput.Language.toString().length()>0
			}
