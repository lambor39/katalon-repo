...
TestData dataInput=TestDataFactory.findTestData(THARGPACancerQuotationVerifyType.INPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
TestData dataOutput=TestDataFactory.findTestData(THARGPACancerQuotationVerifyType.OUTPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
THARGPACancerQuotationVerifyFactory caseFactory=new THARGPACancerQuotationVerifyFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARGPACancerQuotationVerifyType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARGPACancerQuotationVerifyUtil caseUtil=null
Boolean runNext=false
/*Input
 caseInput.Cancer01Input
 caseInput.Cancer02Result
 caseInput.Cancer03Detail1
 caseInput.Cancer04Payment
 caseInput.ActualResult
 */
/*Output
 caseOutput.Cancer01Input
 caseOutput.Cancer02Result
 caseOutput.Cancer03Detail1
 caseOutput.Cancer04Payment
...
if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
	...
	if(GlobalVariable.BrowserInit){
		...
		selenium=driver
		caseUtil=new THARGPACancerQuotationVerifyUtil(driver,selenium)
