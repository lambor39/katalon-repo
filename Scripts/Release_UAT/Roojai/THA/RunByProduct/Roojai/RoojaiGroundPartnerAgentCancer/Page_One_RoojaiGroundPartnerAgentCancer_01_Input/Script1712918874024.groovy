...
import org.roojai.tha.types.roojaigroundpartneragent.cancer.THARGPACancerCoreType
...
TestData dataInput=TestDataFactory.findTestData(THARGPACancerQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THARGPACancerQuotationInputType.OUTPUT_DATA_FILE_NAME)
THARGPACancerQuotationInputFactory caseFactory=new THARGPACancerQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARGPACancerQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARGPACancerQuotationInputUtil caseUtil=null
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(!(GlobalVariable.BrowserInit)){
			...
			if(GlobalVariable.BrowserInit){
				...
				THARGPACancerCoreType.CURRENT_GROUND_PARTNER_AGENT_URL_PORTAL=caseOutput.BaseUrl
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THARGPACancerQuotationInputUtil(driver,selenium)
			//TestName
			String lTestName='RGPACancer_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
