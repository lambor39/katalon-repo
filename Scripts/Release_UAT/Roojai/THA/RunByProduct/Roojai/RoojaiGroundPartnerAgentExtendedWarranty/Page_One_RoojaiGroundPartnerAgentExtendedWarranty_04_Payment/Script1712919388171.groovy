...
import org.roojai.tha.types.roojaigroundpartneragent.roojaiquote.extendedwarranty.THARGPAExtendedWarrantyQuotationPaymentFactory
import org.roojai.tha.types.roojaigroundpartneragent.roojaiquote.extendedwarranty.THARGPAExtendedWarrantyQuotationPaymentType
import org.roojai.tha.types.roojaigroundpartneragent.roojaiquote.extendedwarranty.THARGPAExtendedWarrantyQuotationPaymentUtil
...
TestData dataInput=TestDataFactory.findTestData(THARGPAExtendedWarrantyQuotationPaymentType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THARGPAExtendedWarrantyQuotationPaymentType.OUTPUT_DATA_FILE_NAME)
THARGPAExtendedWarrantyQuotationPaymentFactory caseFactory=new THARGPAExtendedWarrantyQuotationPaymentFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARGPAExtendedWarrantyQuotationPaymentType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARGPAExtendedWarrantyQuotationPaymentUtil caseUtil=null
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THARGPAExtendedWarrantyQuotationPaymentUtil(driver,selenium)
			IGNUemaHelper.delayThreadSecond(0)
			Boolean ready1=IGNUemaHelper.checkElementPresentByElementId(driver,'illness-payment',10)
			Boolean ready2=IGNUemaHelper.checkElementPresentByLocator(driver,'#illness-payment',10)
