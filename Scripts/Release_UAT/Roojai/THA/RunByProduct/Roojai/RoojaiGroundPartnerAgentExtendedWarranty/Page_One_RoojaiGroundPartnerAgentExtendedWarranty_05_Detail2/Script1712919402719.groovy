...
import org.roojai.tha.types.roojaigroundpartneragent.roojaiquote.extendedwarranty.THARGPAExtendedWarrantyQuotationDetail2Factory
import org.roojai.tha.types.roojaigroundpartneragent.roojaiquote.extendedwarranty.THARGPAExtendedWarrantyQuotationDetail2Type
import org.roojai.tha.types.roojaigroundpartneragent.roojaiquote.extendedwarranty.THARGPAExtendedWarrantyQuotationDetail2Util
...
TestData dataInput=TestDataFactory.findTestData(THARGPAExtendedWarrantyQuotationDetail2Type.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THARGPAExtendedWarrantyQuotationDetail2Type.OUTPUT_DATA_FILE_NAME)
THARGPAExtendedWarrantyQuotationDetail2Factory caseFactory=new THARGPAExtendedWarrantyQuotationDetail2Factory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARGPAExtendedWarrantyQuotationDetail2Type caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARGPAExtendedWarrantyQuotationDetail2Util caseUtil=null
...
/*Input
 ...
 caseInput.FinancialOrganisation
 caseInput.WithRedPlate
 caseInput.ChassisNumber
 caseInput.PrevInsuranceCovered
 caseInput.PrevInsuranceCompany
 caseInput.PrevInsuranceType
 caseInput.PolicyExpirationDayOffset
 caseInput.PolicyExpirationDate
 caseInput.PolicyStartDayOffset
 caseInput.PolicyStartDate
 caseInput.SendToPostalCode
 caseInput.SendToProvince
 caseInput.SendToDistrict
 caseInput.SendToSubDistrict
 caseInput.SendToRoad
 caseInput.SendToBuildingName
 caseInput.SendToBuildingNumber
...
/*Output
 ...
 caseOutput.FinancialOrganisation
 caseOutput.WithRedPlate
 caseOutput.ChassisNumber
 caseOutput.PrevInsuranceCovered
 caseOutput.PrevInsuranceCompany
 caseOutput.PrevInsuranceType
 caseOutput.PolicyExpirationDayOffset
 caseOutput.PolicyExpirationDate
 caseOutput.PolicyStartDayOffset
 caseOutput.PolicyStartDate
 caseOutput.SendToPostalCode
 caseOutput.SendToProvince
 caseOutput.SendToDistrict
 caseOutput.SendToSubDistrict
 caseOutput.SendToRoad
 caseOutput.SendToBuildingName
 caseOutput.SendToBuildingNumber
 caseOutput.PositiveCase
 caseOutput.ActualResult
 caseOutput.OpportunityNumber
 caseOutput.PolicyAmtYearly
 caseOutput.PolicyAmtInstalment
...
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THARGPAExtendedWarrantyQuotationDetail2Util(driver,selenium)
			IGNUemaHelper.delayThreadSecond(0)
			Boolean ready1=IGNUemaHelper.checkElementPresentByElementId(driver,'policy2',20)
			Boolean ready2=IGNUemaHelper.checkElementPresentByLocator(driver,'#policy2',20)
			...
			if(!runNext){
				...
				runNext=caseUtil.inputDetail2(caseInput,caseOutput)
