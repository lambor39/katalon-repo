...
import org.roojai.tha.types.roojaigroundpartneragent.roojaiquote.extendedwarranty.THARGPAExtendedWarrantyQuotationSocialFactory
import org.roojai.tha.types.roojaigroundpartneragent.roojaiquote.extendedwarranty.THARGPAExtendedWarrantyQuotationSocialType
import org.roojai.tha.types.roojaigroundpartneragent.roojaiquote.extendedwarranty.THARGPAExtendedWarrantyQuotationSocialUtil
...
TestData dataInput=TestDataFactory.findTestData(THARGPAExtendedWarrantyQuotationSocialType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THARGPAExtendedWarrantyQuotationSocialType.OUTPUT_DATA_FILE_NAME)
THARGPAExtendedWarrantyQuotationSocialFactory caseFactory=new THARGPAExtendedWarrantyQuotationSocialFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARGPAExtendedWarrantyQuotationSocialType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARGPAExtendedWarrantyQuotationSocialUtil caseUtil=null
...
/*Input
 ...
 caseInput.Language
 caseInput.RatingForOverall
 caseInput.RatingForWebsite
 caseInput.RatingForCallCenter
 caseInput.RatingForProduct
 caseInput.RatingText
 caseInput.WithFacebook
 caseInput.FacebookLoginId
 caseInput.FacebookLoginPassword
 caseInput.ShareFacebook
...
/*Output
 ...
 caseOutput.Language
 caseOutput.RatingForOverall
 caseOutput.RatingForWebsite
 caseOutput.RatingForCallCenter
 caseOutput.RatingForProduct
 caseOutput.RatingText
 caseOutput.WithFacebook
 caseOutput.FacebookLoginId
 caseOutput.FacebookLoginPassword
 caseOutput.ShareFacebook
 caseOutput.PositiveCase
 caseOutput.ActualResult
 caseOutput.PolicyInsurer
 caseOutput.PolicyPaymentDetail
 */
if(GlobalVariable.PrevStatus){
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THARGPAExtendedWarrantyQuotationSocialUtil(driver,selenium)
			IGNUemaHelper.delayThreadSecond(0)
			Boolean ready1=IGNUemaHelper.checkElementPresentByElementId(driver,'new-paymentComplete',10)
			Boolean ready2=IGNUemaHelper.checkElementPresentByLocator(driver,'#new-paymentComplete',10)
			...
			if(!runNext){
				...
				runNext=caseUtil.inputSocial(caseInput,caseOutput)
				caseOutput.FacebookLoginPassword=IGNUemaHelper.convertStringToPassword(caseOutput.FacebookLoginPassword.toString())
