...
TestData dataInput=TestDataFactory.findTestData(THARGPALowMileageCarQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THARGPALowMileageCarQuotationInputType.OUTPUT_DATA_FILE_NAME)
THARGPALowMileageCarQuotationInputFactory caseFactory=new THARGPALowMileageCarQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARGPALowMileageCarQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARGPALowMileageCarQuotationInputUtil caseUtil=null
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(!(GlobalVariable.BrowserInit)){
			...
			if(GlobalVariable.BrowserInit){
				...
				THARGPALowMileageCarCoreType.CURRENT_GROUND_PARTNER_AGENT_URL_PORTAL=caseOutput.BaseUrl
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THARGPALowMileageCarQuotationInputUtil(driver,selenium)
			//TestName
			String lTestName='RGPAMotor_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
