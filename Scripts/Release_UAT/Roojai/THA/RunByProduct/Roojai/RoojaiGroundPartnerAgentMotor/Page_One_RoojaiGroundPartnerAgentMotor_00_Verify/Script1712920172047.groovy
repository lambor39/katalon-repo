...
TestData dataInput=TestDataFactory.findTestData(THARGPAMotorQuotationVerifyType.INPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
TestData dataOutput=TestDataFactory.findTestData(THARGPAMotorQuotationVerifyType.OUTPUT_DATA_FILE_NAME)
IGNUemaHelper.delayThreadSecond(1)
THARGPAMotorQuotationVerifyFactory caseFactory=new THARGPAMotorQuotationVerifyFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARGPAMotorQuotationVerifyType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARGPAMotorQuotationVerifyUtil caseUtil=null
...
if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
	...
	if(GlobalVariable.BrowserInit){
		...
		selenium=driver
		caseUtil=new THARGPAMotorQuotationVerifyUtil(driver,selenium)
