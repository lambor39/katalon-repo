...
TestData dataInput=TestDataFactory.findTestData(THARGPAMotorQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THARGPAMotorQuotationInputType.OUTPUT_DATA_FILE_NAME)
THARGPAMotorQuotationInputFactory caseFactory=new THARGPAMotorQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARGPAMotorQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARGPAMotorQuotationInputUtil caseUtil=null
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(!(GlobalVariable.BrowserInit)){
			...
			if(GlobalVariable.BrowserInit){
				...
				THARGPAMotorCoreType.CURRENT_GROUND_PARTNER_AGENT_URL_PORTAL=caseOutput.BaseUrl
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THARGPAMotorQuotationInputUtil(driver,selenium)
			//TestName
