...
import org.roojai.tha.types.roojaigroundpartneragent.personalaccident.THARGPAPersonalAccidentCoreType
...
TestData dataInput=TestDataFactory.findTestData(THARGPAPersonalAccidentQuotationInputType.INPUT_DATA_FILE_NAME)
TestData dataOutput=TestDataFactory.findTestData(THARGPAPersonalAccidentQuotationInputType.OUTPUT_DATA_FILE_NAME)
THARGPAPersonalAccidentQuotationInputFactory caseFactory=new THARGPAPersonalAccidentQuotationInputFactory(dataInput)
caseFactory.CurrentCase=GlobalVariable.CaseNumber
THARGPAPersonalAccidentQuotationInputType caseData=caseFactory.CaseData()
Map caseInput=caseData.Input
Map caseOutput=caseData.Output
WebDriver driver=null
WebDriver selenium=null
THARGPAPersonalAccidentQuotationInputUtil caseUtil=null
	...
	if(caseFactory.IsValid &&(caseFactory.CurrentCase>0)){
		...
		if(!(GlobalVariable.BrowserInit)){
			...
			if(GlobalVariable.BrowserInit){
				...
				THARGPAPersonalAccidentCoreType.CURRENT_GROUND_PARTNER_AGENT_URL_PORTAL=caseOutput.BaseUrl
		...
		if(GlobalVariable.BrowserInit){
			...
			selenium=driver
			caseUtil=new THARGPAPersonalAccidentQuotationInputUtil(driver,selenium)
			//TestName
			String lTestName='RGPAPersonalAccident_'+IGNUemaHelper.getExecutionProfile()+'_'+IGNUemaHelper.getCapabilityBrowserName(driver)+'_'+IGNUemaHelper.convertLocalDateTimeToString('yyyyMMddHHmmss')
